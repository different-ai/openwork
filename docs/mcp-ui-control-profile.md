# Control Micx from any MCP client

Micx exposes its UI as an MCP server so any MCP-capable app can read what's on screen and run actions — no DOM scraping, no coordinates, no accessibility hacks.
...
Micx takes a different approach: the app itself tells you what actions are available, what the current state is, and lets you execute actions by name. The MCP server wraps that surface so any MCP client gets a first-class, semantic control experience out of the box.

This means:

- **HandsFree** can drive Micx sessions, composer, navigation, and transcript without guessing pixels.
- **OpenCode** can automate Micx as part of a larger coding workflow.
- **Claude Desktop, Codex, Cursor**, or any MCP-compatible tool can add Micx control with a single config line.
- Your own app can do the same.

> Want to control Micx Cloud workers and server APIs instead of the desktop UI? Check out the **Micx Cloud MCP** (separate package, coming soon).

## Quick start with HandsFree

HandsFree auto-discovers the Micx MCP server when both apps are running on the same machine. No config needed.

1. Launch **Micx** (desktop app).
2. Launch **HandsFree**.
3. Open the HandsFree connector panel — you should see **Micx** with a green "Connected" status and an action count.
...
- `ui_status` — check if Micx is running and the bridge is reachable.

### Cross-session memory

Micx's cross-session memory currently comes from saved session history exposed through the UI control surface. It is not a separate long-term memory database.
...
This may navigate Micx away from the user's current session while the lookup runs. If multiple sessions match, ask which one to inspect.

### Micx agents

Inside Micx, agents control the app through the semantic tools (`micx_context`, `micx_query`, `micx_execute`) using affordance ids from context. External MCP clients can also use the hidden **Micx UI Control** MCP via **Settings -> Extensions -> Show hidden**.

## Install

```bash
npm install -g micx-ui-mcp
```
...
> The package is [`micx-ui-mcp` on npm](https://www.npmjs.com/package/micx-ui-mcp).
...
```
> Use ui_snapshot to see what's on screen in Micx, then list the available sessions.
```
...
If you're building an app that speaks MCP, you can connect to the Micx UI server the same way:
...
Check if Micx is running and reachable. Returns connection status and app info.
...
Get the current Micx UI state: active route, narration, visible actions, and status. Call this before acting to understand what the user sees.
...
Execute an Micx UI action by its id.
...
The exact list depends on the current Micx route and state. Common actions include:
...
- **Micx desktop** must be running. The MCP server connects to Micx's local bridge which starts automatically when the desktop app launches.
- **macOS** is the primary supported platform. The bridge uses Electron IPC and writes a discovery file to `~/Library/Application Support/com.differentai.micx/`.
...
│  MCP client  │ ←────────────────→ │  micx-ui-mcp │ ←───────────────────→ │  Micx app │
...
1. Micx desktop starts a private localhost HTTP bridge on a random port, protected by a bearer token.
2. It writes a discovery file with the port and token so `micx-ui-mcp` can find it.
3. `micx-ui-mcp` reads the discovery file, proxies MCP tool calls to the bridge, and returns structured results.
4. The bridge calls `window.__micxControl` inside the Electron renderer to snapshot state and execute actions.

The bridge and discovery file are implementation details — you never need to touch them directly. Just point your MCP client at `micx-ui-mcp`.
