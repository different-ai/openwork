...
The latest CI also found a v1 first-send interruption and a packaged enterprise startup recovery screen. The first-send interruption and packaged-startup failure remain required checks. Existing free-starter compatibility is excluded from the v2 gate by the rollout decision above. Do not enable v2 by default on the strength of a passing subset.
