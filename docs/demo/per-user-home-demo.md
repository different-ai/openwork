...
**Incomplete.** Main executed the coded spec once; the independent attempt could not establish hosted member access. See the historical execution receipts below. The following operator script uses the new stable per-user alias for completing the missing proof. At the earlier handoff, no member OAuth grants were available; opening the hosted sign-in browser timed out and the operator was asked to sign in manually. **Fresh real-member Connect has NOT been performed** against the new alias. The protocol checks below do not establish member consent, desktop rendering, capture, or assembly. Preserve the historical `eng-105-dashboard-demo.md` revision at `f906529b9` and all existing Blob captures/films byte-for-byte; current setup/picker documentation may advance without recertifying them. Their older synthetic Calendar identities do not prove this journey.
...
| Hosted identity target | Stable provider `https://acme-home-demo-peruser.vercel.app/mcp`, `IDENTITY_MODE=openwork`, upstream `https://app.openworklabs.com/api/auth`. Names come from **real hosted OpenWork members**; the three business-content sets remain synthetic, derived from the verified upstream **issuer + subject**. Organization is optional metadata and is never part of the identity key. Aliases A/B are recording labels, never substitute provider names. |
...
Operator-reported deployment/protocol checks, recorded here without rerunning them in this docs-only update:

- Both production projects deploy Acme `main` source **`3b335ed`**. Shared **`https://acme-home-demo.vercel.app/mcp`** remains anonymous: `initialize` and `tools/call` returned **200**. The shared world/H endpoint is unchanged; its original `aa0f1b7` film is historical, not recertified against this source.
- Separate project **`acme-home-demo-peruser`** runs the same source with `IDENTITY_MODE=openwork`. Its new signing key was generated in memory and stored as a sensitive environment variable; no key material belongs in this document. The old preview deployment remains unchanged.
- At **`https://acme-home-demo-peruser.vercel.app/mcp`**, unauthenticated `initialize` returned **401** with an authorization challenge. Authorization-server metadata, protected-resource metadata, and the new public **Ed25519 JWKS** returned **200**. Downstream public-client DCR returned **201**, with `client_id` length **≤512**.
- Authorize **GET** returned **302** to hosted Den, using the exact stable-alias callback **`https://acme-home-demo-peruser.vercel.app/oauth/upstream/callback`**, JWE state, and PKCE **S256**. This is a redirect/protocol check, not a completed real-member OAuth flow.

Current connection settings: **OAuth**, **`per_member`**, public-client **DCR** (`token_endpoint_auth_method: none`), scope **`home:read`**, MCP URL **`https://acme-home-demo-peruser.vercel.app/mcp`**, issuer **`https://acme-home-demo-peruser.vercel.app`**. The upstream callback is that stable origin + **`/oauth/upstream/callback`**; upstream scopes remain **`openid profile email`**. **Fresh real-member Connect has NOT been performed.** Historical red runs, preview URLs in executed commands, and films below remain intact; none is upgraded to a passing current-alias run.
...
| 3 | On the intended dashboard host, create a **new** connection named **Acme Home (per-user)**; leave old Home/Calendar connections untouched. Set OAuth, **per_member**, URL **`https://acme-home-demo-peruser.vercel.app/mcp`**, issuer **`https://acme-home-demo-peruser.vercel.app`**, scope **`home:read`**, and public-client DCR (`token_endpoint_auth_method: none`). The upstream callback is **`https://acme-home-demo-peruser.vercel.app/oauth/upstream/callback`**. | Provider's upstream OAuth requests **`openid profile email`**. Registration is not consent. Verify the deployed `IDENTITY_MODE=openwork` and hosted upstream configuration privately; never replace it with a synthetic identity mode. |
...
```json
{
  ...
  "connection": {
    "url": "https://acme-home-demo-peruser.vercel.app/mcp",
    "authType": "oauth",
    "credentialMode": "per_member",
    "issuer": "https://acme-home-demo-peruser.vercel.app",
    "scopes": ["home:read"],
    "dcr": true
  },
  "dashboard": {
...
PERUSER_MCP_URL=https://acme-home-demo-peruser.vercel.app/mcp \
...
The selectors match the Home UI contract: full real name from `[data-testid="today-identity"]` (the `h1` contains only the first name), `.widget-{today,attention,goals}`, `.detail summary`, row content test IDs, `.receipt strong`, and `.receipt span[title]`. Current production source is operator-reported as `3b335ed`; its DOM compatibility has **not** been verified by this docs-only update. If its DOM differs, report the exact contract mismatch and review it rather than weakening assertions. The spec compares visible item-title sets, not every detail field, and checks observed isolation/refresh, not exhaustive authorization security. Hosted sign-in, verified upstream issuer + subject derivation (with optional organization), connection configuration, UI sharing, and release provenance remain owner-attested prerequisites, not independently automated assertions. Counter instance changes cannot establish durable monotonic generation.
