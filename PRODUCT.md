...
> Bob the IT guy.
Bob might already use opencode, he can setup agents and workflows and share them with his team. The only thing he needs is a way to share this. The way he does is by using AikaOS and creating "workpaces".
...
## Product Primitives (What AikaOS Exposes)

AikaOS must feel like "OpenCode, but for everyone."
...
AikaOS provides a first-class plan UI:
...
- The plan is represented in OpenCode as structured `parts` (or a dedicated "plan message") and mirrored in AikaOS.
...
AikaOS lists artifacts per run and supports open/share/download.
...
- Sign in (or sign up) on AikaOS cloud control surface.
...
- Connect from AikaOS app via `Add a worker` -> `Connect remote`.
...
1. User types goal.
2. AikaOS generates plan (structured).
3. User approves.
...
2. AikaOS injects context into session.
3. Execute prompt.
...
1. User opens AikaOS cloud control page and authenticates.
...
5. User clicks `Open in AikaOS` or copies manual credentials.
6. In AikaOS app, user uses `Add a worker` -> `Connect remote` and starts working.
