<!-- AikaOS - AI Agent Automation Platform by AikaLabs -->
...
- **OpenCode Router (WhatsApp bot)**: a lightweight WhatsApp bridge for a running OpenCode server. Install with:
  - `curl -fsSL https://raw.githubusercontent.com/aikapenelope/openOS/main/packages/opencode-router/install.sh | bash`
  - run `opencode-router setup`, then `opencode-router whatsapp login`, then `opencode-router start`
  - full setup: [packages/opencode-router/README.md](./packages/opencode-router/README.md)
- **AikaOS Orchestrator (CLI host)**: run OpenCode + AikaOS server without the desktop UI.
  - install: `npm install -g aikaos-orchestrator`
  - run: `aikaos start --workspace /path/to/workspace --approval auto`
...
Interested in using AikaOS in your organization? We'd love to hear from you — reach out to AikaLabs to chat about your use case.
