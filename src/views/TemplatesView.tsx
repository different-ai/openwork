      ...
      >
        <div class="space-y-6">
          <Show when={props.workspaceTemplates.length}>
            <div class="space-y-3">
              <div class="text-xs font-semibold text-zinc-500 uppercase tracking-wider">{t("templates.groups.workspace")}</div>
                ...
                {(template) => (
                  <div class="bg-zinc-900/30 border border-zinc-800/50 rounded-2xl p-5 flex items-start justify-between gap-4">
                    <div class="min-w-0">
                      ...
                      <div class="mt-2 text-xs text-zinc-600 font-mono">{formatRelativeTime(template.createdAt)}</div>
                    </div>
                    <div class="shrink-0 flex gap-2">
                      <Button variant="secondary" onClick={() => props.runTemplate(template)} disabled={props.busy}>
                        <Play size={16} />
                        {t("templates.actions.run")}
                      </Button>
          ...
          <Show when={props.globalTemplates.length}>
            <div class="space-y-3">
              <div class="text-xs font-semibold text-zinc-500 uppercase tracking-wider">{t("templates.groups.global")}</div>
