...
export default function SessionView(props: SessionViewProps) {
  ...
  const handleRecentsKeyNav = (e: KeyboardEvent) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      e.preventDefault();
      const newIndex = Math.max(currentIndex === -1 ? 0 : currentIndex - 1, 0);
      const session = sessions[newIndex];
      if (session) {
        void props.selectSession(session.id);
        props.setView("session");
  ...
  const unlinkedArtifacts = createMemo(() =>
    props.artifacts.filter((artifact) => !artifact.messageId)
  );
        ...
        <div class="flex-1 flex overflow-hidden">
          <aside class="hidden lg:flex w-72 border-r border-gray-6 bg-gray-1 flex-col">
            ...
            <div class="flex-1 overflow-y-auto px-4 py-4">
              <div class="text-xs text-gray-10 uppercase tracking-wide mb-3">
                Recents
              </div>
              <div
                ref={recentsListRef}
              ...
              <For each={props.messages}>
                {(msg) => {
                  ...
                  const messageArtifacts = () =>
                    artifactsByMessage().get(messageId()) ?? [];

                  return (
                    <Show when={renderableParts().length > 0}>
                      <div
                        class={`flex ${isUser() ? "justify-end" : "justify-start"}`.trim()}
                      >
                        <div
                          class={`w-full ${
                            isUser()
                        ...
                        >
                                ...
                                <Show when={group.kind === "steps"}>
                                  <div
                                    class={
                                      isUser()
                                        ? "mt-2"
                                        : "mt-3 border-t border-gray-6/60 pt-3"
                                    }
                                  >
                                    <button
                                      ...
                                      onClick={() =>
                                        toggleSteps((group as any).id)
                                      }
                                    >
                                      <span>
                                        {props.expandedStepIds.has(
                                          (group as any).id
                                        )
                                          ? "Hide steps"
                                          : "View steps"}
                                    ...
                                    <Show
                                      when={props.expandedStepIds.has(
                                        (group as any).id
                                      )}
                                    >
                                      <div
                                      ...
                                      >
                                        <For
                                          each={(group as any).parts as Part[]}
                                        >
                                          {(part) => {
                                            const summary =
                                              props.summarizeStep(part);
                                            return (
                          ...
                          <Show when={messageArtifacts().length}>
                            <div
                              class={`mt-4 space-y-2 ${isUser() ? "text-gray-12" : ""}`.trim()}
                            >
                              <div class="text-[11px] uppercase tracking-wide text-gray-9">
                                Artifacts
                              </div>
                          ...
                          {(done) => (
                            <div
                              class={`h-6 w-6 rounded-full border flex items-center justify-center ${
                                  ...
                                  ? "border-green-6 text-green-11"
                                  : "border-gray-7 text-gray-8"
                              }`}
                            >
                              <Show when={done}>
                                <Check size={14} />
                ...
                <Show when={props.expandedSidebarSections.artifacts}>
                  <div class="px-4 pb-4 pt-1 space-y-3">
                    <Show
                      ...
                      fallback={
                        <div class="text-xs text-gray-7">No artifacts yet.</div>
                      }
                    >
              ...
              <div class="rounded-2xl border border-gray-6 bg-gray-1/60">
                ...
                <Show when={props.expandedSidebarSections.context}>
                  <div class="px-4 pb-4 pt-1 space-y-4">
                    <Show
                      when={
                        props.activePlugins.length || props.activePluginStatus
                      }
                    >
                      <div>
                    ...
                    <div>
                      ...
                      <div class="mt-2 space-y-2">
                        <Show
                          ...
                          fallback={
                            <div class="text-xs text-gray-7">None yet.</div>
                          }
                        >
          ...
          <div class="max-w-2xl mx-auto">
            <div class="bg-gray-2 border border-gray-6 rounded-2xl overflow-hidden focus-within:ring-1 focus-within:ring-gray-7 transition-all shadow-2xl relative group/input">
              ...
              >
                <Zap
                  size={10}
                  class="text-gray-7 group-hover:text-amber-11 transition-colors"
                />
                <span>
              ...
              <div class="p-2 pt-6 pb-3 px-4">
                <Show when={props.showTryNotionPrompt}>
                  ...
                  >
                    ...
                    <span class="text-xs text-green-12 font-medium">
                      Insert prompt
                    </span>
                  </button>
                </Show>
