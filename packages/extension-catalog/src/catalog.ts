...
/**
 * Canonical built-in descriptor order. Executable bindings remain owned by the
 * realm that hosts each contribution.
 */
...
export type BuiltInOpenWorkExtensionManifest = (typeof BUILT_IN_OPENWORK_EXTENSION_MANIFESTS)[number]

export function builtInOpenWorkExtensionManifestById(id: string) {
  return BUILT_IN_OPENWORK_EXTENSION_MANIFESTS.find((manifest) => manifest.id === id) ?? null
}
