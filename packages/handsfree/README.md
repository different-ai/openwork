...
Native macOS computer-use runtime for Micx.
Packaged Micx builds wrap this runtime in a bundled `Computer Use.app`
...
```bash
pnpm --filter @micx/handsfree check:native
```

Run it as an MCP-compatible adapter:

```bash
pnpm --filter @micx/handsfree exec micx-handsfree-computer-use mcp
```
