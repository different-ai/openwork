...
/**
 * SSRF guard for remote MCP Connection URLs.
 *
 ...
 * network can deliberately use createRealmSafeFetch instead.
 */
...
function isBlockedRealmIpv4(address: string): boolean {
  const octets = parseIpv4(address)
...
function isBlockedRealmIpv6(address: string): boolean {
  const words = parseIpv6Words(address)
  ...
  if (words.slice(0, 5).every((word) => word === 0) && sixth === 0xffff) {
    return isBlockedRealmIpv4(embeddedIpv4(words, 6))
  }
  if (words.slice(0, 4).every((word) => word === 0) && fifth === 0xffff && sixth === 0) {
    return isBlockedRealmIpv4(embeddedIpv4(words, 6))
  }
  ...
  return first === 0x2002 && isBlockedRealmIpv4(embeddedIpv4(words, 1))
}

/**
 * Private-network policy for user-approved local/self-hosted connections.
 * RFC1918, loopback, CGNAT, and IPv6 ULA are permitted; link-local metadata,
 * multicast, documentation, benchmark, unspecified, and reserved ranges stay
 * blocked. DNS is resolved on every request to prevent rebinding around it.
 */
export async function assertRealmUrl(rawUrl: string): Promise<void> {
  const url = parseHttpUrl(rawUrl)
  const hostname = url.hostname.replace(/^\[|\]$/g, "")
  if (isIP(hostname)) {
    const blocked = isIP(hostname) === 4 ? isBlockedRealmIpv4(hostname) : isBlockedRealmIpv6(hostname)
    if (blocked) throw new PrivateUrlError(rawUrl, "the address is reserved or link-local")
    return
  }
  ...
  if (addresses.length === 0) throw new PrivateUrlError(rawUrl, "the hostname does not resolve")
  for (const { address } of addresses) {
    const version = isIP(address)
    const blocked = version === 4
      ? isBlockedRealmIpv4(address)
      : version === 6
        ? isBlockedRealmIpv6(address)
        : true
    if (blocked) throw new PrivateUrlError(rawUrl, `the hostname resolves to a reserved or link-local address (${address})`)
  }
}
...
export const MCP_OUTBOUND_RESPONSE_LIMIT_BYTES = 8 * 1024 * 1024
...
function boundedResponse(response: Response, limit = MCP_OUTBOUND_RESPONSE_LIMIT_BYTES): Response {
  ...
  const body = response.body.pipeThrough(new TransformStream<Uint8Array, Uint8Array>({
    transform(chunk, controller) {
      ...
      if (bytes > limit) {
        controller.error(new Error("MCP outbound response exceeded the byte limit."))
        return
      }
  ...
  return async (input, init) => {
    ...
    for (let redirectCount = 0; ; redirectCount += 1) {
      ...
      if (!REDIRECT_STATUSES.has(response.status) || !location) {
        return boundedResponse(await normalizeResponseRealm(response))
      }
...
export function createRealmSafeFetch(fetchImpl: FetchLike = fetch): FetchLike {
  return createRedirectSafeFetch(fetchImpl, assertRealmUrl)
}
