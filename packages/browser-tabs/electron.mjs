// Electron-only browser host. Shells supply their paths and navigation adapters;
// the platform-neutral registry and the native lifecycle have a single owner.
...
export function createBrowserPanel({
  ...
  popupDisposition = /** @returns {string | Promise<string>} */ (_popup) => "external",
  ...
  if (typeof partition !== "string" || !partition.startsWith("persist:")) {
    throw new Error("Browser host requires a persistent session partition.");
  }
  if (typeof preloadPath !== "string" || !path.isAbsolute(preloadPath)) {
    throw new Error("Browser host requires an absolute browser preload path.");
  }
  if (typeof openExternal !== "function" || typeof runDetachedTask !== "function") {
    throw new Error("Browser host requires openExternal and runDetachedTask callbacks.");
  }
  ...
  // the shell selects the tab or shows its owner. The registry stays unchanged.
  ...
  function browserTarget(tab) {
    ...
    return {
      ...
      canGoBack: panelTab.canGoBack,
      canGoForward: panelTab.canGoForward,
      visible: surfacingFor(tab.tabId) === "foreground",
    };
  }
  ...
  function listBrowsers(ownerId) {
    return registry.tabsFor(requireOwnerId(ownerId))
      .map(({ tabId }) => browserTabs.get(tabId))
      .filter((tab) => tab && !tab.view.webContents.isDestroyed())
      .map(browserTarget);
  }
  ...
  function ownedBrowserTab({ ownerId, tabId, targetId }) {
    ...
    const tab = [...browserTabs.values()].find((candidate) => (
      registry.ownerOf(candidate.tabId) === ownerId &&
      (!tabId || candidate.tabId === tabId) &&
  ...
  async function ensureMenuOverlayView() {
    ...
    const view = new WebContentsView({
      webPreferences: {
        ...
        preload: menuOverlay.preloadPath,
      },
    });
    ...
    resetMenuOverlayReady({ resolvePending: true });
    await menuOverlay.loadRenderer(view);
    return view;
  }
  ...
  async function showLinkContextMenu({ url, point, sessionId }) {
    ...
    const browsers = await menuOverlay.listInstalledBrowsers();
    if (showSerial !== menuOverlayShowSerial) return;
    const items = [
      { id: "open-builtin", label: menuOverlay.openBuiltinLabel },
  ...
  function createBrowserTab(url = "about:blank", { select = true, initializeBlank = true, ownerSessionId = null, inBackground = false } = {}) {
    ...
    const tab = { tabId, view, favicon: null, background: false, inBackground, targetId: null };
    ...
    view.webContents.setWindowOpenHandler(({ url: targetUrl }) => {
      const popupOwnerId = registry.ownerOf(tabId);
      runDetachedTask("open browser popup", async () => {
        const disposition = await popupDisposition({ url: targetUrl, ownerId: popupOwnerId, tabId });
        if (disposition === "embedded") {
          await checkPolicy?.({ url: targetUrl, external: false });
          await openBrowserUrlForAutomation(targetUrl, "builtin", { ownerSessionId: popupOwnerId, inBackground: tab.inBackground });
          return;
        }
        if (disposition !== "external") return;
        try { await checkPolicy?.({ url: targetUrl, external: true }); }
        catch { createBrowserTab(targetUrl, { ownerSessionId: popupOwnerId, initializeBlank: false }); return; }
    ...
    view.webContents.on("did-start-navigation", (_event, targetUrl, isInPlace, isMainFrame) => {
      ...
      // The shell recognizes its own scheme and consumes the handoff.
  ...
  function selectBrowserTab(tabId) {
    ...
    tab.inBackground = false;
    registry.select(tabId);
    if (surfacingFor(tabId) === "foreground") {
  ...
  function closeBrowserTab(tabId = onScreenTabId()) {
  ...
  function attachBrowserView(bounds, { preloadDefault = false, ensureTab = false, sessionId } = {}) {
    ...
    if (sessionId !== undefined) {
      ...
      const next = registry.setVisibleSession(sessionId);
      const released = releaseBackgroundOpens(next);
      if (next !== previous || released) applySurfacing();
    }
  ...
  function registerIpc(ipcMain) {
    ...
    ipcMain.handle("openwork:browser:navigate", (_event, url) => navigate(url));
    ipcMain.handle("openwork:browser:back", () => back());
    ipcMain.handle("openwork:browser:forward", () => forward());
    ipcMain.handle("openwork:browser:reload", () => reload());
    ipcMain.handle("openwork:browser:bounds", (_event, bounds) => setBounds(bounds));
    ipcMain.handle("openwork:browser:state", () => state());
