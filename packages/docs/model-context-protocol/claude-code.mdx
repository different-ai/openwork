---
title: "Micx MCP for Claude Code"
sidebarTitle: "Claude Code"
description: "Claude Code MCP setup for Micx Connect, with OAuth access to your org's Gmail, Calendar, Drive, shared MCP connections, skills, and Cloud resources."
---

The Micx MCP server ([Micx Connect](/cloud/run-in-the-cloud/cloud-mcp))
enables Claude Code to use the Micx capabilities your organization has made
...
1. Add Micx Connect as a remote HTTP MCP server:

   ```sh
   claude mcp add --transport http micx https://api.openworklabs.com/mcp/agent
   ```
...
3. Select `micx`, then follow the browser sign-in and choose your
   organization.

## Switch organizations

Run `/mcp` inside Claude Code, select `micx`, then clear or
