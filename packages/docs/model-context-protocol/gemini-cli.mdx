---
title: "Micx MCP for Gemini CLI"
sidebarTitle: "Gemini CLI"
description: "Gemini CLI MCP setup for Micx Connect, with OAuth access to your org's Gmail, Calendar, Drive, shared connections, skills, and Cloud resources."
---

The Micx MCP server ([Micx Connect](/cloud/run-in-the-cloud/cloud-mcp))
enables Gemini CLI to use the Micx capabilities your organization has made
...
1. Add Micx Connect as a remote HTTP MCP server:
   ...
   ```sh
   gemini mcp add --transport http micx https://api.openworklabs.com/mcp/agent
   ```
...
3. Authenticate Micx inside Gemini:
...
Remove the stored MCP auth token for `micx`, then run `/mcp auth micx`
