---
title: "Micx MCP for Cursor"
sidebarTitle: "Cursor"
description: "Cursor MCP setup for Micx Connect on Cursor Desktop, Cursor Web, and Cursor Agents."
---

The Micx MCP server ([Micx Connect](/cloud/run-in-the-cloud/cloud-mcp))
enables Cursor to use the Micx capabilities your
...
Linear, Slack, Stripe, Exa, and Context7, shared skills, and Micx Cloud
...
Add Micx as a remote MCP server in Cursor's MCP settings, paste the server
URL, and start OAuth from Cursor's connection prompt. Sign in to Micx and
pick the organization Cursor should use.

Cursor Desktop's OAuth callback is the private-use URI
`cursor://anysphere.cursor-mcp/oauth/callback`. Micx accepts exactly this
...
Other private-use callback schemes remain rejected: Micx otherwise accepts
...
Micx, and pick the organization Cursor should use.

Cursor Web and Cursor Agents use HTTPS OAuth callbacks, which Micx
accepts.

## Switch organizations

Remove the stored MCP auth token for `micx`, then connect again and choose
...
steps match Micx Connect's OAuth profile: HTTPS or HTTP loopback callbacks
