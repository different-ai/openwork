...
- `registry.npmjs.org` is used when packaged desktop builds start `npx -y micx-ui-mcp`. If it is blocked, the desktop can still open, but UI-control MCP is unavailable.
...
| --- | --- | --- | --- |
| `app.openworklabs.com` | Required for Micx Cloud | Hosted Cloud web origin. The desktop uses one base URL and derives Den API/MCP calls through `/api/den/...`. | Cloud sign-in, org settings, marketplace, and remote workspace flows cannot load. |
| `api.openworklabs.com` | Required for hosted install and Micx Connect | Hosted public API and Micx Connect MCP endpoint used by install and external-client flows. | Hosted install checks and public Micx Connect fail. |
...
| `registry.npmjs.org` | Required in practice unless npm is mirrored | `npx` package resolution for `micx-ui-mcp` and development-only helper commands. | Micx UI-control MCP is unavailable. |
...
| `openworklabs.com`, `micx.dev`, `discord.gg`, `ollama.com`, `opencode.ai`, `linear.app`, `mail.google.com` | Optional or link-only | Docs, feedback, community, provider help, local model links, sample links, and generated Gmail links. | Links do not open; core app behavior is unaffected. |

`code.claude.com` and `schemas.agentskills.io` appear in source or schema metadata. They are findable in the manifest, but Micx does not fetch them for normal runtime behavior.
...
| Host | Requirement | Why it is reached | What breaks when blocked |
| --- | --- | --- | --- |
| `ghcr.io` | Required unless mirrored | Micx Helm chart and container images. | Kubernetes cannot pull the chart or `micx-den-api`, `micx-den-web`, and optional `micx-inference` images. |
