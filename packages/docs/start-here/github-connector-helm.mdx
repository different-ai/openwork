...
description: "Create a GitHub App and configure the Micx EE Helm chart for repository connectors."
---

The GitHub connector lets a self-hosted Micx deployment install a GitHub App, list repositories the installation can read, discover plugin or marketplace manifests, import selected repositories, and keep imported content fresh from GitHub webhooks.
...
The GitHub App needs these Micx URLs:

| GitHub App field | Value |
| --- | --- |
| Homepage URL | `https://micx.example.com` |
...
## What Micx does with it

When an admin clicks **Connect GitHub** in Micx:
...
```text
https://micx.example.com/dashboard/integrations
```

Choose GitHub, install the GitHub App on a test account or organization, select a repository, and start discovery. A working setup should return to Micx, show repositories visible to the installation, and allow discovery on a selected repository.
