...
description: "Connect a custom or local MCP server that is not available through Micx Connect"
---

Use this guide only when you need a custom or local MCP server that is not available through Micx Connect.
...
Connecting a custom app with Micx takes four clicks.
...
back to Micx. If a custom MCP needs behavior Micx does not support,
