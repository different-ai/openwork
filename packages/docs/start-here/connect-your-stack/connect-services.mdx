...
description: "Sign in to Micx and connect the accounts your agent can use"
---

Use `Settings` > `Micx Connect` for services your organization makes available,
...
Connect requires signing in to an Micx account and joining an organization
that has Connect enabled. This is separate from signing in to an LLM provider
such as ChatGPT.

1. Open the Micx desktop app.
2. Click `Sign in`, or open `Settings` > `Micx Connect` and choose `Sign in`.
3. Complete sign-in in your browser and return to Micx.
4. Choose the Micx organization you want to use if prompted.
...
send you back to `Settings` > `Micx Connect` with the action to take instead of
...
- **Connections** is the organization-admin dashboard in Micx Cloud for
  publishing a service, choosing whose account it uses, and granting member or
  team access.
- **Micx Connect MCP** is the hosted endpoint that lets external MCP
  clients use capabilities from an Micx organization.
