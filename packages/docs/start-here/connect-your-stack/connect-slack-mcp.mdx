...
4. Add this redirect URL: `http://127.0.0.1:19876/mcp/oauth/callback`.
5. Add the `User Token Scopes` for the Slack MCP tools your team wants OpenWork to use.
6. Save the app and install or approve it for the workspace.
7. Go to `Basic Information` > `App Credentials` and copy the `Client ID` and `Client Secret`.

<Frame>
  ![Slack app settings Agents tab with the MCP toggle enabled](/images/slack-app-agents-mcp-toggle.png)
</Frame>
...
If sign-in fails or Slack rejects the MCP connection even though the client ID and secret are correct, check that MCP is enabled for the Slack app. Open the app in [Slack API apps](https://api.slack.com/apps), go to the `Agents` tab, and turn on the `MCP` toggle. Then retry sign-in from OpenWork.
