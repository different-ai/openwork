...
For Gateway, mirror matching images from the capability-version-1 release rather than older stable images with new chart values. Follow [Gateway deployment configuration](/self-host/gateway) and the [safe upgrade procedure](/self-host/gateway-upgrade), including schema migration before runtime rollout. Gateway is GA for admins in every organization when configured, with no organization flag or platform-admin grant. Deployment `GATEWAY_ENABLED` and `deploymentCapabilities.aiGateway` checks still apply, as do admin permissions, credential controls, and network requirements.
