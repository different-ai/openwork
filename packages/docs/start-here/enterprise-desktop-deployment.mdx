...
- **Allowed desktop versions.** An organization setting delivered by your Den server after sign-in; the app's updater only installs versions on the list — no MDM scripting. Manage it in the organization settings.
- **Desktop policies.** Feature restrictions for the desktop app are temporarily not enforced while they are redesigned; they stay saved in Den.
