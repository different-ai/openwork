...
- **Desktop policies.** Version pinning (allowed desktop versions) is delivered by your Den server after sign-in and enforced by the app's updater automatically — no MDM scripting. Desktop feature policies are temporarily not enforced by the desktop while they are redesigned; they stay saved in Den. Manage both in the organization dashboard.
