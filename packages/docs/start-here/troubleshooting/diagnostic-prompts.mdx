...
description: "Paste-able prompts that let your agent debug Micx issues on its own."
---

When something in Micx looks wrong, the fastest debugger is usually the agent itself: it can read its own configuration, probe servers, and verify fixes in-band. This page collects paste-able diagnostic prompts. Copy the whole block for your situation into a chat and let the agent work through it.
...
Your agent's `micx-cloud` MCP entry is a snapshot: it stores a server URL and token from the moment it was connected. If it was created against a local development server or an old deployment — including via a leftover `desktop-bootstrap.json` file that overrides server URLs beneath the Settings UI — the agent keeps querying that old world forever, while the dashboard shows the current one.

**Paste this to your agent**

````text
Debug and fix my Micx Cloud MCP connection. It may be pointed at a stale or local server instead of my real Micx Cloud server. Work through ALL phases, show your findings after each, and ask before anything destructive. Never print bearer tokens or Authorization headers — redact them.
...
1. Print the `micx-cloud` MCP entry's `url` (redact headers) from every config you can find:
...
2. Inspect these files if they exist (they can silently override server URLs beneath the Settings UI), but report only `baseUrl`, `apiBaseUrl`, `requireSignin`, and `writtenAt`. Never print `handoff`, `claimLinks`, grants, tokens, or other credential-bearing fields:
   - `~/.config/micx/desktop-bootstrap.json`
   - `~/Library/Application Support/com.differentai.micx/desktop-bootstrap.json`
...
- micx-cloud url is localhost/127.0.0.1, OR search returned only gmail-drafts, OR execute returned unknown_capability → you are bound to a STALE server, not the expected cloud.
...
10. Tell me to do these steps in Micx, then wait:
    - Restart the Micx app.
    ...
    - Click Show hidden, then verify Micx Cloud Control under Your apps.
...
- The `micx-cloud` entry's `url` points at your real server and ends in `/mcp/agent` (either a direct API host or the web app's `/api/den` proxy).
...
- Settings shows an `micx-cloud` MCP entry, but the agent calls `micx_docs_search` for requests involving connected services.
...
- A connected service such as ServiceNow appears ready in Micx Cloud, but the task does not expose `micx-cloud_search_capabilities` or `micx-cloud_execute_capability`.

**What this diagnostic checks**

The prompt below runs Micx's bundled OpenCode executable against its generated runtime config and checks whether a fresh OpenCode process can connect to `micx-cloud`. It does not attach to the OpenCode server already running inside Micx, so it combines that result with the live status shown in Micx and a forced tool call from a new task.

**Paste this to your agent**

````text
Diagnose whether Micx Cloud MCP tools are available in this workspace. Gather evidence only: do not modify configuration, authenticate, log out, remove anything, restart services, or print configuration-file contents.

## Phase 1 — Test the task's actual tools

1. Attempt to call `micx-cloud_search_capabilities` with `{"query":"ServiceNow incident","type":"mcp","limit":10}`. Do not use documentation or explain alternatives. Record whether the runtime accepts the tool call and its exact non-secret result or rejection.
2. Attempt to call `micx_extension_list_actions`. Record whether the runtime accepts the call. Do not infer tool availability from an answer to "list your tools"; models can omit or invent tools.

## Phase 2 — Find Micx's exact OpenCode runtime

3. Detect whether this machine is Windows, macOS, or Linux.
4. Find the exact OpenCode executable used by the Micx desktop app. Prefer the bundled executable in Micx's `resources/sidecars` directory; do not assume an unrelated `opencode` on `PATH` is the same binary. If multiple Micx builds are running, use the process associated with this task when that can be established safely; otherwise report the ambiguity.
5. Find Micx's generated `runtime-opencode-config.json` beside the active `server.json`. Respect discovered `MICX_SERVER_CONFIG`, `MICX_RUNTIME_DB`, or XDG overrides. Typical locations are:
   - Windows: `%APPDATA%\micx\runtime-opencode-config.json`
   - macOS/Linux: `${XDG_CONFIG_HOME:-$HOME/.config}/micx/runtime-opencode-config.json`
...
Important: `opencode mcp list` starts a fresh process. Do not claim that it attaches to or tests the OpenCode server already running inside Micx. Describe it only as a fresh-process probe using the same binary and generated config.

## Phase 4 — Classify the result

- Fresh probe omits `micx-cloud`: the generated config is missing, the wrong runtime config was selected, or this is a secondary workspace whose MCPs are registered dynamically.
...
2. Click **Show hidden**, then find **Micx Cloud Control** under **Your apps**. This row shows live OpenCode MCP status; the catalog card only proves that the entry is configured.
...
| Fresh `opencode mcp list` | Micx status / forced call | Likely boundary |
| --- | --- | --- |
