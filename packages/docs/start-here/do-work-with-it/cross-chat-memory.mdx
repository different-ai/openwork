...
3. **Archive once it is idle.** Archiving a session with `working: false` completes immediately and preserves its context. When an agent archives a session that is still working (including itself, which is working while it makes the call), OpenWork asks you to confirm **Stop and archive** and immediately tells the agent `awaiting_user_confirmation` with the target and the requester, instead of letting the request time out. That dialog names the target session and, under **Requested by**, the agent session that asked, or "this session itself". Nothing is stopped or archived until you choose. Restoring an archived session never restarts stopped work.
