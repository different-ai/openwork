---
title: "Self-host"
description: "Run Micx on your own server or infrastructure."
---

Most users just use the [desktop app](https://openworklabs.com/download). Self-hosting is for teams that want Micx to run on their own server, VPC, or private cloud and then connect the desktop app to it remotely.

Want hosted workspaces, team marketplaces, or shared providers without running infra? Use [Micx Cloud](/cloud/get-started) instead. Rolling this out across a company? [Book a call](https://calendar.app.google/86QpCENvhfEzDFLu5) about [Enterprise](/cloud/enterprise).

## What you deploy

Micx is split into normal services:

- **Micx app**: the desktop or web client people use.
...
- **Inference service**: optional Micx Models proxy and metering service.

If you only need one private remote workspace, deploy the worker runtime. If you want Micx Cloud-style accounts, teams, and organization management, deploy Den web, Den controller, and optionally the inference service.
...
The chart pulls the matching GHCR images for `micx-den-api`, `micx-den-web`, and optional `micx-inference` unless you mirror them into an internal registry.
...
- Hosted Micx uses PlanetScale for Den control-plane state. PlanetScale production clusters provide one primary and at least two replicas across three availability zones, managed failover, and automated backups.
...
- Keep the local Docker MySQL URL (`mysql://root:password@127.0.0.1:3306/micx_den`) for development only; it is not a production security posture.
- Set a unique `DEN_DB_ENCRYPTION_KEY` of at least 32 characters. Micx uses it to encrypt selected sensitive database columns, but it does not replace infrastructure-level encryption at rest.
...
If you do not deploy Den, worker access is token-based with `MICX_TOKEN` and `MICX_HOST_TOKEN`.

## Minimal working stack

### Micx Cloud and the desktop app

Micx cloud-style self-hosting is composed of three main pieces:
...
Micx does not require many third-party services for the core runtime. These are optional or deployment-specific:
...
Skip the server. [**Micx Cloud**](/cloud/get-started) runs hosted workers for your team.
...
If you're rolling Micx out across a company of 150\+ employees, a single self-hosted server VM might be harder to manage.
