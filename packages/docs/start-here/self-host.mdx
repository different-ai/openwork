...
| --- | --- | --- |
| Docker Compose | Evaluating on one machine | [Evaluate with Docker Compose](/self-host/evaluate-with-docker-compose) |
| Helm on Amazon EKS | Production on AWS with Amazon RDS for MySQL | [Deploy on AWS](/self-host/deploy-to-your-cloud/aws) |
| Helm on Azure AKS | Production on Azure with Azure Database for MySQL | [Deploy on Azure](/self-host/deploy-to-your-cloud/azure) |
| Helm on Google GKE | Production on Google Cloud with Cloud SQL for MySQL | [Deploy on Google Cloud](/self-host/deploy-to-your-cloud/google-cloud) |
| Another container platform (for example, Amazon ECS/Fargate) | Teams that do not run Kubernetes | Use the published images and the service boundaries below |

The tested production path is the published Helm chart on managed Kubernetes. The repository does not ship Terraform modules or ECS task definitions. If you run on ECS/Fargate or another container platform, deploy the published `openwork-den-web`, `openwork-den-api`, and optional `openwork-inference` images from GHCR with your own infrastructure code, and mirror what the chart does: Den web on port `3005`, Den API on port `8788`, database migrations before Den API receives traffic, a MySQL 8-compatible database, and HTTPS in front of both hostnames. See [Deploy to your cloud](/self-host/deploy-to-your-cloud/overview) for the common architecture and the [Docker packaging](https://github.com/different-ai/openwork/tree/dev/packaging/docker) for image details.
...
<AccordionGroup>
  <Accordion title="Can I self-host OpenWork for free?">
    The desktop app is free and open source, and the Free plan includes self-hosting the full platform for up to 5 users. Shared keys and skills need the Team plan, and SSO, SCIM, and desktop policies need Enterprise; the same prices apply to Cloud and self-hosted deployments. See [Pricing and licensing](/start-here/pricing-and-licensing).
  </Accordion>
  <Accordion title="Is there a Terraform module or an ECS deployment?">
    Not in the repository today. The supported production path is Helm on EKS, AKS, or GKE. You can run the published images on ECS/Fargate with your own infrastructure code by following the service boundaries in [Deployment options](#deployment-options).
  </Accordion>
  <Accordion title="Which database does self-hosted OpenWork need?">
    A MySQL 8-compatible database, such as Amazon RDS for MySQL, Azure Database for MySQL, Cloud SQL for MySQL, or PlanetScale. Postgres is not required.
  </Accordion>
  <Accordion title="Can a self-hosted deployment use my own models or AI gateway?">
    Yes. Members can connect providers in the desktop app, admins can share providers with the team, and you can enable [OpenWork Gateway](/self-host/gateway) for routing and metering. See [Add a custom LLM or AI gateway](/start-here/connect-your-stack/add-a-custom-llm).
  </Accordion>
  <Accordion title="Can I share skills and MCP servers from a self-hosted deployment?">
    Yes. Plugins, connectors, and access rules live in the Den control plane. See [Share skills and MCP servers with your team](/start-here/do-work-with-it/share-your-setup).
  </Accordion>
</AccordionGroup>
