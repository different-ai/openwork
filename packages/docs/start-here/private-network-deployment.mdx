...
description: "Run Micx when laptops have internet and VPN access while Den stays inside a private network."
---

Most enterprise Micx installs are not fully air-gapped. The common shape is:
...
`DEN_API_PUBLIC_URL` still must be configured correctly, but it does not need to be a second desktop-facing origin. In the single-origin shape, set it to the proxied API base, for example `https://micx.example.internal/api/den`. Den uses that value for install-link claims, OAuth callbacks, webhooks, MCP resource/audience values, and links.
...
External MCP clients such as Cursor, Claude Code, VS Code, ChatGPT, and OpenCode are separate from the desktop. They must reach the MCP endpoint you configure for them, usually `<DEN_API_PUBLIC_URL>/mcp/agent`; with the single-origin proxy example above, that is `https://micx.example.internal/api/den/mcp/agent`. See [Micx Connect MCP](/cloud/run-in-the-cloud/cloud-mcp) for the external-client flow.
...
- The Micx chart and images at deploy or image-pull time, unless you mirror the Helm chart and enabled images into an internal registry.
...
**Why:** `DEN_DIAGNOSTICS_ORIGIN` defaults to the public Micx diagnostic service.
...
| Cloud MCP works in a real chat | In a chat, the agent can search available Micx capabilities and execute one that the user is allowed to run. | `Settings → Connect` for provider sign-in or admin setup. Use `Settings → Debug` to interpret diagnostics, especially a skipped catalog probe. |
