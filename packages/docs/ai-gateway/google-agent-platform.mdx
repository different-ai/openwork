...
In OpenWork, open **Models → Gateway** and create the appropriate provider as described in the next step. Its credential-set editor displays the OAuth callback URL. Copy that **exact** value into Google's **Authorized redirect URIs** before starting member sign-in.
...
As an organization owner/admin, open **Models → Gateway** and add a provider:

1. Choose `google-vertex` for Gemini or `google-vertex-anthropic` for Claude.
2. Give the provider a recognizable name.
3. Enter the **Project ID** and supported **location** recorded earlier.
4. Select your initial test model and save the provider.
...
1. Open the provider's credential sets and create one, such as **Google member sign-in**.
2. Choose **Each Member Signs In**.
3. Enter the Google **Web OAuth client ID** and **client secret**.
4. Confirm that the displayed callback matches Google's registration exactly.
5. Save.

The client secret is write-only. Leave it unchanged for routine edits; replacing the OAuth client configuration requires affected members to reconnect.

### Assign model access

Create or select a **model group** containing the models members should use. Then create an **access rule** linking:

- That model group.
- Your member-sign-in credential set.
- The intended OpenWork members, teams, or organization audience.

Creating a provider or credential set does **not** automatically grant inference access, even to its administrator. Assign yourself access if you will test it.

## 8. Sign in as a member

In Den, open **Model connections** at `/dashboard/model-connections`. In Desktop, you can also use the assigned provider's **Connect** action in AI provider settings.

1. Find the intended provider and credential set and select **Connect with Google**.
2. Open the browser sign-in link.
3. On the OpenWork connection page, sign in to Den in that browser with the **same OpenWork account that started Connect**. If sign-in opens another tab, finish there and return to the connection tab.
...
| Browser account mismatch | Sign in to Den in that browser with the OpenWork account that initiated Connect, then retry. Desktop sign-in does not automatically sign the browser in. |
