...
description: "Provision Micx members and teams from Microsoft Entra ID"
---

Use Microsoft Entra ID provisioning to create, update, and deactivate Micx
members automatically. Entra security groups can also become Micx teams
...
- An Micx organization with Enterprise SSO and SCIM.
- An Micx owner, or a member who can manage security configuration.
- A working Micx SAML connection. Complete and test
...
sure at least one Micx owner can sign in through SAML before relying on
Entra as the identity source.

## 1. Get the Micx connection values

In Micx, select the organization you want to connect, then open
**Settings → SCIM**.
...
3. Copy the **SCIM bearer token** immediately. Micx shows the full token
   only after creating or rotating it.
4. To synchronize Entra groups as Micx teams, select **Enable team sync**
...
| --- | --- |
| Tenant URL | Micx **Settings → SCIM → SCIM base URL**. It normally ends in `/api/auth/scim/v2`. |
...
2. Select the same enterprise application used for Micx SAML SSO.
3. Open **Provisioning**.
...
6. Paste the Micx **SCIM base URL** into **Tenant URL**.
7. Paste the Micx **SCIM bearer token** into **Secret Token**.
...
principal names use an `onmicrosoft.com` domain but users sign in to Micx
...
create a second Micx member for the same person.
...
not enough; assign it to the Micx enterprise application as well.
...
3. Confirm that import, scope evaluation, matching, and the final action all
   succeed.
4. In Micx, open **Members** and confirm that the member appears with the
   expected email.
...
6. In Micx, open **Members → Teams** and confirm that the group appears as a
...
sends a group-create request to Micx. Add members and select them explicitly
...
5. Refresh Micx **Members** and **Teams** to verify the final state.
...
1. Remove a test user from the Entra group and confirm that Micx removes the
   user from the SCIM-managed team.
2. Disable or unassign a test user and confirm the expected Micx
   deprovisioning behavior.
3. Add another assigned group and confirm that Micx creates a second
   SCIM-managed team.

Change SCIM-managed teams in Entra, not in Micx. Micx keeps manually
...
| --- | --- | --- |
| **Test Connection** fails | Incorrect Tenant URL, expired or rotated token, or copied whitespace | Copy the base URL and a current token again from Micx, then retest |
...
| On-demand group provisioning reports an internal server error without an Micx request | The group is empty or no members were selected for the on-demand run | Add members, select up to five under **Selected users**, and retry |
| A group provisions but no Micx team appears | Micx team synchronization is disabled | Open **Settings → SCIM** and enable **Create teams from SCIM groups** |
| Entra creates a duplicate Micx member | SAML and SCIM use different email identifiers | Align the SAML NameID and the SCIM `userName` mapping |
| Entra reports success but Micx still shows the old state | The UI or incremental cycle has not refreshed | Refresh Micx, check Entra **Provisioning logs**, and review the SCIM health status in Micx |
