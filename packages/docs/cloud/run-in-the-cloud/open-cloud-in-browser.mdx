...
<Warning>
**Hosted access.** OpenWork Web is available to every organization on an OpenWork-hosted deployment with Web billing configured. It costs **$50 per joined organization member per month**; pending invitations are not billed until they are accepted.
</Warning>
...
Self-hosted single-organization deployments do not show OpenWork Web. If you use an OpenWork-hosted organization and do not see it in the sidebar, email [team@openworklabs.com](mailto:team@openworklabs.com) so the deployment configuration can be checked.
