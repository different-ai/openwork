...
Workflow, receipt, output schema, or Automation is required. Authored App tools
are available when the organization has member-facing MCP connections enabled
(the default); where they are turned off, `save_artifact_view` keeps its legacy
creation path. It accepts:

- `title`, `reactSource`, and a non-empty `textFallback`.
- Optional `description`, `cssSource`, and `pluginId`.

Omit `pluginId` to create a new private Plugin named after the App. If you
already have a Plugin with that name, creation returns `duplicate_plugin` with
its ID: pass that `pluginId` or choose another title. Supply an existing Plugin
only when authorized to edit it. The result is a text summary and structured
...
`input` is the launch input, `result` is the launch `CallToolResult` (read
`result?.structuredContent`; it is `undefined` until the host delivers it), and
`hostContext` is supplied by the host. Authored source cannot import modules, fetch, use
...
external provider invocation. Apps add no per-App tool grant. Check
`app.getHostCapabilities()?.serverTools` before calling tools. Hosts may require
a user click before calling a tool that is not read-only: OpenWork does, and
`execute_capability` is not read-only. So only read-only tools such as
`search_capabilities` may run on mount; start data loads and actions that use
`execute_capability` from an explicit button and show `tool_requires_approval`
as a blocked state. Nested provider visibility is not independently
