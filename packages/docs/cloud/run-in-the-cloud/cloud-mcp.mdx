...
Ask the agent for an app, dashboard, or interactive view and it calls `create_app` with React source, a text fallback, and the tools the App needs. No Workflow is required. Each App is saved in a Plugin and served as its own standard MCP server at `/mcp/agent/connections/<appId>` on your OpenWork API, which exposes only:

- `open_app`, bound to the App's current immutable `ui://` revision;
- the tools the App declared, each a clear name bound to one exact capability from `search_capabilities`: a saved Workflow, a connection tool, or an OpenWork action;
- the App's revision resources.

Declared tools run as the person using the App, with that person's own access and connections; sharing an App never shares credentials. A tool is marked read-only only when OpenWork can verify it: OpenWork read actions, connection tools that their provider marks read-only, and saved Workflows declared with `mode: "live"`, which run read-only with server-supplied dates. The OpenWork app runs other tools only after a click inside the App.
...
`read_app` returns source to editors, and `update_app` publishes a new revision; omit `tools` to keep the current ones. `save_artifact_view` only edits existing Workflow-bound views. These App tools are available when your organization has member-facing MCP connections turned on, which is the default.
