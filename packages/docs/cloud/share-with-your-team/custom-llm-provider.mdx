...
An LLM gateway gives you one OpenAI-compatible endpoint that fans out to many model providers — start with [LiteLLM](https://infron.ai/docs/frameworks-and-integrations/litellm) if you want to run the routing yourself. [Infron](https://infron.ai) skips that setup: one API key unlocks every model in its [marketplace](https://infron.ai/models) with automatic provider fallbacks and a single invoice, so adding a new LLM to OpenWork is just another entry under `models`.
