...
An LLM gateway gives you one OpenAI-compatible endpoint that fans out to many model providers. [LiteLLM](https://infron.ai/docs/frameworks-and-integrations/litellm) is the canonical open-source take — a routing library/proxy you run yourself. [Infron](https://infron.ai) is the hosted version of that idea (think OpenRouter): one key, one endpoint, unified billing, and built-in provider fallbacks across its [model marketplace](https://infron.ai/models). Gateways are a good fit for custom providers because the same JSON works for every model they expose — just add more entries under `models`.

Grab a key from the [API Keys dashboard](https://infron.ai/dashboard/apiKeys). New to Infron? Sign up at [infron.ai/login](https://infron.ai/login) and follow the [quickstart](https://infron.ai/docs/overview/quickstart/text).
