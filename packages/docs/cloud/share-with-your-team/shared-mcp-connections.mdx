...
description: "Share Notion, Linear, Stripe, Exa, or any MCP server with your org. Each person signs in, or everyone uses one org account."
---
...
3. Pick a preset (Notion, Linear, Stripe, Sentry, Exa, Context7) or enter any
   MCP server URL.
4. Choose how people sign in: `Each person signs in` (Individual accounts) or `One account for everyone` (One org account).
...
with `mcp:read`. See [MCP token scopes](/cloud/run-in-the-cloud/cloud-mcp#reference).
