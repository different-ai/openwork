...
description: "Share Notion, Linear, Stripe, Exa, or any MCP server in Micx Cloud with Individual accounts by default, or One org account when everyone should act as the same account"
---
...
## Publish the connection in Micx Cloud

1. Open `MCP Connections` in the Micx Cloud dashboard.
...
<Frame>
  ![MCP Connections admin screen in Micx Cloud](/images/cloud-mcp-connections-admin.png)
</Frame>
...
For the hosted Micx Cloud instance, the dashboard is
...
`https://api.micx.example.com`, register:

```text
https://api.micx.example.com/v1/mcp-connections/oauth/callback
```
...
3. In Micx Cloud, add the Slack connection and paste the app's client ID
   and client secret.
...
1. Sign in to Micx, then open `Settings` > `Connect`.
...
4. The browser shows `Connected` — return to Micx.
5. The connection updates by itself, no reload needed, and moves to
   `Ready to use`.

Your sign-in is stored in Micx Cloud, not on this machine — connect once
...
The agent searches the live capabilities available through Micx Connect,
...
services managed through Micx Cloud.
