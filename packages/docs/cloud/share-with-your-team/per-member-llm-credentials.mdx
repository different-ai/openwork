...
When the member has no active binding, the connect route returns HTTP `409`:
...
`credentialState` is one of `missing`, `blocked`, `stale`, or `error`. Clients should stop provider import or sync and prompt for the corresponding repair instead of falling back to a shared key.
...
4. Verify the member's connect request returns `needs_key` with `credentialState: "blocked"`.
