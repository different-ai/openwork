...
A member's OpenWork Web instance receives the same resolution: Den writes that member's own active binding into the instance, never a teammate's, and leaves the provider out while the member has no active binding or is not granted the provider. A rotated binding is written to the instance the next time it is used.
