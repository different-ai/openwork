...
<Update label="April 20th" tags={["🚀 New Features", "🐛 Bug Fixes"]}>
  ...
  - Enforced Den desktop restrictions in the app and added org controls for supported desktop versions so admins can limit blocked models, providers, and releases.
  - Auto-sync cloud providers into the active org workspace and key them by cloud id so updates, removals, and disconnects reconcile more reliably.
  - Improved Den admin flows with explicit names for LLM providers and steadier active-org handling for legacy org-scoped clients.
...
<Update label="April 20th" tags={["🚀 New Features"]}>
  ...
  - Added Den org settings for allowed email domains and desktop restrictions so owners can control who accepts invites and which desktop capabilities signed-in members can use.
  - Improved Den org administration with active-org-aware desktop sign-in, inline organization renaming, clearer invite delivery failures, and compatibility for older org-scoped clients.
  - Added a microsandbox preview toggle in Settings > Advanced, plus Spanish and Catalan localization, an in-app docs shortcut, and an agents-ready docs and landing refresh.
