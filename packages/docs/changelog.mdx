...
<Update label="April 20th" tags={["🚀 New Features"]}>
  ...
  ## [v0.11.210](https://github.com/different-ai/openwork/compare/v0.11.207...v0.11.210): Microsandbox preview lands and the site becomes agents-ready

  - Added a microsandbox preview toggle in Settings > Advanced so Create Sandbox can use the microsandbox flow instead of the default Docker image flow for faster isolated workspaces.
  - Made openworklabs.com agents-ready with `llms.txt`, markdown twins, an API catalog, WebMCP hints, and crawlable metadata so agents can read, crawl, train on OpenWork content, and help users find the right docs, downloads, pricing, and cloud flows.
  - Expanded onboarding coverage with Spanish and Catalan translations, a reworked docs home organized by user intent, and a direct docs shortcut inside the app.
