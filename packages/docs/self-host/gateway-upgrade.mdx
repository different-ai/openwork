...
5. With traffic still closed, deploy the matching Den API, Den Web, and Gateway images. Keep `migrations.enabled: false` for this externally migrated rollout. Review sparse values and required Secret references as described in [the deployment configuration guide](/self-host/gateway).
6. Verify liveness/readiness, then the authenticated `GET /v1/org` top-level `deploymentCapabilities` contract across API replicas. For a configured Gateway deployment, expect `{ "version": 1, "aiGateway": true }`. Confirm fresh organization-admin access; all organizations have Gateway access without a platform-admin grant. Readiness and the capability response are not schema proofs.
...
The organization `gatewayDashboard` flag is retired: Gateway is GA for every organization's admins when the deployment is configured. The deprecated admin API input is a no-op and the response field is constant `true` for older clients, not a real feature flag. Stored values, including `false`, are ignored; retirement needs no database migration or backfill. This does not waive the release schema cutover above. There is no per-organization dashboard toggle.
...
OpenWork Models remains available to eligible hosted admins alongside Gateway, whether or not deployment Gateway management is enabled; existing single-org custom-provider behavior remains. Retiring the organization opt-in and changing deployment management configuration neither cancel nor create a subscription and do not revoke Models or Gateway keys.
