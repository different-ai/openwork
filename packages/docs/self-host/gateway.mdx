...
For eligible hosted organization admins, OpenWork Models navigation and command-palette entries are hidden, and its direct page redirects to Gateway, **only when both deployment support and the organization dashboard grant are effective**. A grant alone on an unsupported deployment leaves OpenWork Models available. Loading temporarily defers those decisions. Existing single-org behavior still routes Models to custom providers. This is a UI transition, not subscription cancellation, a billing change, Models-key revocation, or deletion of provider data. See [disable and recovery choices](/self-host/gateway-upgrade#disable-without-a-schema-downgrade) before turning anything off.
