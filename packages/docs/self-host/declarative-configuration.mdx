...
Review membership: the sample Platform team starts empty. Marketplace access and plugin attachments use
separate endpoints.
...
1. Read `GET /v1/llm-providers/{id}/connect` using the authorized caller. Unlike
   ordinary provider reads, its `llmProvider` response can contain the stored
   credential. Keep this payload in memory; never print it or save it to Job logs.
2. Use that returned endpoint and credential in the **bare** body of
   `POST /v1/llm-providers/test-connection`: `{api, apiKey, modelIds}`. Select the
   configured model IDs to verify (at most eight per request). Do not wrap this
   probe in `{llmProvider: ...}` or merely reuse the manifest's input secret:
   the gate must exercise what Den actually stored and supplies to this caller.
...
| --- | --- |
| Legacy providers | Use `source:"custom"`, explicit `credentialMode`, `customConfig.models`, and top-level `allMembers/memberIds/teamIds`. Write-only `apiKey`/`apiKeys` are preserved where supported; models and audience assignments are replaced, not merged (creator access remains). Omitted `credentialMode` becomes `shared`; omitted `allMembers/memberIds/teamIds` become `false`/empty lists. |
| MCP connections | Omitted `access` becomes org-wide: **confirmed live on 0.18.46**, including replacement of team-only grants. Omitted `credentialMode` becomes `shared` and `exposeDirectly` becomes `false`. Omitted `oauthClient.clientSecret` is preserved when connection identity and client ID are unchanged; reads do not export it. |
| Teams | Bare `name`/`memberIds` write; omitted `memberIds` becomes an empty list. |
| Desktop policies | Use `policyName`, `policy`, and top-level `roles/memberIds/teamIds`. Omitted `policy.access` and `policy.execution` are retained; `priority`/`isEnabled` reset to zero/true and assignment lists become empty. |
