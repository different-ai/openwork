...
export function parsePreviewOutputs(input: unknown): PreviewOutputs {
  ...
  for (const [key, entry] of Object.entries(input)) {
    ...
    if (output.group === "Services" && !/^https:\/\/(?:ow|den|api|engine|gateway)-[a-f0-9]{32}\.(?:style\.dev|preview\.openwork\.software)\/__openwork_launch\?token=[A-Za-z0-9_-]+$/.test(output.value)) {
