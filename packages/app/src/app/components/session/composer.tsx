...
const createMentionSpan = (part: Extract<ComposerPart, { type: "agent" | "file" }>) => {
  ...
  span.className =
    "inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium bg-slate-100 text-slate-700 border border-slate-200";
  return span;
};
...
export default function Composer(props: ComposerProps) {
  ...
  const createPasteSpan = (part: Extract<ComposerPart, { type: "paste" }>) => {
    ...
    span.className =
      "inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold bg-slate-100 text-slate-500 border border-slate-200 cursor-pointer hover:bg-slate-200 hover:text-slate-700";
    return span;
  };
  ...
  return (
    <div class="sticky bottom-0 z-20 bg-gradient-to-t from-white via-white to-transparent px-8 pt-12 pb-6" style={{ contain: "layout style" }}>
      <div class="max-w-[800px] mx-auto">
        <div
          class={`bg-white border border-slate-200/80 rounded-xl overflow-visible transition-all relative group/input ${mentionOpen() || slashOpen() ? "rounded-t-none border-t-transparent" : "shadow-[0_8px_30px_rgba(0,0,0,0.08)]"
            }`}
          onDrop={handleDrop}
        ...
        >
          <Show when={mentionOpen()}>
            <div class="absolute bottom-full left-[-1px] right-[-1px] z-30">
              <div class="rounded-t-xl border border-slate-200 border-b-0 bg-white shadow-xl overflow-hidden">
                <div class="p-2 bg-white max-h-64 overflow-y-auto" onMouseDown={(event: MouseEvent) => event.preventDefault()}>
                  <Show
                    when={mentionVisible().length}
                    fallback={<div class="px-3 py-2 text-xs text-slate-500">No matches found.</div>}
                  >
                    <For each={mentionVisible()}>
                      {(option: MentionOption) => {
                        ...
                        return (
                          <button
                            type="button"
                            class={`w-full flex items-center gap-2 rounded-xl px-3 py-2 text-left transition-colors ${active() ? "bg-slate-100 text-slate-800" : "text-slate-700 hover:bg-slate-50"
                              }`}
                          ...
                          >
                            <Show
                              ...
                              fallback={
                                <>
                                  <FileIcon size={14} class="text-slate-400" />
                                  <div class="flex items-center min-w-0 text-xs">
                                    {(() => {
                                      ...
                                      return (
                                        <>
                                          <span class="text-slate-400 truncate">{dir}</span>
                                          <Show when={name}>
                            ...
                            >
                              <AtSign size={14} class="text-slate-400" />
                              <span class="text-xs font-semibold text-slate-700">@{option.label}</span>
                            </Show>
                          </button>
                        );
                      }}
          ...
          <Show when={slashOpen()}>
            <div class="absolute bottom-full left-[-1px] right-[-1px] z-30">
              <div class="rounded-t-xl border border-slate-200 border-b-0 bg-white overflow-hidden">
                <div class="p-2 bg-white max-h-64 overflow-y-auto" onMouseDown={(event: MouseEvent) => event.preventDefault()}>
                  <Show
                    ...
                    <For each={slashFiltered()}>
                      {(cmd: SlashCommandOption, index) => {
                        ...
                        return (
                          <button
                            type="button"
                            class={`w-full flex items-center justify-between gap-4 rounded-xl px-3 py-2 text-left transition-colors ${active() ? "bg-slate-100 text-slate-800" : "text-slate-700 hover:bg-slate-50"
                              }`}
                            onMouseDown={(event: MouseEvent) => {
                              event.preventDefault();
                              handleSlashSelect(cmd);
                          ...
                          >
                            <div class="flex items-center gap-2 min-w-0">
                              <Terminal size={14} class="text-slate-400 shrink-0" />
                              <span class="text-xs font-semibold text-slate-700 whitespace-nowrap">/{cmd.name}</span>
                              <Show when={cmd.description}>
                                <span class="text-xs text-slate-500 truncate">{cmd.description}</span>
                              </Show>
                            </div>
                            <Show when={cmd.source && cmd.source !== "command"}>
                              <span class="text-[10px] uppercase tracking-wider text-slate-500 shrink-0">
          ...
          <div class="p-3">
              ...
              <div class="mb-3 flex flex-wrap gap-2">
                <For each={attachments()}>
                  {(attachment: ComposerAttachment) => (
                    <div class="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-500">
                      <Show
                        when={attachment.kind === "image"}
                        fallback={<FileIcon size={14} class="text-slate-400" />}
                      >
                        <div class="h-10 w-10 rounded-xl bg-white overflow-hidden border border-slate-200">
                          <img src={attachment.dataUrl} alt={attachment.name} class="h-full w-full object-cover" />
                        </div>
                      </Show>
                      <div class="max-w-[160px]">
                        <div class="truncate text-slate-700">{attachment.name}</div>
                      ...
                      <button
                        type="button"
                        class="ml-1 rounded-full p-1 text-slate-500 hover:text-slate-700 hover:bg-slate-200"
                        onClick={() => {
            ...
            <div class="relative min-h-[120px]">
              <Show when={props.toast}>
                <div class="absolute bottom-full right-0 mb-2 z-30 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600 shadow-lg backdrop-blur-md">
                  <div class="flex items-center gap-3">
                    <span>{props.toast}</span>
                    <Show when={showInboxUploadAction() && props.onUploadInboxFiles}>
                      <button
                        type="button"
                        class="shrink-0 rounded-md border border-slate-200 bg-slate-50 px-2 py-1 text-[10px] text-slate-700 hover:bg-slate-100"
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="relative">
                    <Show when={!hasDraftContent()}>
                      <div class="absolute left-0 top-0 text-slate-400 text-[15px] leading-relaxed pointer-events-none">
                        Ask OpenWork...
                      </div>
                    </Show>
                    <div
                      ...
                      class="bg-transparent border-none p-0 pb-8 pr-4 text-slate-800 focus:ring-0 text-[15px] leading-relaxed resize-none min-h-[24px] max-h-40 overflow-y-auto outline-none relative z-10"
                    />
                    ...
                    <div class="mt-3 flex items-center justify-between px-2 pb-2">
                      <div class="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          class={`p-1.5 hover:bg-slate-100 rounded-md text-slate-500 transition-colors ${attachmentsDisabled() ? "cursor-not-allowed" : ""
                            }`}
                          onClick={() => {
                            if (attachmentsDisabled()) return;
                            fileInputRef?.click();
                        ...
                        <div class="relative" ref={(el) => props.setAgentPickerRef(el)}>
                          <button
                            type="button"
                            class="flex items-center gap-1.5 px-2 py-1 hover:bg-slate-100 rounded-md text-[13px] font-medium text-slate-600 hover:text-slate-800"
                          ...
                          <Show when={props.agentPickerOpen}>
                            <div class="absolute left-0 bottom-full mb-2 w-64 rounded-xl border border-slate-200 bg-white shadow-xl backdrop-blur-md overflow-hidden z-40">
                              <div class="px-3 pt-2 pb-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500 border-b border-slate-200">
                                Agent
                              </div>
                              ...
                              <div class="p-2 space-y-1 max-h-64 overflow-y-auto" onMouseDown={(event: MouseEvent) => event.preventDefault()}>
                                <Show
                                  ...
                                  fallback={
                                    <div class="px-3 py-2 text-xs text-slate-500">Loading agents...</div>
                                  }
                                >
                                  <Show when={!props.agentPickerError}>
                                    ...
                                    >
                                      ...
                                      <Show when={!props.selectedAgent}>
                                        <Check size={14} class="text-slate-500" />
                                      </Show>
                                    </button>
                        ...
                        <div class="relative" ref={(el) => (variantPickerRef = el)}>
                          ...
                          >
                            <span>Thinking</span>
                            <span class="font-mono text-slate-700">{props.modelVariantLabel}</span>
                            <ChevronDown size={14} />
                          </button>
                          <Show when={variantMenuOpen()}>
                            <div class="absolute left-0 bottom-full mb-2 w-48 rounded-xl border border-slate-200 bg-white shadow-xl backdrop-blur-md overflow-hidden z-40">
                              <div class="px-3 pt-2 pb-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500 border-b border-slate-200">
                                Thinking effort
                              </div>
                                  ...
                                  {(option) => (
                                    ...
                                    >
                                      ...
                                      <Show when={activeVariant() === option.value}>
                                        <span class="text-[10px] uppercase tracking-wider text-slate-500">Active</span>
                                      </Show>
                                    </button>
                                  )}
                                </For>
                          ...
                          fallback={
                            <button
                              ...
                              class={`p-1.5 rounded-full transition-colors ${!hasDraftContent()
                                ...
                                : "bg-[#1B29FF] text-white hover:bg-blue-700"
                                }`}
                              title="Send"
                            >
                              <ArrowUp size={18} />
