...
const humanizeCron = (cron: string) => {
  ...
  return t("scheduled.days_at_time").replace("{days}", list).replace("{time}", timeLabel);
};
...
const taskSummary = (job: ScheduledJob) => {
  ...
  if (run?.command) {
    ...
    return { label: t("scheduled.label_command"), value: `${run.command}${args}`, mono: true };
  }
  const prompt = run?.prompt ?? job.prompt;
  if (prompt) {
    return { label: t("scheduled.label_prompt"), value: prompt, mono: false };
  }
  return { label: t("scheduled.label_task"), value: t("scheduled.no_prompt_command"), mono: false };
};

const statusLabel = (status?: string | null) => {
  if (!status) return t("scheduled.status_not_run");
  if (status === "running") return t("scheduled.status_running");
  if (status === "success") return t("scheduled.status_success");
  if (status === "failed") return t("scheduled.status_failed");
  return status;
};
...
const automationTemplates = [
  {
    icon: Calendar,
    nameKey: "scheduled.template_daily_planning",
    descKey: "scheduled.template_daily_planning_desc",
  ...
  {
    icon: BookOpen,
    nameKey: "scheduled.template_inbox_zero",
    descKey: "scheduled.template_inbox_zero_desc",
  ...
  {
    icon: MessageSquare,
    nameKey: "scheduled.template_meeting_prep",
    descKey: "scheduled.template_meeting_prep_desc",
  ...
  {
    icon: TrendingUp,
    nameKey: "scheduled.template_weekly_wins",
    descKey: "scheduled.template_weekly_wins_desc",
  ...
  {
    icon: Trophy,
    nameKey: "scheduled.template_learning_digest",
    descKey: "scheduled.template_learning_digest_desc",
  ...
  {
    icon: Brain,
    nameKey: "scheduled.template_habit_checkin",
    descKey: "scheduled.template_habit_checkin_desc",
...
const dayOptions = [
  { id: "mo", labelKey: "scheduled.day_mon", cron: "1" },
  { id: "tu", labelKey: "scheduled.day_tue", cron: "2" },
  { id: "we", labelKey: "scheduled.day_wed", cron: "3" },
  { id: "th", labelKey: "scheduled.day_thu", cron: "4" },
  { id: "fr", labelKey: "scheduled.day_fri", cron: "5" },
  { id: "sa", labelKey: "scheduled.day_sat", cron: "6" },
  { id: "su", labelKey: "scheduled.day_sun", cron: "0" },
];
...
const buildAutomationPrompt = (options: {
  ...
  const nameSegment = name ? t("scheduled.named").replace("{name}", name) : "";
  const workdirSegment = workdir ? t("scheduled.run_from").replace("{workdir}", workdir) : "";
  return t("scheduled.prompt_construct")
    ...
    <div class="flex flex-col gap-4 rounded-2xl border border-gray-4 bg-gray-1 p-5 shadow-sm">
        ...
        <div class="rounded-xl border border-gray-4 bg-gray-2/60 px-3 py-3 space-y-2">
          ...
          <div class="space-y-2 text-xs text-gray-9">
            ...
            <Show when={props.job.source}>
              <div class="text-[11px] text-gray-8">{t("scheduled.source").replace("{source}", props.job.source)}</div>
            </Show>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-4 text-xs text-gray-9">
        ...
        <Show when={props.job.run?.agent}>
          <div>{t("scheduled.agent").replace("{agent}", props.job.run?.agent ?? "")}</div>
        </Show>
        <Show when={props.job.run?.model}>
          <div>{t("scheduled.model").replace("{model}", props.job.run?.model ?? "")}</div>
        </Show>
      </div>
    </div>
  );
          ...
          <div class="bg-white border border-gray-6 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
            <div class="p-6 space-y-4">
              <div class="flex items-start justify-between gap-4">
                <div>
                  <h3 class="text-lg font-semibold text-gray-12">{t("scheduled.delete_confirm_title")}</h3>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-[2px] p-4">
          <div class="w-full max-w-2xl rounded-3xl bg-white shadow-2xl overflow-hidden border border-gray-6">
            <div class="p-8 space-y-6">
              <div class="flex items-start justify-between gap-4">
                <div>
                  <h2 class="text-xl font-semibold text-gray-12">{t("scheduled.create_modal_title")}</h2>
