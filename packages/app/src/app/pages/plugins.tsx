    ...
    <section class="space-y-6">
      <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-4">
        <div class="flex items-start justify-between gap-4">
          <div class="space-y-1">
            <div class="text-sm font-medium text-gray-12">{tr("plugins.title")}</div>
            <div class="text-xs text-gray-10">{tr("plugins.description")}</div>
          </div>
        ...
        <div class="flex flex-col gap-1 text-xs text-gray-10">
          <div>{tr("plugins.config_label")}</div>
          <div class="text-gray-7 font-mono truncate">{props.pluginConfigPath ?? tr("plugins.config_not_loaded")}</div>
        ...
        <div class="space-y-3">
          <div class="text-xs font-medium text-gray-11 uppercase tracking-wider">{tr("plugins.suggested_label")}</div>
                    ...
                    <div class="flex items-start justify-between gap-4">
                      ...
                      <div class="flex items-center gap-2">
                        <Show when={isGuided()}>
                          ...
                          >
                            {isGuideOpen() ? tr("plugins.hide_setup") : tr("plugins.setup")}
                          </Button>
                        </Show>
                        <Button
                        ...
                        >
                          {isInstalled() ? tr("plugins.added") : tr("plugins.add")}
                        </Button>
                      </div>
                    </div>
                            ...
                            <div class="space-y-1">
                              ...
                              <Show when={step.url}>
                                <div class="text-xs text-gray-10">
                                  {tr("common.open")}: <span class="font-mono text-gray-11">{step.url}</span>
                                </div>
                              </Show>
                              <Show when={step.path}>
                                <div class="text-xs text-gray-10">
                                  {tr("common.path")}: <span class="font-mono text-gray-11">{step.path}</span>
                                </div>
                              </Show>
                            </div>
                          )}
            ...
            <For each={props.pluginList}>
              {(pluginName) => (
                <div class="flex items-center justify-between rounded-xl border border-gray-6/60 bg-gray-1/40 px-4 py-2.5">
                  ...
                  <div class="text-[10px] uppercase tracking-wide text-gray-10">{tr("plugins.enabled")}</div>
                </div>
              )}
            </For>
          </div>
