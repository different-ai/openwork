...
- Extend AikaOS server + app identity APIs to carry `access` and `pairingCode` fields.
...
- Telegram adapter startup with a dummy token fails expectedly with `401 Unauthorized`; pairing logic and code issuance still validate via AikaOS API and UI.
