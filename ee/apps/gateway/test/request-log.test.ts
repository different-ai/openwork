...
test("terminal finalization retains request model provenance with known and unknown costs", async () => {
  for (const requestedModelSource of ["request", "header"] satisfies RequestLogStartInput["requestedModelSource"][]) {
    for (const costUsd of [null, 0.00001]) {
      const rows: InferenceRequestLogRow[] = []
      const receipts: Parameters<NonNullable<InferenceReporter["terminal"]>>[0][] = []
      const recorder = createRequestLogRecorder({
        insertRequestLog: async (row) => { rows.push(row) },
        updateRequestLog: async (row) => { rows[0] = row; return true },
        reporter: { request() {}, handledError() {}, terminal(receipt) { receipts.push(receipt) } },
        pricing: { getModelPrice: () => null },
      })
      recorder.start({ ...startInput, requestedModelSource, modelAlias: "trusted-alias" })
      assert.equal(await recorder.whenStarted?.(), true)
      assert.equal(rows[0]?.metadata?.requested_model_source, requestedModelSource)
      recorder.setUsage({ usageSource: "stream", upstreamRequestId: "response_123", costUsd,
        generation: { generationOutcome: "refused", providerTerminalReason: "refusal" } })
      await recorder.finish({ status: 200, outcome: "ok", upstreamRequestId: "request_123" })
      assert.deepEqual(rows[0]?.metadata, {
        requested_model_source: requestedModelSource,
        cost_source: costUsd === null ? "unknown" : "upstream",
        upstream_response_id: "response_123",
      })
      assert.equal(rows[0]?.cost_micro_usd, costUsd === null ? null : 10)
      assert.equal(rows[0]?.generation_outcome, "refused")
      assert.equal(rows[0]?.provider_terminal_reason, "refusal")
      assert.equal(rows[0]?.outcome, "ok")
      assert.equal(receipts.length, 1)
      assert.equal(receipts[0]?.modelAlias, "trusted-alias")
      assert.equal(receipts[0]?.upstreamRequestId, "request_123")
      assert.equal(receipts[0]?.upstreamResponseId, "response_123")
    }
  }
})
