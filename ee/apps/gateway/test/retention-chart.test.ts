...
test("Helm retention is opt-in, daily, bounded, internal-only and uses the gateway's same Secret", { skip: !helm }, () => {
  ...
  assert.equal((enabled.stdout.match(/name: "accounting-secret"/g) ?? []).length, 4)
  assert.equal((enabled.stdout.match(/key: "INFERENCE_ADMIN_TOKEN"/g) ?? []).length, 4)
  assert.equal((enabled.stdout.match(/name: GATEWAY_ADMIN_TOKEN/g) ?? []).length, 2)
  assert.equal((enabled.stdout.match(/name: INFERENCE_ADMIN_TOKEN/g) ?? []).length, 2)
...
test("Helm rejects a missing token reference or competing inline token", { skip: !helm }, () => {
  ...
  assert.match(missing.stderr, /gateway\.retention\.adminTokenSecret \(or inference\.retention\.adminTokenSecret\) is required/)
  for (const key of ["GATEWAY_ADMIN_TOKEN", "INFERENCE_ADMIN_TOKEN"]) {
    ...
    assert.match(conflicting.stderr, /Use gateway\.retention\.adminTokenSecret/)
  }
})
