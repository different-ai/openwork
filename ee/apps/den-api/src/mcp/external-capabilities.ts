...
type ExternalSearchCacheCounters = {
  cacheHits: number
  staleServed: number
  misses: number
  writesFailed: number
...
function addToolMatches(input: {
  connection: ExternalMcpConnectionRow
  limit: number
  matches: ExternalCapabilityMatch[]
  queryTokens: string[]
  tools: readonly { description?: string; name: string; title?: string }[]
}) {
  for (const tool of input.tools) {
    const summary = tool.description ?? tool.title ?? tool.name
    const nameTokens = tokenize(`${input.connection.name} ${tool.name}`)
    const summaryTokens = tokenize(summary)
    const score = scoreText(nameTokens, summaryTokens, input.queryTokens)
    if (score <= 0) continue
    mergeBoundedExternalCapabilityMatches(input.matches, [{
      name: buildExternalCapabilityName(input.connection.id, tool.name),
      method: "MCP",
      path: input.connection.url,
      score,
      summary: `[${input.connection.name}] ${summary}`,
      pathParams: [],
      queryParams: [],
      hasBody: true,
    }], input.limit)
...
function shouldInvalidateManifest(error: unknown): boolean {
  if (error instanceof ExternalMcpDiagnosticError) {
    return error.diagnostic.jsonRpcCode === -32601
      || /unknown tool|tool not found|method not found/i.test(error.message)
  }
  if (typeof error === "object" && error !== null && "code" in error) {
    const code = error.code
    if (code === -32601) return true
    if (code === -32602) {
      return /unknown tool|tool not found/i.test(error instanceof Error ? error.message : String(error))
    }
  }
  return false
}
...
async function probeExternalMcpConnection(input: {
  ...
  connection: ExternalMcpConnectionRow
  counters: ExternalSearchCacheCounters
  ...
  if (env.mcpManifestCacheEnabled) {
    ...
    if (classification.state === "fresh") {
      ...
      addToolMatches({
        ...
        tools: classification.row.tools,
      })
      return matches
    }
  ...
  try {
    ...
    if (env.mcpManifestCacheEnabled) {
      await saveManifestListing({
        ...
        input.counters.writesFailed += 1
        console.warn("external_mcp_manifest_write_failed", {
    ...
    if (env.mcpManifestCacheEnabled) {
      await saveManifestFailure({
        ...
        console.warn("external_mcp_manifest_write_failed", {
          connectionId: connection.id,
          reason: manifestError instanceof Error ? manifestError.message : String(manifestError),
        })
      })
    }
...
export async function searchExternalCapabilities(input: {
  ...
  const accounts = await getConnectedAccounts({
    organizationId: normalizeDenTypeId("organization", input.organizationId),
    orgMembershipId: member.orgMembershipId,
    providerIds: selectedConnections.flatMap((connection) =>
      connection.credentialMode === "per_member" ? [connection.id] : []),
  })
  const manifestPairs = selectedConnections
    .filter((connection) => connection.credentialMode === "shared"
      ? hasSharedCredential(connection)
      : Boolean(accounts.get(connection.id)?.accessToken))
    .map((connection) => ({
      connection,
  ...
  const manifests = env.mcpManifestCacheEnabled
    ? await getManifests({ pairs: manifestPairs }).catch((error) => {
      console.warn("external_mcp_manifest_read_failed", {
        reason: error instanceof Error ? error.message : String(error),
      })
      return new Map<string, ExternalMcpToolManifestRow>()
    })
    : new Map<string, ExternalMcpToolManifestRow>()
  const counters: ExternalSearchCacheCounters = {
    cacheHits: 0,
    staleServed: 0,
    misses: 0,
    writesFailed: 0,
  }
  const matches = await collectBoundedExternalMcpSearchMatches({
    connections: selectedConnections,
    deadline,
    limit,
    probe: (connection, sharedDeadline) => probeExternalMcpConnection({
      connection,
      account: connection.credentialMode === "per_member" ? accounts.get(connection.id) ?? null : null,
      counters,
      manifest: manifests.get(manifestMapKey(
        connection.id,
        connection.credentialMode === "per_member" ? member.orgMembershipId : "shared",
      )),
      member,
      queryTokens,
  ...
  console.info("external_mcp_capability_search_cache", {
...
export async function executeExternalCapability(input: {
  ...
  try {
    ...
    if (shouldInvalidateManifest(error)) {
      const principal = manifestPrincipalFor(connection, member)
      void markManifestsStale({ connectionId: connection.id, principal }).catch((manifestError) => {
        console.warn("external_mcp_manifest_invalidation_failed", {
          connectionId: connection.id,
          reason: manifestError instanceof Error ? manifestError.message : String(manifestError),
        })
      })
