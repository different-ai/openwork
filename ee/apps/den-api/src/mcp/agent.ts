...
export const AGENT_MCP_INSTRUCTIONS = [
  ...
  "For connection actions and saved Workflows, call search_capabilities with 2-4 keyword variants before concluding something is unavailable. Builder tools such as create_skill and create_app are not capability search results; call them directly. Use execute_capability only with exact names returned by search_capabilities. A successful search_capabilities call proves this connection is authorized: Never tell the user to reconnect OpenWork Cloud because a downstream connector failed.",
  ...
  app.all("/mcp/agent", tokenRoute, async (c) => {
    ...
    if (method === "server/discover" || method === "initialize" || method === "resources/list" || method === "resources/read") {
      if (memberIdentity) {
        ...
        registerConnectMcpServerIndex({
          ...
          apps: externalMcpConnectionsEnabled ? await loadMcpApps() : [],
    ...
    server.registerTool(
      SEARCH_CAPABILITIES_TOOL_NAME,
      {
        ...
        description: [
          ...
          "Builder tools such as create_skill and create_app are outside this search; call them directly. Use create_app for new apps, dashboards, and interactive views; save_artifact_view only edits legacy views.",
      ...
      async ({ query, limit, type, intent }) => {
        ...
        const apps = includeApps ? await loadMcpApps() : []
        const appIds = new Set(apps.map((app) => app.appId))
        // Built Apps replace their generic Plugin match; ask for enough ordinary matches to fill the limit.
        const result = await searchCapabilityRegistry(capabilityContext, { query, limit: Math.min(20, boundedLimit + apps.length), type })
    ...
    server.registerTool(
      ...
      async ({ name, schemaDigest, path, query, body }) => {
        ...
        const plugin = externalMcpConnectionsEnabled ? parseMarketplaceCapabilityName(name) : null
