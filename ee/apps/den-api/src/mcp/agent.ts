...
export function capabilitySearchToolResult<T extends CapabilityMatch>(matches: T[], coverageHint?: string, connectorCatalog?: ConnectorCatalog | null, connectionIntent = false) {
  const hint = [
    ...(matches.length === 0 ? [connectorCatalog ? "These are setup suggestions, not connected tools. The user can complete setup in the native connection card. Do not create the connection yourself or ask for credentials in chat; the card checks setup permission and handles sign-in." : "No matches. Try broader or different keywords."] : []),
