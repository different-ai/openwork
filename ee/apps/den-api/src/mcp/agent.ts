...
export const AGENT_MCP_INSTRUCTIONS = [
  ...
  "When asked to create an app, create or reuse a private Workflow with an output schema, call save_artifact_view to build a draft, and call its preview tool. The user tries and saves it from the app preview. Keep improvements as drafts until the user explicitly saves them. Generated apps display, filter, and explore Workflow results; do not promise approval submission or other writes, which are not supported by generated views.",
