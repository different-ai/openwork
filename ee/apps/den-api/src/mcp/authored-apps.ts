...
export const APP_AUTHORING_GUIDANCE = [
  ...
  "Provide a default-exported React component receiving { app, input, result, hostContext }. React is injected: use React.useState and other React APIs without imports. Do not use fetch, browser/host globals, dynamic code, external resources, URL-bearing elements, or native forms (<form> is blocked). Use labeled inputs and explicit type=button controls.",
  "Use app.callServerTool({ name, arguments }) with exact discovered tool names and arguments for data and actions. search_capabilities, execute_capability, and saved Workflows are ordinary tools, not a parent App runtime. Existing tool authorization, MCP scopes, and normal host consent govern writes; there is no per-App tool grant or blanket read-only restriction. Do not claim nested provider visibility has been checked.",
