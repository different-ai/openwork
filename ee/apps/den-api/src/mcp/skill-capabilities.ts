...
export async function listSkillCapabilityCandidates(input: {
  organizationId: string
  member: McpMemberIdentity | null
...
export async function searchSkillCapabilities(input: {
  ...
  return rankCapabilities(input.query, await listSkillCapabilityCandidates({
    organizationId: input.organizationId,
    member: input.member,
  }), { limit: input.limit })
}
