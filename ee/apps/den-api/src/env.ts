...
const EnvSchema = z.object({
  ...
  DEN_MCP_MANIFEST_REFRESH_INTERVAL_MS: z.string().optional(),
...
export const env = {
  ...
  mcpManifestMaxBytes: Number(parsed.DEN_MCP_MANIFEST_MAX_BYTES ?? "262144"),
  mcpManifestRefreshIntervalMs: Number(parsed.DEN_MCP_MANIFEST_REFRESH_INTERVAL_MS ?? "300000"),
  mcpManifestRefreshBatchSize: Number(parsed.DEN_MCP_MANIFEST_REFRESH_BATCH_SIZE ?? "20"),
  mcpManifestRefreshLeaseMs: Number(parsed.DEN_MCP_MANIFEST_REFRESH_LEASE_MS ?? "60000"),
  mcpManifestRevalidateConcurrency: Number(parsed.DEN_MCP_MANIFEST_REVALIDATE_CONCURRENCY ?? "3"),
