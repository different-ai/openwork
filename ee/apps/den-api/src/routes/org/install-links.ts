import { installConfigSchema, installExperienceConfigSchema } from "@micx/install-config"
import { connectLinkClaimsSchema } from "@micx/connect-link"
import { and, eq, gt, isNull, or } from "@micx-ee/den-db/drizzle"
import { InstallLinkTable, OrganizationTable } from "@micx-ee/den-db/schema"
...
import { z } from "zod"
import { MICX_DOWNLOAD_URL } from "../../CONSTS.js"
...
const defaultInstallerDependencies: InstallExperienceDependencies = {
  ...
  resolveDirectUrl: (platform, releaseTag) => {
    ...
    return fileName ? installerReleaseAssetUrl(fileName, { releaseTag }) : MICX_DOWNLOAD_URL
  },
...
function buildInstallConfig(input: { organization: { name: string; logo: string | null; metadata: unknown }; request: Request }) {
  ...
  return installConfigSchema.parse({
    appName: typeof metadata.brandAppName === "string" ? metadata.brandAppName : "Micx",
...
export function registerOrgInstallLinkRoutes<T extends { Variables: OrgRouteVariables }>(
  ...
  app.post(
    ...
    describeRoute({
      ...
      description: "Mints a shareable Micx desktop install link for a signed-in organization member. Older active links remain valid unless an owner or admin explicitly requests rotation.",
      responses: {
  ...
  app.get(
    ...
    describeRoute({
      tags: ["Organizations"],
      summary: "Download managed Micx desktop",
