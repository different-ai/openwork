...
const connectionResponseSchema = z.object({
  ...
  /** Cached search catalog health for this caller's shared or per-member principal. */
  tools: z.object({
...
function responseManifestPrincipal(
...
async function toConnectionResponses(input: {
  ...
  const [manifests, connectedAccounts] = await Promise.all([
    getManifests({ pairs: manifestPairs }),
    getConnectedAccounts({
...
export function registerMcpConnectionRoutes<T extends { Variables: OrgRouteVariables & RequestIdVariables }>(app: Hono<T>) {
  ...
  app.post(
    ...
    describeRoute({
      ...
      description: "Workspace owners and admins can refresh shared connections. Granted members can refresh their own per-member catalog. Refresh uses the selected MCP runtime and the same bounded tools/list lifecycle as capability search.",
      responses: {
