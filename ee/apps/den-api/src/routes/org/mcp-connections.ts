...
} from "../../capability-sources/external-mcp-live-diagnostics.js"
import {
    ...
    async (c) => {
      ...
      try {
        ...
        await completeExternalMcpAuth({
          ...
          signedState: state,
          credentialFence: diagnosticAttemptId && diagnosticAttemptGeneration && diagnosticLease
        ...
        const diagnostic = externalMcpDiagnosticForResponse(error, c.get("requestId"), diagnosticPhaseFromError(error))
        console.error("external_mcp_connect_callback_token_exchange_failed", {
          connectionId: connection.id,
          organizationId: statePayload.organizationId,
          ...externalMcpDiagnosticForLog(error, c.get("requestId"), diagnostic.phase),
        })
        return c.html(connectCallbackPage({
          ok: false,
          name: connection.name,
