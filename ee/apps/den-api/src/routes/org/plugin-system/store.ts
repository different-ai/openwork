...
import { marketplaceMcpServerEntries, resolveMarketplacePluginCloudReadiness } from "../../../mcp/marketplace-capabilities.js"
...
import { upsertOrgOAuthClient } from "../../../capability-sources/oauth-credentials.js"
import { connectExternalMcp } from "../../../capability-sources/external-mcp-client.js"
...
export async function ensurePluginMcpServerInstance(input: {
  ...
  if (existing) {
    if (existing.pluginId !== input.pluginId) {
      // The instance stays owned by the plugin that configured it. A
      // re-import from another plugin reuses the connection (its payload
      // still references the id) but must not steal governance attribution
      // — the original item would silently lose its configured instance.
      return existing
    }
    const updated = {
      configObjectId: input.configObjectId,
    ...
    await db
      .update(PluginMcpServerInstanceTable)
      .set(updated)
      .where(eq(PluginMcpServerInstanceTable.id, existing.id))
...
async function pluginOwnedExternalMcpConnectionIds(input: {
  ...
  const instanceRows = await db
    .select({ connectionId: PluginMcpServerInstanceTable.externalMcpConnectionId })
    .from(PluginMcpServerInstanceTable)
    .where(and(
  ...
  if (instanceRows.length > 0) {
    return instanceRows.map((row) => row.connectionId)
  }
...
async function activePluginReferencesExternalMcpConnection(input: {
  ...
  const normalizedConnectionId = normalizeDenTypeId("externalMcpConnection", input.connectionId)
  const instanceRows = await db
    .select({ pluginId: PluginMcpServerInstanceTable.pluginId })
    .from(PluginMcpServerInstanceTable)
    .innerJoin(PluginTable, eq(PluginMcpServerInstanceTable.pluginId, PluginTable.id))
    .where(and(
      eq(PluginMcpServerInstanceTable.organizationId, input.context.organizationContext.organization.id),
      eq(PluginMcpServerInstanceTable.externalMcpConnectionId, normalizedConnectionId),
      eq(PluginTable.status, "active"),
      isNull(PluginTable.deletedAt),
    ))
  if (instanceRows.some((row) => row.pluginId !== input.excludingPluginId)) return true
...
async function deletePluginOwnedExternalMcpConnections(input: {
  ...
  await db
    .delete(PluginMcpServerInstanceTable)
    .where(and(
      eq(PluginMcpServerInstanceTable.organizationId, input.context.organizationContext.organization.id),
      eq(PluginMcpServerInstanceTable.pluginId, input.pluginId),
    ))
...
export async function wrapStandaloneExternalMcpConnections(input: {
  ...
  for (const connection of connections) {
    ...
    const grants = await listExternalMcpConnectionAccess(connection.id)
