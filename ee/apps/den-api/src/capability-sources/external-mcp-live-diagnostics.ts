...
export async function expireMcpDiagnosticAuthorizationIfEligible(input: {
  ...
  return db.transaction(async (tx) => {
    ...
    await tx.insert(McpDiagnosticEventTable).values({
      ...
      retryable: false,
      actionOwner: input.actionOwner ?? "member",
      operatorAction: "restart_provider_authorization",
...
export async function appendMcpDiagnosticEvent(input: {
  ...
  return db.transaction(async (tx) => {
    ...
    const rows = await tx
      ...
      .limit(1)
    const attempt = rows[0]
    if (!attempt) throw new McpDiagnosticAttemptClosedError()
    if (attempt.status === "succeeded" || attempt.status === "failed" || attempt.status === "expired") {
...
export async function failMcpDiagnosticAttempt(input: {
  ...
  const classified = classifyMcpDiagnosticFailure(input.error, input.phase)
  const failure = classified.actionOwner === "member"
    && input.credentialMode === "shared"
    && (input.phase.startsWith("AUTH_") || input.phase === "CONTINUITY_REFRESH")
    ? {
        ...classified,
        actionOwner: "organization_admin" as const,
        operatorAction: "reconnect_organization_managed_provider_account",
      }
    : classified
