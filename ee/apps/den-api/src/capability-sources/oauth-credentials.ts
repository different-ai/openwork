...
import { createDenTypeId, isDenTypeId, type DenTypeId } from "@openwork-ee/utils/typeid"
...
export async function getConnectedAccounts(input: {
  ...
  return new Map(rows.map((row) => [row.providerId, normalizeConnectedAccountRow(row)]))
}
...
export async function disconnectAccount(input: {
  ...
  if (isDenTypeId("externalMcpConnection", input.providerId)) {
    await db.delete(ExternalMcpToolManifestTable).where(and(
      eq(ExternalMcpToolManifestTable.externalMcpConnectionId, input.providerId),
      eq(ExternalMcpToolManifestTable.principal, input.orgMembershipId),
    ))
  }
  return true
}
