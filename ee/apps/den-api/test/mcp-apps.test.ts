...
test("shared URI contract accepts only exact immutable App resources", () => {
  ...
  expect(mcpAppToolName(id)).toBe(`open_app_${id}`)
...
describe.skipIf(!process.env.DEN_TEST_DATABASE_URL)("authored MCP Apps with isolated database fixtures", () => {
  ...
  test("create publishes a compiled immutable revision in a private Plugin with creator manager grants only", async () => {
    ...
    const app = await apps.createMcpApp({ context, ...source })
    expect(mcpAppSummarySchema.parse(app)).toEqual(app)
    expect(await apps.listAccessibleMcpApps(access(context))).toEqual([app])
  ...
  test("existing Plugin sharing enables resources, not source edits or viewer resharing", async () => {
    ...
    const app = await apps.createMcpApp({ context, ...source, title: "Second app", pluginId: first.pluginId })
    ...
    await expect(apps.updateMcpApp({ context: viewer, ...source, appId: app.appId, expectedRevisionId: app.revisionId })).rejects.toThrow("Missing editor access")
  ...
  test("App managers can attach existing Apps without creating an ownership dependency or duplicate launch", async () => {
    ...
    expect(await apps.listAccessibleMcpApps(access(viewer))).toEqual([{ ...app, pluginId: plugin.id }])
  ...
  test("explicit marketplace membership grants authorize resource reads without generic context reads", async () => {
    ...
    expect(await apps.listAccessibleMcpApps(access(viewer))).toEqual([app])
  ...
  test("direct config-object grants allow runtime and only editor grants allow source replacement", async () => {
    ...
    await expect(apps.updateMcpApp({ context: editor, ...source, appId: app.appId, expectedRevisionId: app.revisionId })).rejects.toThrow("Missing editor access")
    await db.update(ConfigObjectAccessGrantTable).set({ role: "editor" }).where(eq(ConfigObjectAccessGrantTable.id, grantId))
    const updated = await apps.updateMcpApp({ context: editor, ...source, title: "Editor revision", appId: app.appId, expectedRevisionId: app.revisionId })
  ...
  test("organization crossing, unassigned admins, and foreign exact revisions are denied", async () => {
    ...
    const app = await apps.createMcpApp({ context, ...source })
    const other = await apps.createMcpApp({ context: otherOrg, ...source })
    ...
    await expect(apps.createMcpApp({ context: otherOrg, ...source, pluginId: app.pluginId })).rejects.toThrow("Plugin is not available")
    ...
    const second = await apps.createMcpApp({ context, ...source, title: "Same org other App" })
  ...
  test("failed compilation publishes nothing, preserves the prior revision, and does not leak diagnostics", async () => {
    ...
    try {
      await expect(apps.updateMcpApp({ context, ...source, appId: app.appId, expectedRevisionId: app.revisionId })).rejects.toThrow("No revision was published")
      await expect(apps.createMcpApp({ context, ...source, title: "Failed new app" })).rejects.toThrow("MCP App compilation failed")
    } finally {
      build.mockRestore()
    }
    ...
    try {
      await expect(apps.updateMcpApp({ context, ...source, appId: app.appId, expectedRevisionId: app.revisionId })).rejects.toMatchObject({
  ...
  test("compiler integrity failures publish nothing and source corruption never leaks through generic reads", async () => {
    ...
    for (const invalid of [
      ...
      try {
        await expect(apps.updateMcpApp({ context, ...source, appId: app.appId, expectedRevisionId: app.revisionId })).rejects.toMatchObject({ code: "mcp_app_invalid_build" })
      } finally {
        build.mockRestore()
      }
    }
  ...
  test("successful updates activate only the new revision and preserve immutable old resources", async () => {
    ...
    const updated = await apps.updateMcpApp({ context, ...source, title: "Revised title", textFallback: "Revised fallback", reactSource: "export default function App() { return <p>New</p> }", appId: app.appId, expectedRevisionId: app.revisionId })
    expect(updated.revisionId).not.toBe(app.revisionId)
    expect(updated.toolName).toBe(app.toolName)
    expect(await apps.listAccessibleMcpApps(access(context))).toEqual([updated])
    ...
    await expect(apps.updateMcpApp({ context, ...source, appId: app.appId, expectedRevisionId: app.revisionId })).rejects.toThrow("has changed")
  ...
  test("transactional expected revision guard permits only one concurrent replacement", async () => {
    const context = await actor()
    const app = await apps.createMcpApp({ context, ...source })
    const results = await Promise.allSettled(["First writer", "Second writer"].map((title) => apps.updateMcpApp({ context, ...source, title, appId: app.appId, expectedRevisionId: app.revisionId })))
  ...
  test("archive and delete stop all launches and historical resource reads", async () => {
    const context = await actor()
    const app = await apps.createMcpApp({ context, ...source })
    const updated = await apps.updateMcpApp({ context, ...source, appId: app.appId, expectedRevisionId: app.revisionId })
    const configObjectId = normalizeDenTypeId("configObject", app.appId)
    for (const action of ["archive", "delete"] satisfies Array<"archive" | "delete">) {
      ...
      await expect(apps.updateMcpApp({ context, ...source, appId: app.appId, expectedRevisionId: updated.revisionId })).rejects.toThrow("not available")
  ...
  test("shared writes preserve session freshness while source reads and private edits do not require step-up", async () => {
    ...
    const app = await apps.createMcpApp({ context: stale, ...source })
    const updated = await apps.updateMcpApp({ context: stale, ...source, appId: app.appId, expectedRevisionId: app.revisionId })
    await grantPlugin(context, app, viewer)
    await expect(apps.readMcpApp({ context: stale, appId: app.appId })).resolves.toBeDefined()
    await expect(apps.updateMcpApp({ context: stale, ...source, appId: app.appId, expectedRevisionId: updated.revisionId })).rejects.toMatchObject({ error: "reauth" })
    await expect(apps.createMcpApp({ context: stale, ...source, pluginId: app.pluginId })).rejects.toMatchObject({ error: "reauth" })
  })
  ...
  test("encoded payload size is bounded even when the HTML byte limit passes", async () => {
    ...
    try {
      await expect(apps.createMcpApp({ context, ...source })).rejects.toMatchObject({ code: "mcp_app_invalid_build" })
