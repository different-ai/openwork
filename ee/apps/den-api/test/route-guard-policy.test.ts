...
const routeGuardExceptions = new Map<string, string>([
  ...
  ["POST /v1/webhooks/tag/slack/:connectionId", "Slack HMAC signature and replay window are validated in-handler"],
  ["POST /v1/webhooks/tag/slack/oauth", "Deployment Slack app HMAC signature and replay window are validated in-handler"],
  ["GET /v1/tag/slack/oauth/callback", "Signed single-use OAuth state and active organization membership are validated in-handler"],
