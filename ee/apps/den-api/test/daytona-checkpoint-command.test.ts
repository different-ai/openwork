...
function seedRequiredEnv() {
  ...
  process.env.DAYTONA_RUNTIME_DATA_PATH = "/tmp/micx-data"
  process.env.DAYTONA_RUNTIME_WORKSPACE_PATH = "/tmp/micx-workspace"
  process.env.DAYTONA_DATA_MOUNT_PATH = "/persist/micx"
  process.env.DAYTONA_WORKSPACE_MOUNT_PATH = "/workspace"
  process.env.DAYTONA_SIDECAR_DIR = "/tmp/micx-sidecars"
...
describe("Daytona Micx checkpoint start command", () => {
  test("keeps checkpoint seams and safety invariants in the sandbox script", () => {
    const command = daytona.buildMicxStartCommand({
    ...
    expect(command).toContain("MICX_STATE_MANIFEST=")
    ...
    expect(command).toContain("ENGINE_STATE_PATH=${MICX_ENGINE_STATE_PATH:-$HOME/.local/share/opencode}")
    expect(command).toContain('MICX_STATE_MANIFEST="/tmp/micx-data /tmp/micx-workspace $ENGINE_STATE_PATH"')
    ...
    expect(command).toContain("/tmp/micx-data /tmp/micx-workspace")
    expect(command).toContain("CHECKPOINT_DIR=")
    expect(command).toContain("/persist/micx/checkpoints")
    ...
    const serverStart = command.indexOf(" micx-server --workspace")
