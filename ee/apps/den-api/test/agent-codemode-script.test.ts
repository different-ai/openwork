...
test("advertises standard Workflow discovery and execution instructions", async () => {
  ...
  expect(initialized.instructions).toContain("For a new app, dashboard, or interactive view, call create_app directly")
  expect(initialized.instructions).toContain("Builder tools are not capability search results")
