...
let diagnostics: typeof import("../src/capability-sources/external-mcp-live-diagnostics.js")
...
beforeAll(async () => {
  ...
  const [appMod, dbMod, schemaMod, drizzleMod, diagnosticsMod, connectionsMod, externalMcpClientMod, runnerMod, genericOAuthMod, oauthCredentialsMod, envMod] = await Promise.all([
    ...
    import("../src/capability-sources/external-mcp-live-diagnostics.js"),
...
test("revoked refresh credentials are cleared durably before reauthorization", async () => {
...
test("shared OAuth failures consistently assign recovery to an organization admin", async () => {
  const connection = await createOAuthDiagnosticConnection("Shared owner")
  const attempt = await diagnostics.createMcpDiagnosticAttempt({
    organizationId,
    connectionId: connection.id,
    createdByOrgMembershipId: adminMemberId,
  })
  const invalidToken = new Error("provider detail must remain redacted")
  invalidToken.name = "InvalidTokenError"

  const event = await diagnostics.failMcpDiagnosticAttempt({
    organizationId,
  ...
  expect(event.actionOwner).toBe("organization_admin")
  expect(event.operatorAction).toBe("reconnect_organization_managed_provider_account")
...
test("callback deadline aborts the SDK token exchange without late credential, client, or grant mutation", async () => {
  ...
  try {
    await externalMcpClient.completeExternalMcpAuth({
      ...
      signedState,
      credentialFence: { attemptId, generation, leaseId: lease.leaseId },
  ...
  expect(failure).toHaveProperty("diagnostic.phase", "AUTH_TOKEN_ACQUISITION")
...
test("post-OAuth callback wrong-audience rejection remains resource validation", async () => {
  const attempt = await diagnostics.createMcpDiagnosticAttempt({
    organizationId,
    connectionId: requireConnectionId(),
    createdByOrgMembershipId: adminMemberId,
  ...
  const error = externalMcpClient.postAuthorizationResourceValidationError(
    "https://mcp.enterprise.example.test/mcp?token=must-not-appear",
  )
  ...
  await diagnostics.failMcpDiagnosticAttempt({
    ...
    healthLevel: "reachable",
    error,
    url: "https://mcp.enterprise.example.test/mcp?token=must-not-appear",
