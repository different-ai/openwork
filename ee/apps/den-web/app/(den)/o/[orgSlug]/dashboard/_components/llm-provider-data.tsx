...
export async function requestLlmProviderCatalog(orgId: string) {
  const { response, payload } = await requestJson(`/v1/org/llm-provider-catalog`, { method: "GET" }, 20000);
  if (!response.ok) {
...
export async function requestLlmProviderCatalogDetail(orgId: string, providerId: string) {
  const { response, payload } = await requestJson(
    `/v1/org/llm-provider-catalog/${encodeURIComponent(providerId)}`,
    { method: "GET" },
    20000,
  );
...
export function useOrgLlmProviders(orgId: string | null) {
  ...
  async function loadProviders() {
    ...
    setError(null);
    try {
      const { response, payload } = await requestJson(`/v1/org/llm-providers`, { method: "GET" }, 15000);
      if (!response.ok) {
