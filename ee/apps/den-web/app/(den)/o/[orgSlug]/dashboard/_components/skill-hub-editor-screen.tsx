    ...
    async function saveHub() {
        ...
        try {
            ...
            if (!nextSkillHubId) {
                ...
                const { response, payload } = await requestJson(
                    `/v1/org/skill-hubs/${encodeURIComponent(nextSkillHubId)}`,
                    {
                        method: "PATCH",
                        body: JSON.stringify({
                            name: name.trim(),
            ...
            await Promise.all(
                teamIdsToAdd.map(async (teamId) => {
                    const { response, payload } = await requestJson(
                        `/v1/org/skill-hubs/${encodeURIComponent(nextSkillHubId)}/access`,
                        {
                            method: "POST",
            ...
            await Promise.all(
                teamAccessIdsToRemove.map(async (accessId) => {
                    const { response, payload } = await requestJson(
                        `/v1/org/skill-hubs/${encodeURIComponent(nextSkillHubId)}/access/${encodeURIComponent(accessId)}`,
                        { method: "DELETE" },
                        12000,
                    );
            ...
            await Promise.all(
                skillIdsToAdd.map(async (entry) => {
                    const { response, payload } = await requestJson(
                        `/v1/org/skill-hubs/${encodeURIComponent(nextSkillHubId)}/skills`,
                        {
                            method: "POST",
            ...
            await Promise.all(
                skillIdsToRemove.map(async (entry) => {
                    const { response, payload } = await requestJson(
                        `/v1/org/skill-hubs/${encodeURIComponent(nextSkillHubId)}/skills/${encodeURIComponent(entry)}`,
                        { method: "DELETE" },
                        12000,
                    );
