...
export function OrgDashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const orgSwitcher = (
    <div className="relative">
      ...
      {switcherOpen ? (
        <div className="absolute bottom-[calc(100%+0.5rem)] left-0 z-30 grid w-[240px] gap-1 rounded-2xl border border-gray-200 bg-white py-2 shadow-[0_14px_28px_-18px_rgba(15,23,42,0.18)]">
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#f6f9fc] md:flex-row">
      <aside className="w-full shrink-0 border-b border-gray-200 bg-white md:flex md:min-h-screen md:w-[260px] md:flex-col md:border-b-0 md:border-r">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.label}
                    href={item.href}
                    className={`flex items-center justify-between gap-3 rounded-xl border px-3 py-2.5 text-[13px] tracking-normal transition-colors ${
                      selected
                        ? "border-gray-200 bg-white text-gray-950 shadow-[0_1px_2px_rgba(15,23,42,0.05)]"
                  ...
                  >
                    ...
                    {item.badge ? (
                      <span className="rounded-full border border-gray-200 bg-white px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.14em] text-gray-500">
                        {item.badge}
                      </span>
                    ) : null}
                  </Link>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-14 shrink-0 items-center justify-between border-b border-gray-100 bg-white px-5 md:px-6">
          <div className="flex items-center gap-2">
            <span className="text-[14px] tracking-normal text-gray-900">
              {pageTitle}
            </span>
          </div>
