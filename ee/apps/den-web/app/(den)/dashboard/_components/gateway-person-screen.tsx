...
import { getAiGatewayLimitRoute, getAiGatewayRoute, getNewAiGatewayLimitRoute, type DenOrgMember, type DenOrgTeam } from "../../_lib/den-org";
...
export function GatewayPersonScreen({ memberId }: { memberId: string }) {
  ...
  return <GatewayPerson key={`${orgId}:${memberId}`} orgId={orgId} orgSlug={orgSlug} member={member} teams={orgContext.teams} />;
}

export function GatewayPerson({ orgId, orgSlug, member, teams }: { orgId: string; orgSlug: string | null | undefined; member: DenOrgMember; teams: DenOrgTeam[] }) {
  ...
  return (
    <div className="flex flex-col gap-3" data-testid="gateway-person">
      <Link href={getAiGatewayRoute(orgSlug)} className="inline-flex w-fit items-center gap-1.5 text-[12px] text-gray-500 hover:text-gray-900">
