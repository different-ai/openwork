...
function OrgMark({ name }: { name: string }) {
  ...
  return (
    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#011627] text-xs font-semibold uppercase tracking-[0.08em] text-white dark:bg-[#e2e8f0] dark:text-[#0a0f1a]">
      {initials}
    </div>
  );
}
...
export function OrgDashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const orgSwitcher = (
    <div className="relative">
      <button
        type="button"
        className="flex w-full items-center justify-between gap-3 rounded-xl px-2 py-2 text-left transition-colors hover:bg-gray-100 dark:hover:bg-white/5"
          ...
          <div className="grid gap-0.5 px-1.5">
            {orgDirectory.map((org) => (
              ...
              >
                ...
                {org.isActive ? (
                  <svg className="h-4 w-4 shrink-0 text-gray-900 dark:text-gray-100" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
  ...
  const sidebarContent = (
    <div className="relative flex flex-1 flex-col overflow-hidden">
      <div className="border-b border-gray-100 px-4 pb-4 pt-5 dark:border-gray-800">
      ...
      <nav className="flex-1 overflow-y-auto min-h-0 px-3 py-5 pb-24">
      ...
      <div className="absolute bottom-0 left-0 right-0 border-t border-gray-100 bg-white p-3 dark:border-gray-800 dark:bg-[var(--dls-sidebar)]">
        {orgSwitcher}

        {orgBusy ? (
          <p className="mt-3 px-2 text-[11px] text-gray-400 dark:text-gray-400">Refreshing workspace…</p>
        ) : null}
        {orgError ? (
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#fafafa] dark:bg-[var(--dls-app-bg)] md:flex-row md:pl-[260px]">
      {/* Desktop sidebar — always visible at md+ */}
      <aside className="hidden border-r border-gray-100 bg-white dark:bg-[var(--dls-surface)] md:fixed md:left-0 md:top-0 md:z-30 md:flex md:h-screen md:w-[260px] md:flex-col dark:border-gray-800 dark:bg-[var(--dls-sidebar)]">
        {sidebarContent}
      </aside>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-14 shrink-0 items-center justify-between border-b border-gray-100 bg-white dark:bg-[var(--dls-surface)] px-4 md:px-6 dark:border-gray-800 dark:bg-[var(--dls-sidebar)]">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[14px] tracking-[-0.1px] text-gray-900 dark:text-gray-100">
              {pageTitle}
            </span>
          </div>
        ...
        <main className="flex-1 overflow-y-auto bg-[#fafafa] dark:bg-[var(--dls-app-bg)]">{children}</main>
      </div>
    </div>
  );
}
