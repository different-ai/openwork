...
import { ArrowDown, ArrowLeft, ArrowUp, Check, LayoutDashboard, Plus, Trash2 } from "lucide-react";
...
import { AccessAddPicker, AccessGrantRow, OrgWideAccessToggle, TeamIdentity, formatAccessDate, type AccessCandidate } from "./access-audience";
import {
      ...
      {error ? (
        ...
        <div className="rounded-2xl border border-gray-100 bg-white">
          {isLoading ? (
            ...
            <>
              <OrgWideAccessToggle
                on={Boolean(orgWideGrant)}
                disabled={busy}
                onDescription="All organization members see this dashboard on their desktop."
                offDescription="Only people and teams you add below see this dashboard on their desktop."
                onToggle={() => void handleToggleOrgWide()}
              />
                ...
                {memberGrants.map((grant) => {
                  ...
                  return (
                    <AccessGrantRow
                      ...
                      meta={<>assigned by {sharedBy} · {formatAccessDate(grant.createdAt)}</>}
                      disabled={busy}
                      onRevoke={() => void handleRevoke(grant.id)}
                    />
                  );
                })}
