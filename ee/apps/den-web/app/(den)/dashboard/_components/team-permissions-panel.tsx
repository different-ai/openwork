  ...
  return <section aria-label="Team permissions" className="mb-8 space-y-5">
    ...
    {preview ? <section id={previewId} aria-label="Member experience under this team policy" className="rounded-xl border border-gray-200 bg-white p-4">
      ...
      <dl aria-live="polite" className="mt-3 divide-y divide-gray-100 text-sm">
        {[{ label: "Browse websites", value: teamWebsiteSummary(draft.execution) }, { label: "Run computer commands", value: draft.execution.commands === "deny" ? "Blocked" : draft.execution.blockedCommands.length ? "Some commands blocked" : "Allowed" }, { label: "Upload files & submit forms", value: draft.execution.blockBrowserUploads ? "Blocked" : "Allowed" }, ...teamCapabilities.map((entry) => ({ label: entry.teamLabel, value: capabilities[entry.id] ? "Allowed" : "Blocked" }))].map((entry) => <div key={entry.label} className="flex flex-wrap justify-between gap-2 py-2"><dt className="text-gray-700">{entry.label}</dt><dd className="text-gray-500">{entry.value}</dd></div>)}
      </dl>
