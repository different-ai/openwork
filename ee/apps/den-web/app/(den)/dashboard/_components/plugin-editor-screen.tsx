...
import { getMcpPluginImportRoute, getPluginRoute, getPluginsRoute } from "../../_lib/den-org";
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-6 py-10">
      ...
      <div className="mt-8 rounded-[24px] border border-blue-100 bg-blue-50 p-6">
        <h2 className="text-[18px] font-semibold text-gray-900">Import a plugin from GitHub</h2>
        <p className="mt-1 text-[13px] leading-5 text-gray-600">
          Use the Connections importer so OpenWork can inspect every MCP server separately, collect only the credentials it needs, and apply the right team or member access.
        </p>
        <Link
          data-testid="open-smart-github-importer"
          href={getMcpPluginImportRoute(orgSlug)}
          className={`${buttonVariants({ variant: "primary" })} mt-4`}
        >
          Open GitHub importer in Connections
        </Link>
      </div>
