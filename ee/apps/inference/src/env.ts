...
const EnvSchema = z.object({
  ...
  const mode = value.DB_MODE ?? (value.DATABASE_URL ? "mysql" : "planetscale")
  if (mode === "mysql" && !value.DATABASE_URL) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["DATABASE_URL"], message: "DATABASE_URL is required in mysql mode" })
  }
  if (mode === "planetscale") {
    for (const key of ["DATABASE_HOST", "DATABASE_USERNAME", "DATABASE_PASSWORD"] as const) {
      if (!value[key]) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, path: [key], message: `${key} is required in planetscale mode` })
      }
    }
  }
})
...
const planetscale: PlanetScaleCredentials | null =
  parsed.DATABASE_HOST && parsed.DATABASE_USERNAME && parsed.DATABASE_PASSWORD !== undefined
    ? { host: parsed.DATABASE_HOST, username: parsed.DATABASE_USERNAME, password: parsed.DATABASE_PASSWORD }
    : null
...
export const env = {
  ...
  dbMode: (parsed.DB_MODE ?? (parsed.DATABASE_URL ? "mysql" : "planetscale")) as DenDbMode,
  planetscale,
  dbEncryptionKey: parsed.DEN_DB_ENCRYPTION_KEY,
  proxyBaseUrl: optionalString(parsed.INFERENCE_PROXY_BASE_URL),
  openRouterUpstreamUrl: normalizeUrl(parsed.OPENROUTER_UPSTREAM_URL ?? "https://openrouter.ai/api/v1"),
