...
test("the journal preserves consolidated 0097 while allowing later migrations, not obsolete split migrations", () => {
  const journal = JSON.parse(readFileSync(new URL("../drizzle/meta/_journal.json", import.meta.url), "utf8"))
  assert.deepEqual(journal.entries[96], {
    idx: 97, version: "5", when: 1788895934602, tag: "0097_gateway_access_matrix", breakpoints: true,
  })
  assert.equal(journal.entries[97].tag, "0098_gateway_uncountable_usage")
  const consolidated = JSON.parse(readFileSync(new URL("../drizzle/meta/0097_snapshot.json", import.meta.url), "utf8"))
  const next = JSON.parse(readFileSync(new URL("../drizzle/meta/0098_snapshot.json", import.meta.url), "utf8"))
  assert.equal(next.prevId, consolidated.id)
