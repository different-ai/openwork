...
const previous = JSON.parse(await readFile(new URL("0095_snapshot.json", meta), "utf8"))
...
if (last?.idx !== 95 || last.tag !== "0095_gateway_access_matrix") {
  throw new Error("Expected registered 0095; refusing to overwrite migration history")
}
const tag = "0096_gateway_provider_model_universe"
...
await writeFile(new URL("0096_snapshot.json", meta), `${JSON.stringify(snapshot, null, 2)}\n`, { flag: "wx" })
journal.entries.push({ idx: 96, version: snapshot.version, when: Math.max(Date.now(), last.when + 1), tag, breakpoints: true })
...
console.log("Execution SQL stages nullable ADD, selection backfill, then NOT NULL/default. No SQL executed; 0095 and unrelated snapshots unchanged.")
