...
- Node.js + `pnpm`
- 透過 `pnpm install` 安裝 Electron 桌面依賴
...
桌面外殼現在使用 Electron `main` + `preload`，並透過 `window.openworkDesktop` 暴露對話框、Shell、更新與本地執行能力。
...
如果 Electron 打包或沙箱驗證在 Linux 上失敗，請確認已安裝 `libfuse2`、`rpm` 與 `libarchive-tools`，然後重新執行桌面建置。
