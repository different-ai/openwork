...
* For the local `app.openwork.software` flow, use `pnpm dev:web-local` from the repo root. This starts MySQL via `packaging/docker/docker-compose.web-local.yml`, runs Den migrations, points the Next web proxy at the local Den service on `http://127.0.0.1:8788`, and serves the web app on `http://127.0.0.1:3005`.
