---
name: run-tests
description: Run the tests, run e2e locally, run on Daytona, why is this skipped. Use for running and interpreting @micx/testkit specs.
---
...
- App specs require `MICX_EVAL_APP_SPECS=1` and run with
  `vitest --project stack <file>`.
- Set `MICX_EVAL_DAYTONA=1` to place resources in sandboxes. Leave it unset
...
- While iterating, reuse a warm Den with `MICX_EVAL_DEN_API_URL`.
