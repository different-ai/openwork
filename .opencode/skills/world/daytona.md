# Daytona world reference

Follow [world](SKILL.md) for ref selection, matching and private login handoff.
This reference owns Daytona scenarios, release previews and updates. The update
script stays at its compatibility path under `preview-my-work/scripts/`.
...
passing test. Do not dump secret outputs into logs or put them in a PR. Test
account passwords are masked; read the owner-only receipt privately when signing
in, and provide the disposable email/password in the private user handoff as
described in [world](SKILL.md#private-login-handoff).
