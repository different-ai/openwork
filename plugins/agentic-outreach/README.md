...
[OUTPUT]: 对外提供 Agentic Outreach 的价值说明、安装前提、运行流程和产物索引
...
- A worst-case cost preview and explicit approval before contact purchase.
- A short, evidence-grounded sequence with suppression, dedupe, and unsubscribe controls.
- A second explicit approval before any external sender is called.
...
The chosen design centralizes durable business invariants and leaves volatile provider execution outside the product.
...
- `run.json`: current state, totals, approvals, and external references.
- `events.ndjson`: append-only intent/result journal for safe resume and dedupe.
