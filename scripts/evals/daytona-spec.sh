...
LOG_DIR="${MICX_SPEC_LOG_DIR:-/workspace/evals/results/spec-run}"
mkdir -p "$LOG_DIR"
...
if [ -x "$HOME/mark-verified.sh" ]; then
  export MICX_EVAL_MARK_VERIFIED_CMD="bash $HOME/mark-verified.sh {email}"
fi
...
if [ "${MICX_SPEC_NEEDS_DEN:-0}" = "1" ]; then
...
DEN_API_CANDIDATE="${MICX_EVAL_DEN_API_URL:-http://127.0.0.1:8790}"
if curl -sf -m 3 -o /dev/null "$DEN_API_CANDIDATE/health"; then
  export MICX_EVAL_DEN_API_URL="$DEN_API_CANDIDATE"
  export MICX_EVAL_DEN_WEB_URL="${MICX_EVAL_DEN_WEB_URL:-http://localhost:3005}"
  echo "==> Den answers at $MICX_EVAL_DEN_API_URL"
else
  unset MICX_EVAL_DEN_API_URL MICX_EVAL_DEN_WEB_URL
  echo "==> No Den running; den-gated specs will skip (set MICX_SPEC_NEEDS_DEN=1 to start one)"
fi
