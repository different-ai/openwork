...
: "${DATABASE_URL:=mysql://root:password@127.0.0.1:3306/micx_den}"
...
pnpm --filter @micx-ee/den-db db:push <<'EOF'
y
EOF
...
(
  ...
  env \
    ...
    NEXT_PUBLIC_MICX_AUTH_CALLBACK_URL="$DEN_WEB_ORIGIN" \
