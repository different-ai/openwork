...
Use this directory to build and install the Micx package locally on Arch Linux without Docker.
...
- Installs the Electron bundle under `/opt/micx`.
- Adds `/usr/bin/micx`, a desktop entry, and the Micx icon.
...
2. build an Arch package such as `micx-<version>-1-x86_64.pkg.tar.zst`
3. install it locally with `pacman`

After install, `micx` is available as the desktop launcher. The bundled sidecars remain inside `/opt/micx`; the package does not claim the standalone `opencode` command.
...
```bash
sudo pacman -U micx-<version>-1-x86_64.pkg.tar.zst
```
