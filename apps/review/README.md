...
supports `app-web` and the co-located `acme-web` demo. ACME snapshots also boot the real OpenWork desktop
...
goes through the same per-sandbox access check as the other services. The
desktop is optional: if it cannot start, the web, Den and gateway preview still
launch and the desktop link is omitted. The reviewer offers a world selector and a visible personal sandbox panel with service URLs and sign-in details. Developer credentials expand below; copying always returns usable values. Desktop recipes retain their existing placements.
