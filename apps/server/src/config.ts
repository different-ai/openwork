...
export async function resolveServerConfig(cli: CliArgs): Promise<ServerConfig> {
  const configPath = cli.configPath ?? micxServerConfigPath();
  const fileConfig = await loadFileConfig(configPath);
  const configDir = dirname(configPath);

  const envWorkspaces = parseList(process.env.MICX_WORKSPACES);
  ...
  const envOpencodeBaseUrl = process.env.MICX_OPENCODE_BASE_URL;
  const envOpencodeDirectory = process.env.MICX_OPENCODE_DIRECTORY;
  const envOpencodeUsername = process.env.MICX_OPENCODE_USERNAME;
  const envOpencodePassword = process.env.MICX_OPENCODE_PASSWORD;
  ...
  const tokenFromEnv = process.env.MICX_TOKEN;
  const hostTokenFromEnv = process.env.MICX_HOST_TOKEN;
  ...
  const approvalMode =
    cli.approvalMode ??
    (process.env.MICX_APPROVAL_MODE as ApprovalMode | undefined) ??
    fileConfig.approval?.mode ??
    "manual";

  const approvalTimeoutMs =
    cli.approvalTimeoutMs ??
    (process.env.MICX_APPROVAL_TIMEOUT_MS ? Number(process.env.MICX_APPROVAL_TIMEOUT_MS) : undefined) ??
  ...
  const envCorsOrigins = process.env.MICX_CORS_ORIGINS;
  ...
  const envReadOnly = process.env.MICX_READONLY;
  ...
  const envLogFormat = process.env.MICX_LOG_FORMAT;
  const logFormat =
    cli.logFormat ??
  ...
  const envLogRequests = parseBoolean(process.env.MICX_LOG_REQUESTS);
  ...
  const host = cli.host ?? process.env.MICX_HOST ?? fileConfig.host ?? DEFAULT_HOST;
  const port = cli.port ?? (process.env.MICX_PORT ? Number(process.env.MICX_PORT) : undefined) ?? fileConfig.port ?? DEFAULT_PORT;
