...
process.env.MICX_MCP_SYNC_RETRY_DELAY_MS = "10";
...
async function createWorkspaceRoot() {
  const root = await mkdtemp(join(tmpdir(), "micx-mcp-engine-sync-"));
  roots.push(root);
  return root;
}
...
async function startMicxServer(
...
describe("runtime MCP engine sync", () => {
  test("hot-adds a runtime MCP into the running engine when added", async () => {
    ...
    const previousDb = process.env.MICX_RUNTIME_DB;
    process.env.MICX_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    try {
      const mock = startMockOpencode();
      const micx = await startMicxServer(workspaceRoot, `http://127.0.0.1:${mock.server.port}`);

      const response = await fetch(`${micx.base}/workspace/ws_1/mcp`, {
        method: "POST",
        headers: auth(micx.token),
  ...
  test("hot-syncs an external engine without treating its response as trusted registration evidence", async () => {
    ...
    try {
      const mock = startMockOpencode();
      const micx = await startMicxServer(
        workspaceRoot,
  ...
  test("keeps accepted delivery separate from bounded normalized registration evidence", async () => {
    ...
    try {
      ...
      const inspectRegistration = (name: string, config: Record<string, unknown>) =>
        inspectEngineMcpRegistration(micx.config, micx.config.workspaces[0]!, name, config);
      ...
      const listResponse = await fetch(`${micx.base}/workspace/ws_1/mcp`, {
        headers: auth(micx.token),
      });
  ...
  test("records transport-failure provenance and performs one deferred re-sync", async () => {
    const workspaceRoot = await createWorkspaceRoot();
    const previousDb = process.env.MICX_RUNTIME_DB;
    const previousDeferredDelay = process.env.MICX_MCP_SYNC_DEFERRED_DELAY_MS;
    process.env.MICX_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.MICX_MCP_SYNC_DEFERRED_DELAY_MS = "50";
    let posthogPosts = 0;
    try {
      const mock = startMockOpencode({
        mcpResponseForName: (name) => {
      ...
      const micx = await startMicxServer(workspaceRoot, `http://127.0.0.1:${mock.server.port}`);
      const workspace = micx.config.workspaces[0]!;
      ...
      expect(response.status).toBe(200);
      expect(inspectEngineMcpRegistration(micx.config, workspace, "posthog", POSTHOG_CONFIG)).toBe("failed");
      expect(inspectEngineMcpRegistrationDetails(micx.config, workspace, "posthog", POSTHOG_CONFIG).source)
        .toBe("transport_failure");

      await waitForRegistration(
        () => inspectEngineMcpRegistration(micx.config, workspace, "posthog", POSTHOG_CONFIG),
        "connected",
      );
      ...
      if (previousDeferredDelay === undefined) delete process.env.MICX_MCP_SYNC_DEFERRED_DELAY_MS;
      else process.env.MICX_MCP_SYNC_DEFERRED_DELAY_MS = previousDeferredDelay;
    }
  });

  test("scopes registration evidence to the concrete Micx server instance", async () => {
    const workspaceRoot = await createWorkspaceRoot();
    const previousDb = process.env.MICX_RUNTIME_DB;
    process.env.MICX_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    try {
      ...
      const serverA = await startMicxServer(workspaceRoot, `http://127.0.0.1:${engineA.server.port}`);
      const serverB = await startMicxServer(workspaceRoot, `http://127.0.0.1:${engineB.server.port}`);
  ...
  test("invalidates registration evidence when the engine endpoint changes", async () => {
    ...
    try {
      const engineA = startMockOpencode();
      const engineB = startMockOpencode();
      const micx = await startMicxServer(workspaceRoot, `http://127.0.0.1:${engineA.server.port}`);
      const workspace = micx.config.workspaces[0]!;
      ...
      expect(response.status).toBe(200);
      expect(inspectEngineMcpRegistration(micx.config, workspace, "posthog", POSTHOG_CONFIG)).toBe("connected");

      workspace.baseUrl = `http://127.0.0.1:${engineB.server.port}`;
      expect(inspectEngineMcpRegistration(micx.config, workspace, "posthog", POSTHOG_CONFIG))
        .toBe("not-recorded");
  ...
  test("invalidates registration evidence when a managed engine restarts at the same endpoint", async () => {
    ...
    try {
      ...
      const micx = await startMicxServer(workspaceRoot, baseUrl, {
      ...
      await syncAllWorkspacesRuntimeMcpToEngine(micx.config);
      expect(inspectEngineMcpRegistration(micx.config, workspace, "posthog", POSTHOG_CONFIG)).toBe("connected");
  ...
  test("invalidates registration evidence on stop and requires a new server generation", async () => {
    ...
    try {
      const engine = startMockOpencode();
      const micx = await startMicxServer(workspaceRoot, `http://127.0.0.1:${engine.server.port}`);
      const workspace = micx.config.workspaces[0]!;
      ...
      await micx.server.stop(true);
      expect(inspectEngineMcpRegistration(micx.config, workspace, "posthog", POSTHOG_CONFIG))
        .toBe("not-recorded");

      const restarted = await startServer(micx.config) as Served;
  ...
  test("expires registration evidence after the bounded freshness window", async () => {
    const workspaceRoot = await createWorkspaceRoot();
    const previousDb = process.env.MICX_RUNTIME_DB;
    const previousMaxAge = process.env.MICX_MCP_REGISTRATION_MAX_AGE_MS;
    process.env.MICX_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.MICX_MCP_REGISTRATION_MAX_AGE_MS = "5";
    try {
      const engine = startMockOpencode({
      ...
      await Bun.sleep(10);
      expect(inspectEngineMcpRegistrationDetails(micx.config, workspace, "posthog", POSTHOG_CONFIG))
        .toMatchObject({ status: "not-recorded", errorSummary: null });
    } finally {
      if (previousDb === undefined) delete process.env.MICX_RUNTIME_DB;
      else process.env.MICX_RUNTIME_DB = previousDb;
      if (previousMaxAge === undefined) delete process.env.MICX_MCP_REGISTRATION_MAX_AGE_MS;
      else process.env.MICX_MCP_REGISTRATION_MAX_AGE_MS = previousMaxAge;
    }
  });
  ...
  test("invalidates registration evidence before an engine reload attempt", async () => {
    ...
    try {
      ...
      rejectDispose = true;
      const reload = await fetch(`${micx.base}/workspace/ws_1/engine/reload`, {
        method: "POST",
        headers: auth(micx.token),
      });
  ...
  test("cloud plugin install writes a remote MCP and hot-syncs it into the engine", async () => {
    ...
    try {
      ...
      const response = await fetch(`${micx.base}/workspace/ws_1/cloud-plugins`, {
        method: "POST",
        headers: auth(micx.token),
        body: JSON.stringify({
          marketplaceId: null,
      ...
      expect((await readRuntimeOpencodeConfig(micx.config, "ws_1")).mcp?.brief).toMatchObject({
        type: "remote",
  ...
  test("cloud plugin install warns for dropped MCP payloads while still installing skills", async () => {
    ...
    try {
      ...
      expect((await readRuntimeOpencodeConfig(micx.config, "ws_1")).mcp?.broken).toBeUndefined();
  ...
  test("re-registers runtime MCPs with the engine after a reload", async () => {
    ...
    try {
      ...
      const addResponse = await fetch(`${micx.base}/workspace/ws_1/mcp`, {
        method: "POST",
        headers: auth(micx.token),
        body: JSON.stringify({ name: "posthog", config: POSTHOG_CONFIG }),
      });
      ...
      const reloadResponse = await fetch(`${micx.base}/workspace/ws_1/engine/reload`, {
        method: "POST",
        headers: auth(micx.token),
      });
  ...
  test("pushes toggled enabled state to the engine", async () => {
    ...
    try {
      ...
      const toggleResponse = await fetch(`${micx.base}/workspace/ws_1/mcp/posthog/enabled`, {
        method: "POST",
        headers: auth(micx.token),
  ...
  test("disconnects a removed MCP from the engine", async () => {
    ...
    try {
      ...
      const removeResponse = await fetch(`${micx.base}/workspace/ws_1/mcp/posthog`, {
        method: "DELETE",
        headers: auth(micx.token),
      });
  ...
  test("startup sync pushes runtime MCPs for every workspace", async () => {
    ...
    const previousDb = process.env.MICX_RUNTIME_DB;
    process.env.MICX_RUNTIME_DB = join(rootA, "runtime.sqlite");
    try {
      const mock = startMockOpencode();
  ...
  test("MCP add still succeeds when the engine is unreachable", async () => {
    ...
    process.env.MICX_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    try {
      const micx = await startMicxServer(workspaceRoot, "http://127.0.0.1:9");
  ...
  test("engine reload reports engine-unreachable when the engine is down", async () => {
    ...
    try {
      ...
      const response = await fetch(`${micx.base}/workspace/ws_1/engine/reload`, {
        method: "POST",
        headers: auth(micx.token),
      });
