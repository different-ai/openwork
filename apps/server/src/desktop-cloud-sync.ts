...
export function readWorkspaceCloudImports(micx: Record<string, unknown>): WorkspaceCloudImports {
  const cloudImports = isRecord(micx.cloudImports) ? micx.cloudImports : {};
  return {
...
export function readDesktopCloudSyncState(micx: Record<string, unknown>): DesktopCloudSyncState {
  const raw = isRecord(micx.desktopCloudSync) ? micx.desktopCloudSync : {};
...
export function syncDesktopCloudResources(input: {
  ...
  const state = readDesktopCloudSyncState(input.micx);
  const key = contextKey(input.snapshot);
  const previousEntry = state.entries[key] ?? null;
  const changes = diffInstalledCloudResources(readWorkspaceCloudImports(input.micx), input.snapshot, now);
