...
describe("env-file", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "micx-env-"));
  ...
  test("isReservedEnvKey blocks MICX_ / OPENCODE_ prefixes", () => {
    expect(isReservedEnvKey("MICX_TOKEN")).toBe(true);
  ...
  test("upsertMany rejects reserved keys", async () => {
    const svc = new EnvService({ path });
    const promise = svc.upsertMany([{ key: "MICX_TOKEN", value: "x" }]);
    await expect(promise).rejects.toBeInstanceOf(InvalidEnvKeyError);
