...
import {
  readMcpSkillIndex,
  readMicxConnectSkillCatalog,
  renderMicxConnectSkillInstruction,
  resetMicxConnectSkillCatalogCacheForTests,
  type MicxConnectSkill,
...
async function serverConfig(): Promise<ServerConfig> {
  const root = await mkdtemp(join(tmpdir(), "micx-connect-skills-"));
  roots.push(root);
  ...
  return {
    ...
    token: "test",
    hostToken: "host",
    configPath: join(root, "micx.json"),
...
describe("Micx Connect skill catalog", () => {
  test("renders discovery metadata and capability retrieval guidance", () => {
    const instruction = renderMicxConnectSkillInstruction([{
      name: "customer-briefing",
      type: "skill-md",
    ...
    expect(instruction).toContain("micx-cloud_execute_capability");
    ...
    expect(instruction).toContain("Do not call micx-cloud_search_capabilities first");
  ...
  test("renders every authorized skill beyond the former count and character limits", () => {
    const skills: MicxConnectSkill[] = Array.from({ length: 150 }, (_, index) => ({
    ...
    const instruction = renderMicxConnectSkillInstruction(skills);
  ...
  test("omits the prompt block when no authorized skills exist", () => {
    expect(renderMicxConnectSkillInstruction([])).toBe("");
  });
  ...
  test("reads the skill catalog from server-scoped Connect MCP config", async () => {
    ...
    const skills = await readMicxConnectSkillCatalog(config, skillIndexFetcher());
    expect(skills).toHaveLength(1);
    expect(skills[0]?.name).toBe("customer-briefing");
  });

  test("promotes legacy workspace micx-cloud config into server scope", async () => {
    const config = await serverConfig();
    ...
    const skills = await readMicxConnectSkillCatalog(config, skillIndexFetcher("skill:skill_promoted"));
    ...
    resetMicxConnectSkillCatalogCacheForTests();
    const again = await readMicxConnectSkillCatalog(config, skillIndexFetcher("skill:skill_promoted"));
  ...
  test("skips revoked or dead configs and promotes the first working candidate", async () => {
    ...
    const skills = await readMicxConnectSkillCatalog(config, fetcher);
  ...
  test("returns empty when every candidate config is unusable", async () => {
    ...
    expect(await readMicxConnectSkillCatalog(config, fetcher)).toEqual([]);
