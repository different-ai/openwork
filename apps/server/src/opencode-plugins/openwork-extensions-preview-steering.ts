...
export const OPENWORK_CLOUD_SKILL_AUTHORING_INSTRUCTION =
  "Skill creation: Cloud. Follow the injected default skill-creator instructions and their Remote Cloud flow directly. Create or update the skill in OpenWork Cloud, not in the workspace. Do not look for a skill-creator tool or create a local copy to access its instructions. Use a workspace-local skill only when the user explicitly requests one. Do not create both copies; migrate an existing local skill only when the user asks.";

export const OPENWORK_LOCAL_SKILL_AUTHORING_INSTRUCTION =
  "Skill creation: Local. Follow the injected default skill-creator instructions and their local flow directly. Do not look for a skill-creator tool. Do not create a Cloud copy.";
