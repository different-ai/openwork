...
export const OPENWORK_CLOUD_SKILL_AUTHORING_INSTRUCTION =
  "Skill creation: Cloud. When the user asks to create or update a skill, use the built-in skill-creator skill and follow its Remote Cloud flow. Create or update the skill in OpenWork Cloud, not in the workspace. Use a workspace-local skill only when the user explicitly requests one. Do not create both copies; migrate an existing local skill only when the user asks.";

export const OPENWORK_LOCAL_SKILL_AUTHORING_INSTRUCTION =
  "Skill creation: Local. When the user asks to create or update a skill, use the built-in skill-creator skill and follow its local flow. Do not create a Cloud copy.";
