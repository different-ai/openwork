...
- Micx Connect's public hosted endpoint is \`https://api.openworklabs.com/mcp/agent\`. \`app.openworklabs.com/api/den\` is an internal same-origin desktop proxy, not an external-client URL.
- OpenCode is verified with native remote MCP OAuth. Codex is setup-only until native proof is rerun on this exact branch, but its add/login/reconnect commands remain: \`codex mcp add micx --url https://api.openworklabs.com/mcp/agent\`, \`codex mcp login micx\`, and \`codex mcp logout micx\` then \`codex mcp login micx\`. Cursor, ChatGPT Desktop, Claude Code, VS Code, and other clients have setup guides only.
...
- Micx Connect OAuth uses RFC9728 discovery, authorization/browser sign-in at \`https://app.openworklabs.com/api/auth\`, the exact resource \`https://api.openworklabs.com/mcp/agent\`, dynamic client registration fallback, and PKCE S256. For OpenCode, add the remote config then run \`opencode mcp auth micx\`; reconnect or switch orgs with \`opencode mcp logout micx\` then \`opencode mcp auth micx\`. The organization chosen in the browser is pinned into the token.
