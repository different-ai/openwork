...
describe("agent diagnostics effective engine inspection", () => {
  test("reduces valid engine config and agent responses to the bounded allowlist", () => {
    const snapshot = validateEffectiveEngineSnapshot({
      config: {
        default_agent: "micx",
        plugin: [["file:///plugins/micx-extensions-preview.ts", { secret: "not-copied" }]],
        mcp: {
          "micx-cloud": {
            type: "remote",
      ...
      agents: [{
        ...
        hidden: false,
        permission: [{ permission: "micx-cloud_*", pattern: "*", action: "allow" }],
    ...
    expect(snapshot).toMatchObject({
      defaultAgent: "micx",
      pluginSpecs: ["file:///plugins/micx-extensions-preview.ts"],
      agents: [{ name: "micx", mode: "primary", hidden: false }],
      mcps: [{ name: "micx-cloud" }],
    });
  ...
  test("mirrors OpenCode's last-match whole-resource tool visibility rule", () => {
    const rules = [
      { permission: "micx-cloud_*", pattern: "*", action: "allow" as const },
      { permission: "micx-cloud_search_*", pattern: "tenant-a", action: "deny" as const },
      { permission: "micx-cloud_execute_capability", pattern: "*", action: "deny" as const },
    ];

    expect(effectiveToolDecision(rules, "micx-cloud_search_capabilities")).toBe("allow");
    expect(effectiveToolDecision(rules, "micx-cloud_execute_capability")).toBe("deny");
  });
