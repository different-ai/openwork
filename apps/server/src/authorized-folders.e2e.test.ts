...
const roots: string[] = [];
const priorDataDir = process.env.MICX_DATA_DIR;
...
async function createWorkspaceRoot() {
  return createTempRoot("micx-authorized-folders-");
}

async function startMicxServer(workspaceRoot: string, options?: { readOnly?: boolean }) {
...
beforeEach(async () => {
  const envRoot = await createTempRoot("micx-authorized-folders-env-");
  process.env.MICX_DATA_DIR = join(envRoot, "data");
  process.env.MICX_TOKEN_STORE = join(envRoot, "tokens.json");
});
...
afterEach(async () => {
  ...
  if (priorDataDir === undefined) {
    delete process.env.MICX_DATA_DIR;
  } else {
    process.env.MICX_DATA_DIR = priorDataDir;
  }
...
describe("authorized folders routes", () => {
  test("lists visible folders and counts preserved hidden entries", async () => {
    ...
    const { base, config } = await startMicxServer(root);
  ...
  test("requires client auth, collaborator scope, and writable server", async () => {
    ...
    const { base } = await startMicxServer(root);
    ...
    const readOnly = await startMicxServer(readOnlyRoot, { readOnly: true });
