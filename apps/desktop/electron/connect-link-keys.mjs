...
// (micx://connect?token=<JWT>), keyed by the token's `kid` header.
//
...
// Micx Cloud: the vendor's secret store) and never enter this repository.
 ...
 * embedded vendor keys. Dev mode (MICX_DEV_MODE=1) may add ONE ephemeral
...
export function resolveConnectLinkPublicKeys() {
  ...
  if (process.env.MICX_DEV_MODE === "1") {
    const testPem = process.env.MICX_CONNECT_TEST_PUBLIC_KEY_PEM?.trim();
    const testKid = process.env.MICX_CONNECT_TEST_PUBLIC_KEY_KID?.trim();
    if (testPem && testKid) {
