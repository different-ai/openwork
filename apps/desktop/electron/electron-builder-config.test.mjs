...
describe("Electron distribution configs", () => {
  ...
  it("installs both macOS CPU variants for Electron releases", async () => {
    ...
    const publishElectron = workflow.jobs["publish-electron-unix"];
    ...
    assert.match(macosInstall.run, /install --frozen-lockfile/);
    ...
    assert.match(nativeInstall.run, /install --frozen-lockfile/);
    assert.doesNotMatch(nativeInstall.run, /--cpu=/);
  });
});
