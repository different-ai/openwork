...
test("enterprise first run does not create a local workspace", async () => {
  await withIsolatedBootstrapStore(async ({ root }) => {
    const store = createWorkspaceStore({
      app: {
        isPackaged: true,
        getPath: (name) => name === "userData" ? path.join(root, "enterprise-userData") : root,
      },
      defaultDenBaseUrl: "https://default.example.com",
      defaultRequireSignin: false,
      forceRequireSignin: false,
