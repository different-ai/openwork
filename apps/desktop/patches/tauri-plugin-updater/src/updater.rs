...
fn stage_package_for_elevated_install(path: &Path) -> std::io::Result<PathBuf> {
    ...
    let fname = path
        .file_name()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::InvalidInput, "package path has no file name"))?;
...
impl Update {
    ...
    fn try_install_with_privileges(
        ...
        let pkg_path = match stage_package_for_elevated_install(pkg_path) {
            Ok(p) => p,
            Err(err) => {
                log::warn!("failed to stage package for elevated install: {err}");
                pkg_path.to_path_buf()
            }
        };
        let pkg_path = pkg_path.as_path();

        // 0) Debian/Ubuntu: `apt-get install /path.deb` resolves dependencies and
        //    avoids common permission/polkit issues with `dpkg -i` on local files.
        if install_cmd == "dpkg" && install_arg == "-i" {
            ...
            {
                if status.success() {
                    log::debug!("installed {pkg_path:?} with pkexec apt-get");
                    return Ok(());
                }
            }
        }
        ...
        if let Ok(password) = self.get_password_graphically() {
            if install_cmd == "dpkg" && install_arg == "-i" {
                if self.install_apt_with_sudo(pkg_path, &password)? {
                    log::debug!("installed {pkg_path:?} with GUI sudo (apt-get)");
                    return Ok(());
                }
            } else if self.install_with_sudo(pkg_path, &password, install_cmd, install_arg)? {
        ...
        let status = if install_cmd == "dpkg" && install_arg == "-i" {
    ...
    fn install_apt_with_sudo(&self, pkg_path: &Path, password: &str) -> std::io::Result<bool> {
        use std::io::Write;
