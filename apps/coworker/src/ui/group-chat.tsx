...
function GroupChatView({
  ...
  function startTurn(text: string, clientMessageId: string, recovery?: { turn: CoworkerGroupTurn; only?: string }): void {
    ...
    submitGroupSend(group.id, receipt, async () => {
      if (existing?.turnId) {
        ...
        if (status.queue.some((item) => item.clientMessageId === receipt.clientMessageId) || (turn && turn.updatedAt > (receipt.turnUpdatedAt ?? receipt.at))) return { accepted: true };
      }
  ...
  function resume(turn: CoworkerGroupTurn, only?: string): void {
    ...
    setError("");
    startTurn(turn.prompt, existing?.clientMessageId ?? newId("resume"), { turn, only });
  }
    ...
    <div className="glass-main flex h-full min-w-0 flex-1" data-testid="group-chat" data-group-id={group.id} data-live={live ? "true" : "false"}>
      ...
      <header className="glass-header window-drag flex min-h-[78px] shrink-0 flex-wrap items-center gap-x-3 gap-y-2 border-b border-line px-4 py-3" data-testid="conversation-header">
        ...
        <div className="window-no-drag flex shrink-0 items-center gap-1" data-testid="conversation-header-actions">
          ...
          {live ? <Button variant="ghost" disabled={busyActions.includes("stop")} onClick={() => void runAction("stop", () => stopGroupRun(group.id))}>{busyActions.includes("stop") ? "Stopping…" : actionAttempts.current.get("stop")?.state === "retryable" ? "Retry stop" : "Stop all"}</Button> : null}
          <ActionMenu
      ...
      <div ref={scrollRef} onScroll={(event) => { const node = event.currentTarget; followBottom.current = node.scrollHeight - node.scrollTop - node.clientHeight < 80; }} className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
        <div className="mx-auto max-w-3xl space-y-3">
              ...
              return (
                <div key={key} data-client-message-id={event.clientMessageId} data-delivery-state={delivery?.state ?? "recorded"}>
                  ...
                  {delivery ? <div className="mt-1 flex flex-wrap items-center justify-end gap-x-3 gap-y-1 px-2 text-[11px] text-mist" role="status" data-testid={queued ? "group-queued" : delivery.state === "failed" || delivery.state === "uncertain" ? "group-turn-failed" : "group-send-receipt"}>
                    ...
                    {delivery.state === "failed" || delivery.state === "uncertain" ? <button type="button" className="font-medium text-snow/80 underline-offset-2 hover:underline" onClick={() => startTurn(delivery.text, delivery.clientMessageId)}>Retry</button> : null}
          ...
          {localSends.filter((item) => item.turnId).map((item) => <div key={item.clientMessageId} className="flex items-center justify-center gap-3 text-[11px] text-mist" role="status" data-testid="group-recovery-receipt">
            ...
            {item.state === "failed" || item.state === "uncertain" ? <button type="button" className="font-medium text-snow/80 underline-offset-2 hover:underline" onClick={() => startTurn(item.text, item.clientMessageId)}>Retry Continue</button> : null}
          </div>)}
          {pendingAssignment ? (
          ...
          <div className={`relative rounded-[24px] border bg-panel/60 p-3 transition-colors focus-within:border-spark/50 ${assignmentMode ? "border-spark/35" : "border-line"}`} data-testid="coworker-input-surface">
            ...
            <div>
              ...
              <div className="flex items-center gap-2" data-testid="coworker-composer-actions">
                ...
                {live && !assignmentMode ? <Button variant="ghost" disabled={busyActions.includes("stop")} className="mb-0.5 rounded-full px-3 py-1 text-xs" onClick={() => void runAction("stop", () => stopGroupRun(group.id))}>{busyActions.includes("stop") ? "Stopping…" : actionAttempts.current.get("stop")?.state === "retryable" ? "Retry stop" : "Stop"}</Button> : null}
                {assignmentMode ? (
