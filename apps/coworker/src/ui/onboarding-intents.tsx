    ...
    <div className="window-shell flex h-full min-h-[560px] flex-col overflow-y-auto" data-testid="onboarding-intents">
      ...
      <main className="window-no-drag flex flex-1 items-center justify-center px-6 py-8">
        <section className="w-full max-w-[720px]">
          ...
          <div className="mt-7 grid gap-2.5 sm:grid-cols-2 md:grid-cols-3" role="group" aria-label="What your team will help with">
            {catalog.map((role) => {
              ...
              return (
                <button
                  key={role.id}
                  type="button"
                  className={`flex min-h-[104px] flex-col rounded-[18px] border px-4 py-3.5 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-spark/50 ${
                ...
                >
                  <span className="flex items-center justify-between gap-2">
                    <span className="text-[14px] font-semibold text-snow">{role.role}</span>
                    <span
