  ...
  return <div className="space-y-3 border-t border-line/70 px-1 py-3 text-xs leading-relaxed text-mist [overflow-wrap:anywhere]" data-testid="worker-detail">
    ...
    {control ? <section aria-label={`${surface} access for ${worker.name}`} data-testid="worker-control" data-state={control.state} className="space-y-2 rounded-xl border border-line bg-panel px-3 py-2.5">
      ...
      {control.state !== "approved" && alive ? <>
        ...
        <p>{control.surface === "browser" ? "Uses this discussion's tabs and Coworker's shared local login profile, not a new private account. Use Take over in Browser before signing in." : "First allow Computer in this discussion. You must also approve the actual app and window in the native panel. This is your Mac, not a remote computer; human input pauses foreground control. There is no live computer video here."}</p>
        ...
        <p>{control.surface === "browser" ? "Watch the discussion browser beside chat. Take over pauses browser tools; only you can return control." : "Approval is not proof of active control. The Computer strip and native panel show the approved window and current state. Take over and Continue stay in the native panel; no live video is available here."}</p>
        <div className="flex flex-wrap gap-2">
