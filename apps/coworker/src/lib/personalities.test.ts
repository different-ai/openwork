...
test("unknown or missing personalities fall back to the default; none produces no saying", () => {
  ...
  assert.equal(normalizePersonality("chef"), "chef");
...
test("sayings rotate deterministically without immediate repeats, and differ between coworkers", () => {
  const first = Array.from({ length: 40 }, (_, tick) => workingSaying("craftsman", "scout:t1", tick));
  const again = Array.from({ length: 40 }, (_, tick) => workingSaying("craftsman", "scout:t1", tick));
  ...
  const cycle = WORKING_SAYINGS.craftsman.length;
  assert.equal(new Set(first.slice(0, cycle)).size, cycle, "one full cycle visits every saying once");

  const other = Array.from({ length: 6 }, (_, tick) => workingSaying("craftsman", "editor:t9", tick));
...
test("previews are the first sayings the coworker would actually use", () => {
  const preview = previewSayings("navigator", "ops", 3);
  assert.equal(preview.length, 3);
  assert.deepEqual(
    preview,
    [0, 1, 2].map((tick) => workingSaying("navigator", "ops", tick)),
  );
});
