...
test("steering and an admitted turn survive rereads, while pause and stop win settlement", async () => {
  ...
  const appDefaults = { thinking: { model: "delivery/fast", modelVariant: "" }, delivery: { model: "reasoning/deep", modelVariant: "high" } };
  assert.deepEqual(resolveWorkerModel(owner, "thinking", providers, null, {}, appDefaults), modelSnapshot, "coworker role override wins app default");
  assert.equal(resolveWorkerModel({ ...owner, thinkingModel: "" }, "thinking", providers, null, {}, appDefaults).providerId, "delivery");
  assert.equal(resolveWorkerModel({ ...owner, deliveryModel: "" }, "delivery", providers, null, {}, appDefaults).variant, "high");
  assert.throws(() => resolveWorkerModel({ ...owner, thinkingModel: "" }, "thinking", providers, null, {}, { ...appDefaults, thinking: { model: "gone/exact", modelVariant: "" } }), /unavailable.*will not switch/);
  const priced = (reasoning, price) => ({ status: "active", capabilities: { toolcall: true, reasoning }, cost: { input: price, output: price }, variants: { high: {}, low: {} } });
  const automaticProviders = [{ id: "same", models: { anchor: priced(false, 0), deep: priced(true, 0), paid: priced(true, 1) } }, { id: "other", models: { deep: priced(true, 0) } }];
  assert.deepEqual(resolveWorkerModel({ model: "same/anchor" }, "thinking", automaticProviders), { providerId: "same", modelId: "deep", variant: "high" });
  assert.equal(resolveWorkerModel({ model: "same/anchor" }, "delivery", automaticProviders).modelId, "anchor");
  automaticProviders[0].models.deep.cost.output = 1;
  assert.equal(resolveWorkerModel({ model: "same/anchor" }, "thinking", automaticProviders).modelId, "anchor", "automatic Workers cannot escalate either price or provider");
  ...
  assert.deepEqual(resolveWorkerModel(edited, "thinking", providers, (await getWorker(coworkersDir, "scout", worker.id)).modelSnapshot, {}, appDefaults), modelSnapshot, "saved snapshots ignore later owner and app edits");
