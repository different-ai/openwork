...
export function createGroupExecution({ directory, collaboration, coworkerFor, coordinator, catalogFor, clientFor, settings = async () => ({ modelDefaults: DEFAULT_MODEL_DEFAULTS }), onPublished = async () => {}, eventContext = async () => { throw new Error("Event execution is unavailable."); }, conversationContext = async () => { throw new Error("Managed conversation context is unavailable."); }, setupTimeoutMs = 130_000, replyTimeoutMs = 180_000, pollMs = 750 }) {
  ...
  return {
    ...
    async submit(groupId, input) {
      ...
      if (["eventRunId", "eventPhase", "plan", "participantSlugs", "conversationIdentity", "legacyAllHands", "personRequest", "humanRequest", "activityEligible", "coworkerCreatedAts"].some((key) => Object.hasOwn(input, key))) throw new Error("Native group execution identity is not writable.");
      ...
      await collaboration.change((state) => {
        ...
        const request = queued ?? { id: input.clientMessageId, text: input.text, context: input.context ?? "", turnId: input.turnId ?? "", only: input.only, humanRequest: !input.turnId, attempt: input.turnId ? group.retryCounts[input.turnId] : 0,
          activityEligible: !input.turnId, coworkerCreatedAts: Object.fromEntries(members.map((member) => [member.slug, conversation ? conversation.identity.identities[member.slug].createdAt : member.createdAt ?? null])),
