...
export default Plugin.define({ id: "coworker.collaboration", effect: (ctx) => Effect.gen(function* () {
  ...
  const tools = [
    ...
    define("group_manage", "For a direct request in this group chat, add or remove coworkers here, or start a separate parallel group chat. Use exact coworker slugs from the team roster. Event conversation participants change through event_update instead; start_parallel can create a separate ordinary group. Never copy private history into the new chat.",
      { action: schema.enum(["add", "remove", "start_parallel"]), participantSlugs: schema.array(schema.string().regex(/^[a-z0-9][a-z0-9-]*$/)).min(1).max(20), title: schema.string().trim().min(1).max(80).optional() }),
