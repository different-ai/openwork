...
addHost(process.env.MICX_PUBLIC_HOST ?? null);
...
const isElectronPackagedBuild = process.env.MICX_ELECTRON_BUILD === "1";
...
export default defineConfig({
  ...
  define: {
    ...
    "import.meta.env.VITE_MICX_APP_VERSION": JSON.stringify(buildAppVersion),
    "import.meta.env.VITE_MICX_BUILD_SHA": JSON.stringify(shortBuildSha),
  },
  plugins: [
    {
      name: "micx-dev-server-id",
      configureServer(server) {
        server.middlewares.use("/__micx_dev_server_id", (_req, res) => {
