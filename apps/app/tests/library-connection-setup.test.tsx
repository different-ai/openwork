...
describe("LibraryConnectionSetup", () => {
  test.each([
    { authType: "oauth", supportedAuthTypes: ["oauth"] },
    { authType: "apikey", supportedAuthTypes: ["apikey"] },
    { authType: "none", supportedAuthTypes: [] },
    { authType: "oauth", supportedAuthTypes: ["apikey", "none"] },
    { authType: "oauth" },
  ] satisfies Partial<DenExternalMcpPreset>[])("preset setup restricts authentication to the advertised choices: %j", async policy => {
    const preset: DenExternalMcpPreset = {
      presetId: "reference", displayName: "Reference preset", description: "Reference service",
      url: "https://reference.example.test/mcp", authType: "oauth", ...policy,
    };
    const client = clientWith({ listMcpConnectionPresets: mock(async () => [preset]) });
    const host = await mount(client);
    await host.click("Reference preset");
    const selector = host.host.querySelector<HTMLSelectElement>('select[aria-label="Authentication"]');
    if (!selector) throw new Error("Missing authentication selector");
    const expected = preset.supportedAuthTypes?.length ? preset.supportedAuthTypes : [preset.authType];
    expect([...selector.options].map(option => option.value)).toEqual(expected);
    expect(selector.disabled).toBe(expected.length === 1);
    const selected = expected.includes(preset.authType) ? preset.authType : expected[0];
    expect(selector.value).toBe(selected);
    if (selected === "apikey") await host.fill("API key", "fixture-api-key");
    await host.submit();
    expect(client.createMcpConnection).toHaveBeenCalledWith("org_reference", expect.objectContaining({ authType: selected }));
  });

  test("connection status does not disclose the outside account identifier", async () => {
    const saved = connection({ externalAccountId: "outside-account@example.test" });
    const host = await mount(clientWith({ listMcpConnections: mock(async () => [saved]) }), { connectionId: saved.id });
    expect(host.host.textContent).not.toContain(saved.externalAccountId);
    expect(host.host.textContent).toContain("Account sign-in is still required");
  });
