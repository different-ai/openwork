...
describe("deriveOpenTargets", () => {
  ...
  it("extracts PowerPoint decks from assistant artifact summaries", () => {
    const targets = deriveOpenTargets([
      message("msg_1", "assistant", "Updated file: decks/micx-vertebrae-deck.pptx"),
    ]);
    const deck = targets.find((target) => target.value === "decks/micx-vertebrae-deck.pptx");
  ...
  it("extracts artifact paths from Micx extension call metadata", () => {
    const targets = deriveOpenTargets([
      toolMessage("msg_tool", "micx_extension_call", {
