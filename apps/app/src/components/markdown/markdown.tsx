...
function MarkdownBlockInner({
  ...
  const syncHtml = useMemo(
    () => browserMarkdownRenderingKernel.renderSync(text, "conversation"),
    [text],
  );
  const [highlightedHtml, setHighlightedHtml] = useState<{ text: string; html: string } | null>(null);

  useEffect(() => {
    if (streaming || !browserMarkdownRenderingKernel.hasFencedCodeBlock(text)) {
      setHighlightedHtml(null);
      return;
    }

    let cancelled = false;
    void browserMarkdownRenderingKernel.renderHighlighted(text, "conversation").then((html) => {
