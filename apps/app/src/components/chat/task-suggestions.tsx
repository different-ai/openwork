  ...
  return (
    <div className={cn("@container flex flex-col gap-4 pt-1", className)}>
      <p className="text-muted-foreground font-medium select-none">
        {noProviders
          ? t("suggestions.connect_provider_hint")
          : hasOrganizationPrompts
            ? t("suggestions.try_org_prompts")
            : t("suggestions.try_one")}
      </p>
      <div className="grid min-w-0 gap-2 @lg:grid-cols-2 @2xl:grid-cols-3">
        {noProviders ? (
          ...
          >
            ...
            <DescriptiveButtonContent>
              <DescriptiveButtonTitle>{t("suggestions.connect_provider_title")}</DescriptiveButtonTitle>
              <DescriptiveButtonDescription>
        ...
        {hasOrganizationPrompts ? (
          ...
          <>
            <DescriptiveButton orientation="vertical" onClick={() => setPrompt(CSV_PROMPT)}>
              ...
              <DescriptiveButtonContent>
                <DescriptiveButtonTitle>{t("suggestions.edit_csv")}</DescriptiveButtonTitle>
                <DescriptiveButtonDescription>{t("suggestions.edit_csv_desc")}</DescriptiveButtonDescription>
              </DescriptiveButtonContent>
            </DescriptiveButton>
            ...
            <DescriptiveButton orientation="vertical" onClick={() => setPrompt(BROWSER_PROMPT)}>
              ...
              <DescriptiveButtonContent>
                <DescriptiveButtonTitle>{t("suggestions.browse_web")}</DescriptiveButtonTitle>
                <DescriptiveButtonDescription>{t("suggestions.browse_web_desc")}</DescriptiveButtonDescription>
              </DescriptiveButtonContent>
            </DescriptiveButton>
            ...
            >
              ...
              <DescriptiveButtonContent>
                <DescriptiveButtonTitle>{t("suggestions.connect_extension")}</DescriptiveButtonTitle>
                <DescriptiveButtonDescription>{t("suggestions.connect_extension_desc")}</DescriptiveButtonDescription>
              </DescriptiveButtonContent>
            </DescriptiveButton>
          </>
        )}
