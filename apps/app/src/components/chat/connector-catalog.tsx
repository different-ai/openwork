...
export function ConnectorCatalogCard({ catalog }: { catalog: ConnectorCatalog }) {
  ...
  const { connectorIdentities, onConnectionSetup, readyConnectionTargets } = useMessageList()
  ...
  const setup = (entry: ConnectorCatalog["entries"][number]) => {
    void onConnectionSetup(entry.serviceUrl ?? entry.id)
  }
  return <section data-testid="connector-catalog" aria-label="Quick-add connectors" className="w-96 max-w-full self-start rounded-xl bg-muted/40 p-3">
    ...
    <div className="max-h-80 overflow-y-auto">
      {entries.map(entry => {
        const ready = readyConnectionTargets.has(entry.serviceUrl ?? entry.id)
        ...
        return <div key={entry.id} data-connector-preset={entry.id} className="flex min-h-14 items-center gap-2.5 rounded-lg px-1 py-2">
          ...
          <div className="min-w-0 flex-1"><p className="truncate text-[13px] font-medium">{entry.name}</p><p className="text-[11px] text-muted-foreground">{ready ? "Ready to use" : added ? "Added to your organization" : SETUP_LABELS[entry.setup]}</p></div>
          {!ready ? <Button variant="ghost" size="sm" className="h-7 w-24 shrink-0 gap-1 px-2 text-xs" disabled={!auth.isSignedIn} onClick={() => void setup(entry)} aria-label={`Set up ${entry.name}`}>Set up</Button> : <Button variant="ghost" size="sm" className="w-24 gap-1 text-xs" onClick={() => setup(entry)} aria-label={`Manage ${entry.name}`}><Check className="size-3.5" />Connected</Button>}
        </div>
      })}
      {entries.length === 0 ? <p className="py-3 text-xs text-muted-foreground">No connectors match your search.</p> : null}
    </div>
    <p className="mt-2 text-[11px] text-muted-foreground">{auth.isSignedIn ? "Complete setup here, then continue your task." : "Sign in to OpenWork to set up a connection."}</p>
  </section>
}
