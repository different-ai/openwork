...
export function describeSharedBundleImport(bundle: SharedBundleV1): {
  ...
  if (bundle.type === "skill") {
    return {
      title: t("bundle.import_one_skill"),
      description: bundle.description?.trim() || t("bundle.skill_description", undefined, { name: bundle.name }),
      items: [bundle.name],
    };
  }
  ...
  if (bundle.type === "skills-set") {
    ...
    return {
      title: t("bundle.import_n_skills", undefined, { count }),
      description:
        bundle.description?.trim() ||
        t("bundle.skills_set_description", undefined, { name: bundle.name || t("bundle.shared_skills_fallback") }),
      items: bundle.skills.map((skill) => skill.name),
    };
  }

  return {
    title: bundle.name?.trim() || t("bundle.open_workspace_template"),
    description:
      bundle.description?.trim() ||
      t("bundle.workspace_profile_description", undefined, { name: bundle.name || t("bundle.workspace_template_fallback") }),
...
export function parseSharedBundle(value: unknown): SharedBundleV1 {
  const record = readRecord(value);
  if (!record) {
    throw new Error(t("bundle.invalid_payload"));
  }
  ...
  if (schemaVersion !== 1) {
    throw new Error(t("bundle.unsupported_version"));
  }

  if (type === "skill") {
    const content = typeof record.content === "string" ? record.content : "";
    if (!name || !content) {
      throw new Error(t("bundle.invalid_skill_payload"));
    }
    return {
      schemaVersion: 1,
      type: "skill",
  ...
  if (type === "skills-set") {
    const skills = Array.isArray(record.skills)
      ...
      : [];
    if (!skills.length) {
      throw new Error(t("bundle.no_importable_skills"));
    }
    return {
      schemaVersion: 1,
      type: "skills-set",
      name: name || t("bundle.shared_skills_fallback"),
      description: typeof record.description === "string" ? record.description : undefined,
      skills,
    };
  }
  ...
  if (type === "workspace-profile") {
    ...
    if (!workspace) {
      throw new Error(t("bundle.missing_workspace_payload"));
    }
    ...
    return {
      schemaVersion: 1,
      type: "workspace-profile",
      name: name || t("bundle.shared_workspace_profile_fallback"),
      description: typeof record.description === "string" ? record.description : undefined,
      workspace: {
...
export async function fetchSharedBundle(
  ...
  try {
    ...
    throw new Error(t("bundle.invalid_url"));
  }

  if (targetUrl.protocol !== "https:" && targetUrl.protocol !== "http:") {
    throw new Error(t("bundle.url_must_be_https"));
  }
