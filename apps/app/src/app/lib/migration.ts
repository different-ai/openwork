...
export const MIGRATION_KEY_PATTERNS: Array<RegExp> = [
  /^micx\.react\.activeWorkspace$/,
  /^micx\.react\.sessionByWorkspace$/,
  /^micx\.server\.list$/,
  /^micx\.server\.active$/,
  /^micx\.server\.urlOverride$/,
  /^micx\.server\.token$/,
];
...
function electronMigrationBridge(): ElectronMigrationBridge | null {
  ...
  const bridge = (window as unknown as {
    __MICX_ELECTRON__?: { migration?: ElectronMigrationBridge };
  }).__MICX_ELECTRON__;
...
export const MIGRATION_DEFER_KEY = "micx.migration.deferredUntil";
