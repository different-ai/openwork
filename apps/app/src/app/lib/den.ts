...
import {
  ...
  type OpenWorkExtensionSourceFormat,
...
function parseExtensionSourceFormat(value: unknown): OpenWorkExtensionSourceFormat | null {
  const result = openWorkExtensionSourceFormatSchema.safeParse(value);
  return result.success ? result.data : null;
}

function parseOpenWorkExtensionManifest(value: unknown) {
  if (value === null || value === undefined) return null;
  const result = validateOpenWorkExtensionManifest(value);
