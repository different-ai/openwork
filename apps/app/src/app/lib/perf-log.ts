...
type PerfRoot = typeof globalThis & {
  __micxPerfSeq?: number;
  __micxPerfLogs?: PerfLogRecord[];
  __micxPerfConsoleAt?: Record<string, number>;
  __micxPerfConsoleSuppressed?: Record<string, number>;
};
...
export const recordPerfLog = (
  ...
  const id = (root.__micxPerfSeq ?? 0) + 1;
  root.__micxPerfSeq = id;
  ...
  try {
    ...
    const now = Date.now();
    const lastByKey = root.__micxPerfConsoleAt ?? (root.__micxPerfConsoleAt = {});
    const suppressedByKey =
      root.__micxPerfConsoleSuppressed ?? (root.__micxPerfConsoleSuppressed = {});
