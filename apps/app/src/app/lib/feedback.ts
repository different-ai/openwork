const ENV_FEEDBACK_URL = String(import.meta.env.VITE_MICX_FEEDBACK_URL ?? "").trim();
const ENV_APP_VERSION = String(import.meta.env.VITE_MICX_APP_VERSION ?? "").trim();

export const DEFAULT_FEEDBACK_URL =
  ENV_FEEDBACK_URL || "https://micxlabs.com/feedback";

type FeedbackUrlOptions = {
  entrypoint: string;
  deployment?: string | null;
  appVersion?: string | null;
  micxServerVersion?: string | null;
...
export function buildFeedbackUrl(options: FeedbackUrlOptions): string {
  ...
  url.searchParams.set("source", "micx-app");
  ...
  const entries = {
    ...
    micxServerVersion: options.micxServerVersion?.trim() ?? "",
