...
import type {
  MicxDesktopCloudSyncChange,
  MicxDesktopCloudSyncResult,
  MicxDesktopCloudSyncState,
  MicxServerClient,
} from "../lib/micx-server";
...
function readSyncChange(value: unknown): MicxDesktopCloudSyncChange | null {
...
export function readPendingCloudSyncChanges(state: MicxDesktopCloudSyncState): MicxDesktopCloudSyncChange[] {
...
async function runDesktopCloudSync(input: {
  micxClient: MicxServerClient;
  workspaceId: string;
}): Promise<MicxDesktopCloudSyncResult | null> {
  ...
  return input.micxClient.syncDesktopCloud(input.workspaceId, snapshot);
}

export function refreshDesktopCloudSync(input: {
  micxClient: MicxServerClient | null | undefined;
  workspaceId: string | null | undefined;
}): Promise<MicxDesktopCloudSyncResult | null> {
  const micxClient = input.micxClient ?? null;
  const workspaceId = input.workspaceId?.trim() ?? "";
  if (!micxClient || !workspaceId) return Promise.resolve(null);

  const run = desktopCloudSyncQueue.then(() => runDesktopCloudSync({ micxClient, workspaceId }));
