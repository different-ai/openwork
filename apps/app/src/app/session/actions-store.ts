...
export function createSessionActionsStore(options: {
  ...
  const describeProviderError = (error: unknown, fallback: string) => {
    ...
    const heading = (() => {
      if (status === 401 || status === 403) return td("app.error_auth_failed", "Authentication failed", currentLocale());
      if (status === 429) return td("app.error_rate_limit", "Rate limit exceeded", currentLocale());
  ...
  const assertNoClientError = (result: unknown) => {
    ...
    throw new Error(describeProviderError(maybe.error, td("app.error_request_failed", "Request failed", currentLocale())));
  };
  ...
  async function createReadySession(workspaceId: string, initialPrompt?: string) {
    ...
    try {
      ...
      const message = e instanceof Error ? e.message : td("app.unknown_error", "Unknown error", currentLocale());
  ...
  async function sendPrompt(draft?: ComposerDraft) {
    ...
    if (compactCommand && !options.selectedSessionId()) {
      options.setError(td("app.error_compact_no_session", "Select a session with messages before running /compact.", currentLocale()));
      return;
    }
    ...
    try {
      ...
      if (resolvedDraft.mode === "shell") {
        ...
        if (!command) {
          throw new Error(td("app.error_command_not_resolved", "Command was not resolved.", currentLocale()));
        }
  ...
  async function compactCurrentSession(sessionIdOverride?: string) {
    const c = options.client();
    if (!c) {
      throw new Error(td("app.error_not_connected", "Not connected to a server", currentLocale()));
    }

    const sessionID = (sessionIdOverride ?? options.selectedSessionId() ?? "").trim();
    if (!sessionID) {
      throw new Error(td("app.error_compact_no_session_id", "Select a session before compacting.", currentLocale()));
    }

    const visible = options.messages();
    if (!visible.length) {
      throw new Error(td("app.error_compact_empty", "Nothing to compact yet.", currentLocale()));
    }
  ...
  async function renameSessionTitle(sessionID: string, title: string) {
    ...
    if (!trimmed) {
      throw new Error(td("app.error_session_name_required", "Session name is required", currentLocale()));
    }
  ...
  const BUILTIN_COMPACT_COMMAND = {
    id: "builtin:compact",
    name: "compact",
    description: td("app.compact_command_desc", "Summarize this session to reduce context size.", currentLocale()),
