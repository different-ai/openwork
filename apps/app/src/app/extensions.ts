...
export const BUILT_IN_OPENWORK_EXTENSION_MANIFESTS: OpenWorkExtensionManifest[] = [
  ...
  },
  {
    ...
    description: "Connect Google Calendar, Gmail drafts, and selected Drive files to OpenWork agents with scoped OAuth and audit logging.",
    preview: true,
    source: { format: "openwork-builtin", origin: "builtin", trusted: true },
    icon: { simpleIconSlug: "google" },
    composer: { prompt: "Use Google Workspace to " },
    setup: {
      instructions: "Phase 1 uses narrow Google OAuth scopes: calendar read, Gmail draft creation, and Drive files explicitly selected or created through OpenWork.",
      primaryCta: "Connect Google Workspace",
      secondaryCta: "Test connection",
      testActionRef: "openwork.googleWorkspace.testConnection",
    },
    resources: [
      { type: "provider", id: "google-oauth", label: "Google OAuth client", providerId: "google-workspace", required: true },
      { type: "local-service", id: "google-workspace-connector", label: "OAuth token vault and Google API connector", required: true },
      { type: "tool", id: "google-calendar-read", label: "Calendar read tools", required: true },
      { type: "tool", id: "google-gmail-drafts", label: "Gmail draft tools", required: true },
      { type: "tool", id: "google-drive-selected-files", label: "Selected Drive file tools", required: true },
    ],
    contributions: [
