                        ...
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-gray-12">
                                    {currentQuestion()!.header || td("common.question", "Question")}
                                </h3>
                                <div class="text-xs text-gray-11 font-medium">
                                    {td("question_modal.question_counter", "Question {current} of {total}", { current: currentIndex() + 1, total: props.questions.length })}
                                </div>
                            </div>
                        </div>
                        ...
                        <Show when={currentQuestion()!.custom}>
                            <div class="mt-4 pt-4 border-t border-dls-border">
                                <label class="block text-xs font-semibold text-dls-secondary mb-2 uppercase tracking-wide">
                                    {td("question_modal.custom_answer_label", "Or type a custom answer")}
                                </label>
                                <input
                                    type="text"
                                    value={customInput()}
                                    ...
                                    placeholder={td("question_modal.custom_answer_placeholder", "Type your answer here...")}
                                    onKeyDown={(e) => {
                                        if (e.key === "Enter") {
                                            if (e.isComposing || e.keyCode === 229) return;
                                            e.stopPropagation();
                        ...
                        <div class="flex gap-2">
                            <Show when={currentQuestion()?.multiple || currentQuestion()?.custom}>
                                <Button onClick={handleNext} disabled={!canProceed() || props.busy} class="!px-6">
                                    {isLastQuestion() ? td("common.submit", "Submit") : td("common.next", "Next")}
