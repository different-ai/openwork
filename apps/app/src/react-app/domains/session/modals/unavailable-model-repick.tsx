...
export function UnavailableModelRepick(props: {
  ...
  const suggestion = suggestOpenworkReplacement(props.from, catalog.data ?? [], props.workspaceDefault);
  ...
  return <Dialog open onOpenChange={(open) => { if (!open && !saving) props.onClose(); }}>
    <DialogContent showCloseButton={!saving}>
      <DialogHeader>
        <DialogTitle className="pr-8 break-words">{t("models.repick_title", { model: props.from.displayName ?? props.from.modelId })}</DialogTitle>
      </DialogHeader>
      {saved ? <p role="status">{t("models.repick_saved")}</p> : <div className="flex flex-col gap-4">
        <Select disabled={saving} value={choice ? JSON.stringify([choice.providerId, choice.modelId]) : null} onValueChange={(value) => {
          ...
          <SelectContent><SelectGroup>{(catalog.data ?? []).map((model) => <SelectItem key={JSON.stringify([model.providerId, model.modelId])} value={JSON.stringify([model.providerId, model.modelId])} aria-label={`${model.displayName} (${model.providerName})`}><span>{model.displayName}</span><span className="text-muted-foreground">{model.providerName}</span></SelectItem>)}</SelectGroup></SelectContent>
        </Select>
