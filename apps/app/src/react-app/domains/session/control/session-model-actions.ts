...
export function createSessionModelActions<Workspace extends SessionModelWorkspace>(deps: {
  ...
  const readSession = async (workspace: Workspace, sessionId: string, read: ModelRead) => {
    ...
    const session = openworkModelSessionSchema.parse(getSession ? await read(() => getSession(workspace, sessionId)) : (await inventory(workspace, read)).find((session) => session.id === sessionId));
    ...
    async setModel(rawArgs: unknown, helpers?: Pick<OpenworkControlHelpers, "requestCreatedAt">) {
      ...
      if (!args.workspaceId) {
        const matches = (await Promise.all(deps.workspaces.map(async (workspace) => ({
          workspace, sessions: (await inventory(workspace, read)).filter((session) => session.id === args.sessionId),
      ...
      const session = await readSession(workspace, args.sessionId, read);
      ...
      if (statuses[session.id] && statuses[session.id].type !== "idle" || deps.working?.(workspace, args.sessionId)) throw new Error("Working sessions cannot be repicked. Wait until the session is idle.");
      ...
      return save(workspace.id, [session], model, args.dryRun === true, checkDeadline);
    },
