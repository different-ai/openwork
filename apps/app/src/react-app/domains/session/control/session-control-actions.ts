...
export function useSessionControlActions(input: UseSessionControlActionsInput) {
  ...
  const listSessionsControlAction = useMemo<OpenworkControlAction>(() => ({
    ...
    description: "Return every loaded session across workspaces (pinned first, then newest). Entries include `pinned`, `archived` (true only when the archive timestamp is positive; restored sessions with timestamp 0 are false), `status` (idle, thinking, responding, waiting, compacting, error), `working` (own work or known busy/waiting descendants), `descendantActivity` ({ busy, waiting, unknown } counts), `inventoryComplete` (false when referenced descendant activity is unreadable; unknown alone does not imply working) and `model` ({ providerId, modelId, variant }: the model and reasoning effort the session is bound to, null before any model is bound). Check `working` before session.archive. Pass `limit` to cap the count or `workspaceId` to narrow to one workspace.",
    kind: "query",
