...
export function buildLibraryModelProviders(input: {
  ...
  for (const provider of input.pending) {
    ...
    // Sync names a pending set "<provider> / <credential set>"; the Library lists the provider.
    const providerName = provider.name.split(" / ")[0]?.trim() || provider.name;
    const key = `pending:${provider.cloudProviderId}`;
    const current = rows.get(key);
    const models = toModels([...(current?.models ?? []), ...(provider.models ?? []).map((model) => ({ id: model.upstreamModelId || model.id, name: model.name }))], { name: providerName, sourceProviderId });
    rows.set(key, {
      key,
