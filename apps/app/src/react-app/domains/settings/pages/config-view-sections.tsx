...
import type { MicxTestState, TokenVisibilityKey } from "./config-view-state";
...
export function ConfigServerConnectionSection(props: {
  ...
  micxStatusStyle: string;
  resolvedWorkspaceUrl: string;
  resolvedWorkspaceId: string;
  micxTestState: MicxTestState;
  micxTestMessage: string | null;
  hasMicxChanges: boolean;
  ...
  return (
    <div className="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-4">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        ...
        <div className={`text-xs px-2 py-1 rounded-full border ${props.micxStatusStyle}`}>{props.micxStatusLabel}</div>
      </div>
      <div className="grid gap-3">
        <TextInput label={t("config.server_url_input_label")} value={props.micxUrl} onChange={(event) => props.onUrlChange(event.currentTarget.value)} placeholder="http://127.0.0.1:<port>" hint={t("config.server_url_hint")} disabled={props.busy} />
        <label className="block">
          <div className="mb-1 text-xs font-medium text-gray-11">{t("config.token_label")}</div>
          <div className="flex items-center gap-2">
            <input type={props.tokenVisible ? "text" : "password"} value={props.micxToken} onChange={(event) => props.onTokenChange(event.currentTarget.value)} placeholder={t("config.token_placeholder")} disabled={props.busy} className="w-full rounded-xl bg-gray-2/60 px-3 py-2 text-sm text-gray-12 placeholder:text-gray-10 shadow-[0_0_0_1px_rgba(255,255,255,0.08)] focus:outline-none focus:ring-2 focus:ring-gray-6/20" />
      ...
      <div className="flex flex-wrap gap-2">
        <Button variant="outline" onClick={() => void props.onTestConnection()} disabled={props.busy || props.micxTestState === "testing"}>{props.micxTestState === "testing" ? t("config.testing") : t("config.test_connection")}</Button>
        <Button onClick={props.onSave} disabled={props.busy || !props.hasMicxChanges}>{t("common.save")}</Button>
        <Button variant="outline" onClick={props.onReset} disabled={props.busy}>{t("common.reset")}</Button>
      </div>
      {props.micxTestState !== "idle" ? <ConfigConnectionTestStatus state={props.micxTestState} message={props.micxTestMessage} /> : null}
      {props.micxStatusLabel !== t("config.status_connected") ? <div className="text-xs text-gray-9">{t("config.server_needed_hint")}</div> : null}
    </div>
  );
}

function ConfigConnectionTestStatus(props: { state: MicxTestState; message: string | null }) {
  return (
