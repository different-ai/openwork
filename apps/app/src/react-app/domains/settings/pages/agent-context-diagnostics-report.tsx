...
const OWNER_LABEL_KEYS: Record<AgentContextDiagnosticOwner, string> = {
  "micx-client": "connect.diagnostics_owner_micx_client",
  "micx-server": "connect.diagnostics_owner_micx_server",
  ...
  "micx-support": "connect.diagnostics_owner_micx_support",
};
...
const AGENT_STATE_LABEL_KEYS: Record<AgentContextDiagnosticsReport["agent"]["configuredMicxAgent"]["state"], string> = {
...
function AgentEvidence(props: {
  ...
  const agent = props.report.agent.configuredMicxAgent;
  return (
...
function CloudCatalog(props: {
  ...
  const cloudMcp = props.report.mcps.find(
    (mcp) => mcp.source === "config.remote" && mcp.name === "micx-cloud" && mcp.path === "/mcp/agent",
  ) ?? props.report.mcps.find(
    (mcp) => mcp.name === "micx-cloud" && mcp.path === "/mcp/agent",
  );
