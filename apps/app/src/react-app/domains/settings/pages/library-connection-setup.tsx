    ...
    <DialogContent className="flex max-h-[85vh] flex-col sm:max-w-xl">
      ...
      <div className="min-h-0 overflow-y-auto space-y-4">
        ...
        {page.kind === "status" ? <>
          ...
          {selected?.externalAccountId ? <p className="text-xs text-dls-secondary">Account: {selected.externalAccountId}</p> : null}
...
function ExternalConnectionForm(props: Props & { preset?: DenExternalMcpPreset; connection?: DenExternalMcpConnection; onSaved: (id: string) => void; onCancel: () => void }) {
  ...
  const invalid = !fields.name.trim() || !fields.url.trim() || missingKey || (clientRequired && !fields.clientId.trim()) || (!connection && !fields.orgWide && !memberId);
  ...
  return <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
    <fieldset disabled={busy || uncertain} className="space-y-3">
      ...
      <label className="block text-sm">Authentication<select aria-label="Authentication" className="mt-1 block w-full rounded-xl bg-dls-hover p-2" value={fields.authType} disabled={locked} onChange={(event) => { const value = event.currentTarget.value; if (value === "oauth" || value === "apikey" || value === "none") change("authType", value); }}>
        <option value="oauth">OAuth</option><option value="apikey">API key</option><option value="none">No authentication</option>
      </select></label>
