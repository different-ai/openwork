...
import { gatewayConnectProviderKey, isCloudManagedProviderKey, type GatewayConnectProvider } from "./cloud-provider-config";
 ...
 * model, then one of the organization's models, then any ready model, and
 * switch back once the wanted model is ready.
 */
export function decideFirstChatModel(input: {
  ...
  if (input.wanted) {
    ...
    // Once the sign-in lands the model is no longer waiting on it; switch back
    // even if the engine's catalog is still reloading (the composer treats a
    // remembered model as usable until it is confirmed gone).
    const stillPending = input.pendingOptions.some((option) => same(wanted, option));
    return stillPending ? { kind: "none" } : { kind: "restore", wanted };
  }
  ...
  const fallback = ready.find((option) => option.providerID === "openwork")
    ?? ready.find((option) => isCloudManagedProviderKey(option.providerID))
    ?? ready[0];
