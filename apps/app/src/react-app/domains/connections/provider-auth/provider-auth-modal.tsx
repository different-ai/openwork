...
const MICX_MODELS_PROVIDER_ID = "micx";
...
export default function ProviderAuthModal(props: ProviderAuthModalProps) {
  ...
  const [view, setView] = useState<
    "list" | "method" | "api" | "cloud" | "oauth-code" | "oauth-auto" | "micx-subscribe"
  >("list");
  ...
  const entries = useMemo<ProviderAuthEntry[]>(() => {
    ...
    if (props.showMicxModelsSubscribe) {
      const connectedToMicx = connected.has(MICX_MODELS_PROVIDER_ID);
      return [
        {
          id: MICX_MODELS_PROVIDER_ID,
          name: "Micx",
          methods: [{ type: "cloud", label: "Subscribe" }],
          connected: connectedToMicx,
          env: [],
        },
        ...nextEntries.filter((entry) => entry.id.trim().toLowerCase() !== MICX_MODELS_PROVIDER_ID),
      ];
    }

    return nextEntries;
  }, [isRemoteWorker, props.authMethods, props.connectedProviderIds, props.providers, props.showMicxModelsSubscribe]);
  ...
  const handleEntrySelect = (entry: ProviderAuthEntry) => {
    ...
    if (props.showMicxModelsSubscribe && entry.id.trim().toLowerCase() === MICX_MODELS_PROVIDER_ID) {
      setView("micx-subscribe");
      return;
    }
  ...
  const handleBack = () => {
    if (resolvedView === "micx-subscribe") {
      resetState();
      return;
    }
  ...
  const methodDescription = (entry: ProviderAuthEntry, method: ProviderAuthMethod) => {
    ...
    if (method.type === "oauth") {
      return "Continue in the browser and let Micx finish the connection automatically.";
    }
    if (method.type === "cloud") {
    ...
    return "Paste a secret key that Micx stores locally on this device.";
  };
            ...
            <div className="-mr-1 min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
              ...
              {resolvedView === "cloud" && selectedEntry && selectedCloudMethod ? (
                <div className="rounded-xl border border-gray-6/40 bg-gray-2/50 shadow-sm p-5 space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="text-[11px] text-gray-9">
                      Micx will install the provider config and use the credential stored for your org.
                    </div>
              ...
              {resolvedView === "micx-subscribe" && selectedEntry ? (
                <div className="rounded-xl border border-blue-6/50 bg-blue-2/25 shadow-sm p-5 space-y-4">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <div className="text-sm font-medium text-gray-12">Micx Models</div>
                  ...
                  <div className="flex items-center justify-end">
                    <Button onClick={() => void props.onSubscribeMicxModels?.()} disabled={actionDisabled}>
                      Subscribe
                    </Button>
                  </div>
                </div>
