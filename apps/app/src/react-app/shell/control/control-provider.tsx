...
export function OpenworkControlProvider({ children }: { children: ReactNode }) {
  ...
  const featureEnabled = local.prefs.featureFlags?.realtimeControl === true;
  featureEnabledRef.current = featureEnabled;
  const enabled = featureEnabled && enabledState;
  const status: OpenworkControlSnapshot["status"] = !enabled ? "off" : busyActionId ? "acting" : "ready";

  const setEnabled = useCallback((nextEnabled: boolean) => {
    setEnabledState(featureEnabledRef.current ? nextEnabled : false);
  }, []);
  ...
  const executeAction = useCallback(async (actionId: string, args?: unknown): Promise<OpenworkControlResult> => {
    if (!featureEnabled) return { ok: false, actionId, error: "Control mode is disabled in Feature Preview settings" };
  ...
  }, [busyActionId, featureEnabled, playTargetChoreography]);
...
export function OpenworkRouteControlActions() {
  ...
  const actions = useMemo<OpenworkControlAction[]>(() => [
    ...
    {
      id: "route.settings.feature_preview",
      label: "Open feature preview settings",
      description: "Navigate to Feature Preview settings.",
      sideEffect: "navigation",
      execute: () => navigate("/settings/feature-preview"),
    },
  ], [navigate]);
