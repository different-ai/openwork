...
const TOOL_SHORTCUTS = [
  {
    id: "skills",
    label: "Skills",
    description: "Install skills and browse skill bundles.",
    tab: "skills",
  },
  {
    id: "extensions",
    label: "MCPs and plugins",
    description: "Configure MCP servers, plugins, and command surfaces.",
    tab: "extensions",
  },
  {
    id: "automations",
    label: "Automations",
    description: "Review recurring jobs and command-driven workflows.",
    tab: "automations",
  },
] as const;
...
export function ChatPanel() {
  ...
  const openSettingsShortcut = (tab: (typeof TOOL_SHORTCUTS)[number]["tab"]) => {
    setToolsOpen(false);
            ...
            <>
              ...
              <form className="border-t border-slate-100 bg-slate-50/60 px-4 py-4 lg:px-6" onSubmit={handleSubmit}>
                <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                  <label className="block text-xs font-semibold uppercase tracking-[0.18em] text-slate-400" htmlFor="openwork-chat-prompt">
                    Prompt
                  </label>
                  <div className="relative" ref={toolsMenuRef}>
                    ...
                    {toolsOpen ? (
                      <div className="absolute right-0 top-full z-20 mt-2 w-[20rem] rounded-[24px] border border-dls-border bg-white p-2 shadow-[var(--dls-shell-shadow)]">
                        ...
                        <div className="space-y-1">
                          {TOOL_SHORTCUTS.map((shortcut) => (
                            <button
                              key={shortcut.id}
                              className="w-full rounded-[18px] px-3 py-3 text-left transition-colors hover:bg-slate-50"
                              onClick={() => openSettingsShortcut(shortcut.tab)}
                              type="button"
                            >
