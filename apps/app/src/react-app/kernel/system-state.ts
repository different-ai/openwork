...
import { relaunchDesktopApp, resetMicxState } from "../../app/lib/desktop";
import {
...
export type SystemStateControls = {
  ...
  reset: ResetState;
  openResetModal: (mode: ResetMicxMode) => void;
...
function clearMicxLocalStorage(mode: ResetMicxMode) {
  ...
  try {
    ...
    for (const key of keys) {
      if (/micx/.test(key)) window.localStorage.removeItem(key);
    }
    window.localStorage.removeItem("micx_mode_pref");
  } catch {
    // ignore
  }
}
...
export function useSystemState(
  ...
  const [resetModalMode, setResetModalMode] =
    useState<ResetMicxMode>("onboarding");
  ...
  const confirmReset = useCallback(async () => {
    ...
    try {
      if (isDesktopRuntime()) {
        await resetMicxState(resetModalMode);
      }
      clearMicxLocalStorage(resetModalMode);
      if (isDesktopRuntime()) {
