...
`apps/app` is a React 19 + Vite app. It is the UI for every Micx
deployment: the Electron desktop shell loads it, plain web serves it, and it
talks to micx-server / opencode / Den over HTTP. (The Solid runtime it
...
│   ├── lib/                   Clients + bridges: opencode, micx-server, den, desktop (IPC),
...
2. Leaf modules (`runtime-env`, `desktop-types`, `den-types`, `extensions`)
   ...
   (`opencode`, `micx-server`, `den`) import leaves — never the `utils/`
...
- The legacy `micx.react.activeWorkspace` and
  `micx.react.sessionByWorkspace` values are only restore/fallback memory.
...
`shell/micx-context-projector.ts` combines route state, the workbench store,
UI chrome state, and panel-tab state into the shared
`MicxContextSnapshot` contract. `MicxContextPublisher` publishes that
snapshot through `window.__micxControl`; the desktop loopback bridge and
`micx-ui-mcp` expose the same contract without requiring every element to
