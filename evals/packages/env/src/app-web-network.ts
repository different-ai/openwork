...
export async function observeAppWebNetwork(debuggerUrl: string | undefined, webUrl: string) {
  ...
  const requests = new Map<string, { path: string; status?: number; mimeType?: string }>();
  const failures: Array<{ path: string; status?: number; mimeType?: string; error?: string; canceled?: boolean; blockedReason?: string; type?: string }> = [];
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      socket.addEventListener("message", event => {
        ...
        if (message.method === "Network.requestWillBeSent" && record(params.request) && typeof params.request.url === "string") {
          ...
          if (url.origin === origin) requests.set(params.requestId, { path: url.pathname });
        }
        ...
        if (message.method === "Network.responseReceived" && record(params.response)
          ...
          if (params.response.status >= 400 && failures.length < 20) failures.push({ ...request });
        }
        if (message.method === "Network.loadingFailed") {
          if (failures.length < 20) failures.push({ ...request,
