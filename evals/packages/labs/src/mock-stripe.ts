...
type Subscription = {
  ...
  latest_invoice: string; items: { data: { id: string; price: ReturnType<typeof price>; quantity: number }[] };
...
export async function mockStripe() {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (url.pathname.startsWith("/v1/prices/")) result = price(url.pathname.split("/").at(-1) ?? "");
      ...
      if (url.pathname.endsWith("/line_items")) result = list(session ? [{ price: price(session.price), quantity: session.quantity }] : []);
  ...
  return {
    ...
    complete(sessionId: string, paid = true) {
      ...
      subscriptions.set(id, { id, object: "subscription", customer: session.customer, status: "active", metadata: session.metadata,
        latest_invoice: invoiceId, cancel_at_period_end: false, items: { data: [{ id: `si_${id}`, price: price(session.price), quantity: session.quantity }] } });
      return id;
    },
    async webhook(apiUrl: string, type: string, object: unknown, valid = true) {
      const payload = JSON.stringify({ id: `evt_${++eventNumber}`, object: "event", type, data: { object } });
