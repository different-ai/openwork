...
export type TargetRole = "button" | "link" | "textbox" | "checkbox" | "menuitem" | "tab" | "option" | "separator" | "alert";
  ...
  const value = await callFunctionOnSurface(surface, (serialized) => {
    ...
    const selector = target.composer
      ...
      : target.text && !target.role && !target.label && !target.placeholder && !target.testId
        ? 'body *'
        : 'button, a[href], input, textarea, select, [role="combobox"], [role="listbox"], [contenteditable="true"], [role="button"], [role="link"], [role="textbox"], [role="checkbox"], [role="menuitem"], [role="tab"], [role="option"], [role="separator"], [role="alert"], [data-testid]';
