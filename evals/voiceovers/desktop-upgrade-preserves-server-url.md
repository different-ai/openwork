...
1. This Windows desktop installation is connected to Example Manufacturing’s on-prem Micx server, and its organization URL is recorded in the desktop-bootstrap configuration.
...
3. When Micx launches after the upgrade, it reconnects to the configured on-prem server—not a stale, default, or cloud URL.
...
6. Finally, with cloud access unavailable, Micx continues using the on-prem URL without attempting to replace it, preserving support for air-gapped deployments.
