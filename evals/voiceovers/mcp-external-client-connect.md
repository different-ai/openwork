...
MCP on a direct API origin (127.0.0.1:8790 here, api.micxlabs.com in prod)
while browser auth lives on the den-web origin (localhost:3005 here,
app.micxlabs.com in prod). Before the fix, RFC 9728 metadata on the API
...
3. Playing a Cursor-like client, I register via dynamic client registration with a loopback redirect and get sent to the real Micx sign-in page to authorize.
