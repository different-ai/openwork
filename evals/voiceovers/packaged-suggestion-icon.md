...
`/micx-mark.svg` bypasses Vite's asset rebasing, resolves to the filesystem
...
which prepends `import.meta.env.BASE_URL`, producing a relative `./micx-mark.svg`
...
2. I confirm the icon src is a relative path — ./micx-mark.svg — rebased by the resolveExtensionIconSrc helper, not a bare absolute /micx-mark.svg that would break under file://.
