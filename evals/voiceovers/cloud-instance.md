# cloud-instance — Micx Cloud opens a full instance in the browser, booted just in time
...
required for them to run. An enabled org's members get a full Micx instance in
the browser — a Daytona sandbox running micx-server plus the web UI from our
own snapshot. No orchestrator, no download.

1. I'm signed in to Micx on the web. There's no Cloud anywhere — not in the
...
4. I open Cloud. It brings an instance up for me on the spot, and in a few
   seconds I'm looking at the full Micx interface in my browser.
