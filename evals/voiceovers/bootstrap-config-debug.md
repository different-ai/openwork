...
This proof uses an isolated desktop instance, so the bootstrap file shown here is a temporary eval file rather than the user's real Micx configuration.
...
5. Alex returns to Advanced and clears the server configuration. The Organization server section returns to standard Micx Cloud, and the temporary bootstrap file is gone without resetting workspaces.
