...
```text
Create a short welcome checklist for this Micx workspace. Use exactly three bullets and mention one thing I can do next.
```
...
**Why**: When a user opens Micx for the first time with zero
...
Pass criteria:
- URL is `/welcome`.
- Heading "Welcome to Micx" is visible.
...
3. Expect: the local panel shows:
   ...
   - Explanation text: "This folder becomes your workspace. Micx
     will be able to:"
...
**Why**: Users connecting to a remote Micx server from the welcome
flow should also have onboarding marked complete.

Steps:
1. From `/welcome`, click "Get started".
2. Click "Connect custom remote".
3. Enter a valid Micx server URL.
...
## Flow 28 — Micx Models path explains payment before value

**Why**: Micx Models is the preferred business path, but it must make the tradeoff clear: pay/sign in through Micx Cloud to skip API keys, then return to task execution.

Steps:
1. Start from a clean first-run app at `/welcome` with no workspaces.
2. Create a local workspace through the welcome CTA.
3. On “Power your first task,” choose “Use Micx Models.”
4. Expect: the user sees the Micx Models value proposition and sign-in/payment CTA.
5. If a paid/signed-in test account is available, complete sign-in, select an Micx model, and run the recommended first task prompt.
