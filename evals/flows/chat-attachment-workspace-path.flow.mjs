      ...
      run: async (ctx) => {
        await ctx.prove("A normal chat attachment appears in the composer before sending", {
          ...
          assert: async () => {
            ...
            const composer = await ctx.eval("window.__micx?.slice('composer')");
      ...
      run: async (ctx) => {
        await ctx.prove("The submitted user turn includes the worker inbox path", {
          ...
          assert: async () => {
            ...
            assertEvidence(ctx, text.includes(".opencode/micx/inbox/chat-attachments/"), "Transcript contains the workspace inbox path", text);
      ...
      run: async (ctx) => {
        await ctx.prove("The uploaded scan path is a real file path for tools such as Docling", {
          ...
          assert: async () => {
            ...
            assertEvidence(ctx, !text.toLowerCase().includes("ocr complete"), "The prompt does not claim Micx performed native OCR", text);
