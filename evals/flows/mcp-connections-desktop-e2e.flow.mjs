 ...
 * happens in the Micx DESKTOP APP, from the member's point of view:
 *
 ...
 *
 *   1. Jordan signs the DESKTOP APP into Micx Cloud (real handoff
 *      grant exchange against the local Den).
 *   2. The app auto-configures "Micx Cloud Control" — the /mcp/agent
 ...
 * - den-api at MICX_EVAL_DEN_API_URL with the seeded demo org.
 ...
 *   automatically with MICX_EVAL_MARK_VERIFIED_CMD if missing).
...
const DEN_WEB_URL = (process.env.MICX_EVAL_DEN_WEB_URL ?? DEN_API_URL).trim().replace(/\/+$/, "");
...
const WORKSPACE_PATH = "/tmp/micx-mcp-desktop-e2e";
...
export default {
  ...
  steps: [
    ...
    {
      name: "Member signs the desktop app into Micx Cloud (real handoff grant)",
      run: async (ctx) => {
        const alreadySignedIn = await ctx.eval("Boolean((localStorage.getItem('micx.den.authToken') ?? '').trim())");
        if (!alreadySignedIn) {
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        // Dismiss the Micx Models upsell if it appears.
        await ctx.eval(`(() => {
    ...
    {
      name: "Micx Cloud Control auto-configures with the member's token",
      run: async (ctx) => {
        ...
        await ctx.waitFor(
          "Boolean(localStorage.getItem('micx.den.mcp.sync'))",
          { timeoutMs: 120_000, label: "micx.den.mcp.sync marker" },
        );
        ctx.log(`marker: ${await ctx.eval("localStorage.getItem('micx.den.mcp.sync')")}`);
        await ctx.prove("Micx Cloud Control is configured in the member's app", {
          assert: async () => {
            await ctx.expectText("Micx Cloud Control", { timeoutMs: 30_000 });
          },
          screenshot: {
            name: "desktop-cloud-control-configured",
            claim: "The member's desktop app auto-configured the Micx Cloud Control connection after cloud sign-in.",
            requireText: ["Micx Cloud Control"],
