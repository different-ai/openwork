 ...
 *   2. MICX_EVAL_DEN_WEB_URL=http://127.0.0.1:3005 MICX_EVAL_WEB_CDP_ADMIN=http://127.0.0.1:9855 pnpm fraimz --flow org-delete-danger-zone --stack den
...
const OWNER_PASSWORD = `Micx-${RUN_TAG}!`;
...
async function signInBrowser(ctx, email, password, label) {
  ...
  await goToDenWeb(ctx, "/");
  await ctx.waitFor(`localStorage.getItem('micx:web:auth-token') === ${JSON.stringify(session.token)}`, { timeoutMs: 30_000, label: `${label} den-web token persisted` });
  return session;
}
