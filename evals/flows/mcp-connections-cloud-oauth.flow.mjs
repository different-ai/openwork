 ...
 * plain den-web page (Next.js, real path routing, no window.__micxControl),
 * so it skips ensureLightMode() (preserveTheme: true) and uses ctx.eval to
 * navigate instead of navigateHash.
 *
 * Prerequisites:
 * - den-api reachable at MICX_EVAL_DEN_API_URL, signed in with
 *   MICX_EVAL_DEMO_EMAIL / MICX_EVAL_DEMO_PASSWORD (defaults to the
 *   seeded demo owner).
 * - den-web reachable at MICX_EVAL_DEN_WEB_URL, pointed at that den-api.
