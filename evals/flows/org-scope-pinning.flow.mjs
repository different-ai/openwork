 ...
 *
 * Fix under test: den-web pins Connections requests with x-micx-org-id,
 ...
 * MICX_EVAL_DEN_MULTI_ORG so single-org CI setups skip instead of fail.
 */
...
function denWebUrl() {
  return (process.env.MICX_EVAL_DEN_WEB_URL ?? "").trim().replace(/\/+$/, "");
}
