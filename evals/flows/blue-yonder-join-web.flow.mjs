/**
 * Blue Yonder new-user first run — Den Web invite join + installer handoff.
 *
 * Required env:
 ...
 * - OPENWORK_EVAL_BLUE_YONDER_ADMIN_EMAIL: inviter/admin email (default chetan@blueyonder.dev).
 * - OPENWORK_EVAL_BLUE_YONDER_NEW_USER: invited member email (default venkat@blueyonder.dev).
...
const DEFAULT_ADMIN_EMAIL = "chetan@blueyonder.dev";
...
export default {
  id: "blue-yonder-join-web",
  title: "Blue Yonder new user joins in Den Web and gets the desktop installer",
  kind: "user-facing",
  preserveTheme: true,
  requiredEnv: ["OPENWORK_EVAL_DEN_API_URL", "OPENWORK_EVAL_DEN_WEB_URL"],
  steps: [
    {
      name: "API invite Venkat to Blue Yonder",
      run: async (ctx) => {
        const email = newUserEmail(ctx);
        ...
        assertEvidence(ctx, Boolean(state.inviteToken), "A pending Blue Yonder invitation exposes inviteToken", {
          email,
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Venkat opens the Blue Yonder join screen from the invite", {
          action: async () => {
            await clearDenWebSession(ctx);
            await navigateAbsolute(ctx, requireState(state.inviteUrl, "invite URL"));
            await ctx.waitForText("Join Blue Yonder", { timeoutMs: 45_000 });
          },
          assert: async () => {
            await ctx.expectText("Join Blue Yonder");
            await ctx.expectText(newUserEmail(ctx));
          },
          screenshot: {
            name: "join-screen",
            claim: "The brand-new invitee sees the Blue Yonder invite join screen before creating the account.",
            requireText: ["Join Blue Yonder", newUserEmail(ctx)],
    ...
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Venkat joins Blue Yonder and lands on the welcome step", {
          action: async () => {
            await fillPasswordAndJoin(ctx, password(ctx));
          },
          assert: async () => {
            await ctx.expectText("You're in, welcome to Blue Yonder", { timeoutMs: 45_000 });
          },
          screenshot: {
            name: "joined-welcome",
            claim: "The invite acceptance completes and welcomes Venkat to Blue Yonder.",
            requireText: ["You're in, welcome to Blue Yonder"],
            rejectText: ["Something went wrong"],
          },
        });
      },
    },
    {
      ...
      run: async (ctx) => {
        await ctx.prove("Venkat can get the desktop app installer from the welcome screen", {
          action: async () => {
    ...
    {
      name: "API sanity fixes Venkat's display name",
      run: async (ctx) => {
        state.newUserToken = await signInByEmail(ctx, newUserEmail(ctx));
        const patched = await denApiFetch(ctx, "/v1/me/profile", {
          method: "PATCH",
          headers: { authorization: `Bearer ${state.newUserToken}` },
          body: JSON.stringify({ firstName: "Venkat", lastName: "" }),
        });
        ...
        assertEvidence(ctx, name.includes("Venkat"), "GET /v1/me returns Venkat's updated display name", {
...
function adminEmail(ctx) {
  return envText(ctx, "OPENWORK_EVAL_BLUE_YONDER_ADMIN_EMAIL") || DEFAULT_ADMIN_EMAIL;
}
...
function password(ctx) {
  return envText(ctx, "OPENWORK_EVAL_BLUE_YONDER_PASSWORD") || DEFAULT_PASSWORD;
}
...
async function fillPasswordAndJoin(ctx, value) {
  ...
  await clickButtonStartingWith(ctx, "Join Blue Yonder", 30_000);
  await ctx.waitFor(`(() => {
    ...
    return text.includes("You're in, welcome to Blue Yonder") || text.includes("You're one click away from the team workspace.") || (!passwordVisible && buttons.some((button) => button.startsWith('Join Blue Yonder')));
  })()`, { timeoutMs: 60_000, label: "join success or signed-in confirmation" });
  const clickedConfirm = await clickButtonStartingWithIfVisible(ctx, "Join Blue Yonder");
  if (clickedConfirm) {
    ctx.log("Clicked the signed-in Join Blue Yonder confirmation button.");
  }
  await ctx.waitForText("You're in, welcome to Blue Yonder", { timeoutMs: 60_000 });
}
...
async function captureInstallerTarget(ctx) {
  ...
  await ctx.eval(`(() => {
    ...
    window.__blueYonderDownloadCapture = { opened: '', downloadHref: '', before: ${JSON.stringify(before)} };
    if (!window.__blueYonderOriginalOpen) window.__blueYonderOriginalOpen = window.open;
    window.open = function (url, ...rest) {
      window.__blueYonderDownloadCapture.opened = String(url ?? '');
      return window.__blueYonderOriginalOpen.call(window, url, ...rest);
    };
    document.addEventListener('click', (event) => {
      const anchor = event.target?.closest?.('a[download], a[href]');
      if (anchor) window.__blueYonderDownloadCapture.downloadHref = anchor.href || anchor.getAttribute('href') || '';
    }, { capture: true });
  ...
  const url = await ctx.waitFor(`(() => {
    const capture = window.__blueYonderDownloadCapture;
  ...
  await ctx.eval(`(() => {
    if (window.__blueYonderOriginalOpen) window.open = window.__blueYonderOriginalOpen;
    return true;
  })()`).catch(() => undefined);
