 ...
 * - The connection appears in Micx Connect as an org MCP item that needs
 *   Jordan's sign-in.
 * - Jordan clicks through Connect, the OS browser completes a real OAuth
 *   round trip, and the item becomes Ready in Connect.
 * - A real chat turn executes the external MCP tool through Micx Cloud
...
const WORKSPACE_PATH = `/tmp/micx-desktop-org-mcp-demo-${RUN_TAG}`;
...
async function readRuntimeCloudControlMcp(ctx) {
  ...
  return ctx.eval(`(async () => {
    ...
    const entry = items.find((item) => item.name === 'micx-cloud');
...
async function waitForRuntimeCloudControlMcp(ctx) {
  ...
  ctx.assert(false, `Runtime Micx Cloud Control MCP config never became ready: ${JSON.stringify(last)}`);
}
    ...
    created = await ctx.eval(`(async () => {
      try {
        ...
        const response = await fetch(base + '/workspaces/local', {
          method: 'POST',
          headers,
          body: JSON.stringify({ folderPath: ${JSON.stringify(WORKSPACE_PATH)}, name: 'micx-desktop-org-mcp-demo', preset: 'starter' }),
        });
        const text = await response.text();
        let payload = null;
        try { payload = JSON.parse(text); } catch {}
...
export default {
  ...
  steps: [
    ...
    {
      name: "Jordan discovers the org MCP connection in Micx Connect",
      run: async (ctx) => {
        ...
        await ctx.prove("Jordan sees the org MCP connection in Micx Connect, with a connect action", {
          voiceover: "Jordan opens Micx Connect and sees the org-shared MCP connection under Needs your sign-in, clearly marked as something they can connect with their own account.",
          assert: async () => {
          ...
          screenshot: {
            ...
            claim: "Micx Connect shows the org-published MCP connection with a 'Connect your account' action.",
    ...
    {
      name: "The item becomes ready in Micx Connect",
      run: async (ctx) => {
        await openConnectSettings(ctx);
        await waitForConnectReadyRow(ctx, CONNECTION_NAME);
        await ctx.prove("After the browser sign-in completes, the same org MCP item is ready in Micx Connect", {
          voiceover: "The OAuth callback completed in the browser, and the desktop did not need a manual reload. Jordan's row is now ready in Micx Connect as a connected MCP integration.",
          assert: async () => {
            await ctx.waitFor(
              `(() => {
                const row = [...document.querySelectorAll('[data-testid="connect-organization-row"]')]
          ...
          screenshot: {
            ...
            claim: "The org MCP connection is marked Ready in Micx Connect after the browser OAuth round trip.",
    ...
    {
      name: "Micx Cloud Control is ready",
      run: async (ctx) => {
        ...
        const alreadyConnected = await ctx.eval(`(() => {
          const card = [...document.querySelectorAll('button')].find((el) => el.textContent.includes('Micx Cloud Control'));
        ...
        if (!alreadyConnected) {
          ...
          ctx.assert(opened, "Could not open the Micx Cloud Control card.");
          ...
          ctx.assert(clicked, "Could not click Connect for Micx Cloud Control.");
        }
        ...
        await ctx.waitFor(
          ...
          { timeoutMs: 60_000, label: "Micx Cloud Control connected card" },
        );
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        await ctx.prove("Jordan's agent can immediately execute the newly connected org MCP tool", {
          voiceover: "The connection is not just a settings card. In a real desktop chat, the agent discovers the organization's MCP capability through Micx Cloud Control and executes it with Jordan's own connected account.",
          action: async () => {
            await ctx.waitFor(
              "window.__micxControl?.listActions?.().find((a) => a.id === 'session.create_task')?.disabled === false",
            ...
            const prompt = `Use the Micx Cloud Control connection: call search_capabilities with query "echo", then call execute_capability with the exact match name and body {"text":"${ECHO_TEXT}"}. Reply with the exact text the tool returned.`;
            await ctx.control("composer.set_text", { text: prompt });
            await ctx.waitFor(
              "window.__micxControl?.listActions?.().find((a) => a.id === 'composer.send')?.disabled === false",
          ...
          screenshot: {
            ...
            claim: "A real desktop chat executed the org MCP tool through Micx Cloud Control and showed the exact result.",
