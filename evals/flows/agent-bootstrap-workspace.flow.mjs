const BOOTSTRAP = {
  ...
  prepared: {
    ...
    skillTitle: "First Micx Skill",
    skillsDir: "/tmp/micx-agent-bootstrap-skills",
    skillPath: "/tmp/micx-agent-bootstrap-skills/first-micx-skill/SKILL.md",
  ...
  claimLinks: [
    {
      ...
      url: "https://app.micxlabs.com/workspace-claim?token=eval-token-not-real",
...
export default {
  ...
  spec: "packages/micx-bootstrap/start.md",
  steps: [
    {
      ...
      run: async (ctx) => {
        await ctx.prove("A non-email bootstrap opens the desktop app to setup-complete onboarding", {
          action: async () => {
            ...
            await ctx.eval(`(() => localStorage.removeItem("micx.den.settings"))()`);
          ...
          assert: async () => {
            ...
            const marker = await ctx.eval(`(() => {
              const prepared = document.querySelector('[data-micx-prepared="true"]');
              const provisional = document.querySelector('[data-micx-provisional="true"]');
