...
handoff explicit: install Micx Desktop, sign in, then access organization
marketplaces from the app.

## Preconditions

- Run Den Web and Den API from a fresh Daytona server sandbox or a clean local DB.
- Use `MICX_DEV_MODE=1` for local/Daytona email-password sign-up without email delivery.
...
- `Micx Marketplace`
  - Description explains built-in Micx AI capabilities available in desktop after sign-in.
...
6. Confirm the onboarding screen includes:
   ...
   - Desktop guidance: download Micx Desktop, sign in with the same account, then open Marketplace.
   - A visible `Micx Marketplace` card.
   - A visible `Anthropic-Compatible Plugins` card.
   - A link or visible reference to `anthropics/knowledge-work-plugins`.
   - Micx Connect endpoint `https://api.micxlabs.com/mcp/agent`, with OpenCode verified and setup guides for Codex, Cursor Web/Agents, ChatGPT Desktop, Claude Code, VS Code, and other MCP clients.
...
1. Start Micx Desktop in dev mode pointed at the fresh Den Web/API URLs.
2. Open Settings -> Extensions -> Marketplace while signed out.
3. Confirm signed-out copy says Micx is usable without an account and sign-in unlocks Marketplace/built-ins/org marketplaces.
4. Sign in to Micx Cloud with the same Den account.
5. Return to Marketplace and refresh if needed.
6. Confirm:
   - `Micx Marketplace` appears as a marketplace source/filter.
...
- Den sandbox: `micx-server-onboarding-20260603-2108`.
- Electron sandbox: `micx-test-20260603-211949`.
...
- Desktop Marketplace showed `Micx Marketplace` and `Anthropic-Compatible Plugins`.
...
- Marketplace list shows only `Micx Marketplace`.
