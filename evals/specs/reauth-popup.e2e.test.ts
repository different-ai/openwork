...
test("workspace SSO verifies through a real popup and safely recovers from interrupted sign-in", async ({ world, user, probe, evidence, step }) => {
  ...
  await step("successful SSO resumes and persists the pending settings change", async () => {
    ...
    await user.screenshot();
    popup.client.close();
    evidence.recordAssertionEvidence("Real OIDC verification closes both surfaces and resumes a server-validated settings mutation", "A session aged beyond 2 hours could not save; signed-token approval refreshed authentication, showed completion, closed the popup/dialog, and persisted the new workspace name across reload.", true);
  });
  ...
  await step("routine plugin work on shared plugins never asks for verification", async () => {
    ...
    for (const [label, result, status] of [["rename", renamed, 200], ["grant", grant, 201], ["mcp", mcp, 201]] as const) {
      expect({ label, status: result.response.status }).toEqual({ label, status });
      expect(JSON.stringify(result.body)).not.toContain("fresh_auth_required");
    }
    evidence.recordAssertionEvidence("Routine plugin changes skip the identity check", "With a three-hour-old session, renaming a shared plugin, sharing it, and adding an MCP config all succeeded without fresh_auth_required.", true);

    await user.navigate(editUrl);
    await user.see({ text: "Edit shared-editing" }, { timeoutMs: 60_000 });
    await user.type(skillBody, "Saved without another prompt.", { replace: true });
    ...
    expect(stored.source).toContain("Saved without another prompt.");
    expect(stored.versions).toBe(2);
    evidence.recordAssertionEvidence("An old session saves shared skill content with no verification dialog", "The UI saved and navigated to the skill; exactly one new version persisted and no dialog opened. Workspace security settings above still required verification.", true);
    await user.screenshot();
  });
});
