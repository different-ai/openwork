...
test("OpenWork system-prompt hooks keep the engine request at one system message", async ({ evidence }) => {
  ...
  // first, then extensions (app mechanics, live steering, catalogs), then the
  // spreadsheets plugin (workbook tool guidance).
  ...
  evidence.recordAssertionEvidence(
    ...
    "Running the capabilities-knowledge, extensions-preview, and spreadsheets transform hooks in registration order leaves the engine system array at length 1, with the engine header first and every OpenWork section retained in order (rules, app mechanics, browser, live steering, then workbook guidance); an empty engine array also yields exactly one non-empty entry.",
    true,
  );
});
