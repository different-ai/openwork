 ...
 * so this holds with no network at all; while policy sign-in is suspended, the
 * activated install lands on its local session shell.
 *
...
const SIGN_IN_GATE_HEADING = "Welcome to OpenWork";
...
test("an activated enterprise install boots past the activation gate without a render crash", async ({ world, user, probe, evidence }) => {
  ...
  // Stop on the first of: the session shell, either gate, the recovery
  ...
  const rootText = await probe.eventually(() => world.rootText(), {
    ...
    until: (text) => text.includes(SESSION_HEADING) || text.includes(SIGN_IN_GATE_HEADING)
      || text.includes(ACTIVATION_GATE_HEADING) || RECOVERY_HEADING.test(text) || world.exceptions().some(isRenderCrash),
  });
  ...
  expect(rootText, "an activated install retained the suspended policy sign-in gate").not.toContain(SIGN_IN_GATE_HEADING);
  ...
  await user.notSee({ text: SIGN_IN_GATE_HEADING });
  ...
  expect(final.rootText, "the activated local session shell disappeared during settle").toContain(SESSION_HEADING);
  expect(final.rootText, "an activated install returned to the activation gate").not.toContain(ACTIVATION_GATE_HEADING);
  expect(final.rootText, "an activated install returned to the suspended policy sign-in gate").not.toContain(SIGN_IN_GATE_HEADING);
  ...
  evidence.recordAssertionEvidence(
    `An activated enterprise install with Den at ${world.denBaseUrl} (closed port) mounts "${SESSION_HEADING}" and shows neither "${ACTIVATION_GATE_HEADING}" nor "${SIGN_IN_GATE_HEADING}"`,
    ...
    exceptions.filter(isRenderCrash).length === 0 && unexpected.length === 0
      && final.rootText.includes(SESSION_HEADING)
      && !final.rootText.includes(ACTIVATION_GATE_HEADING)
      && !final.rootText.includes(SIGN_IN_GATE_HEADING),
  );
});
