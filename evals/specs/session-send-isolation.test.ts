...
test("scoped send preflight and Stop retain ownership, interruption and environment contracts", { timeout: 120_000 }, async ({ evidence }) => {
  ...
  const files = ["tests/session-stop-refresh.test.ts", "tests/session-send-isolation.test.ts", "tests/env-context.test.ts", "tests/send-step-diagnostics.test.ts", "tests/session-ownership.test.ts", "tests/opencode-session-native.test.ts", "tests/safe-edit-resend.test.ts", "tests/session-history.test.tsx", "tests/session-scroll.test.tsx"];
