...
import { createOrg, needs, queryDenDatabase, server, test } from "@openwork/testkit";
...
test("organization billing survives selection and blocks destructive deletion", { timeout: 300_000 }, async ({ evidence, place }) => {
  needs({ placement: "local", env: ["OPENWORK_EVAL_MYSQL_URL"] });
  await using den = await server({ place, web: false, org: { name: "Billing Recovery", members: {} }, env: {
  ...
  await using subscribed = await createOrg(den, "Subscribed Workspace");
  ...
  try {
    await queryDenDatabase(databaseUrl,
      ...
      ["orgSubscription_synthetic_billing", subscribed.id]);
