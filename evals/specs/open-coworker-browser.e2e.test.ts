...
import { BROWSER_INSPECT, BROWSER_OTHER, BROWSER_REOPEN, BROWSER_REVIEW, BROWSER_START, coworkerBrowserWorld } from "../worlds/coworker-browser.ts";
...
browserTest("A native Coworker browser turn keeps pages owned while discussions and panels change", { timeout: 300_000 }, async ({ world, user, step }) => {
  ...
  const send = async (prompt: string, reply: string, finalReceipt?: string) => {
    await user.type({ label: "Message Editor" }, prompt);
  ...
  await step("The real engine opens, snapshots, evaluates and screenshots its owned HTTP page", async () => {
    for (const prompt of [BROWSER_START, BROWSER_INSPECT, BROWSER_REVIEW, BROWSER_OTHER, BROWSER_REOPEN]) {
  ...
  await step("Closing the last tab and reopening at unchanged bounds restores the page inside the discussion", async () => {
    ...
    await expect.poll(() => world.ui(), { timeout: 10_000 }).toMatchObject({ bounds: before.bounds });
    ...
    await expect.poll(async () => {
      ...
      if (!isRecord(current.bounds) || !isRecord(page)) throw new Error("No reopened viewport bounds");
      // Native bounds round both CSS edges to whole device-independent pixels.
  ...
  await step("Fresh single-use snapshot IDs dispatch click and fill, while a consumed ID dispatches no input", async () => {
    await user.press("Escape");
    await expect.poll(() => world.ui(), { timeout: 10_000 }).toMatchObject({ aside: { width: 56, collapsed: "true" } });
