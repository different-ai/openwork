...
End-to-end scenarios that run against a real Electron Micx instance in a
...
Use the helper. It creates from the reusable `micx-eval-vnc` snapshot,
...
The reusable `micx-eval-secrets` volume is mounted at `/daytona-secrets`.
...
the reusable `micx-eval-artifacts` volume at `/daytona-artifacts`, starts a
...
The `@micx/testkit` ambient tape binds observable assertions to validated
...
Should return the Micx page target.

### 4. Verify opencode sidecar

```bash
daytona exec micx-test 'ps aux | grep opencode | grep -v grep'
```
...
1. Create the workspace directory on the sandbox:
   ```bash
   daytona exec micx-test 'mkdir -p /workspace/hello'
   ```
...
1. Start the reusable mock OAuth MCP server in the Daytona sandbox:
   ```bash
   daytona exec micx-test 'bash -lc "cd /workspace && nohup env PORT=3978 HOST=127.0.0.1 AUTO_APPROVE=1 node scripts/mock-oauth-mcp-server.mjs > /tmp/mock-mcp.log 2>&1 &"'
   ```
...
2. Verify the mock server is healthy:
   ```bash
   daytona exec micx-test 'bash -lc "curl -s http://127.0.0.1:3978/health"'
   ```
...
8. Wait up to 30s, then verify the mock OAuth server saw an authorization
   ...
   ```bash
   daytona exec micx-test 'bash -lc "curl -s http://127.0.0.1:3978/requests"'
   ```
...
5. Create or sign in to a Den Web account, then use the desktop handoff code or
   full `micx://den-auth?...` link in `Paste sign-in code`.
...
- Server sandbox: `micx-server-20260602-154721`
- Electron sandbox: `micx-test-20260602-155000`
...
3. Restart Electron without local AI-provider secrets so the baseline has no
   ...
   ```bash
   DAYTONA_SECRETS_ENV=/tmp/no-daytona-secrets bash /opt/micx-daytona/start-daytona-electron.sh --detach
   ```
...
6. Local file checks:
   - Confirm Micx-owned cloud import metadata is stored in the Micx
     runtime DB, not `.opencode/micx.json`.
...
```bash
daytona stop micx-test    # preserves state
daytona delete micx-test  # destroys everything
```
...
```bash
daytona exec micx-test 'bash -lc "pkill -f electron || true; pkill -f electron-dev || true"'
sleep 3
daytona exec micx-test 'bash -lc "cd /workspace && bash /opt/micx-daytona/start-daytona-electron.sh --detach"'
```

`[micx] Electron CDP exposed...` only means Micx requested CDP. The real
