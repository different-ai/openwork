...
export async function sessionHome(seed: Seed) {
  ...
  const mock = seed.mock({ isolatedProcessEnv: true, agentWorkloads: [
    { promptMarker: prompt, finalReply: "Waiting finished.", steps: [
  ...
  return { app, workspace, session, home, destination, prompt, followup, reply,
